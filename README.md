...
| Features | Frontend-agnostic `gb-core`, battery saves, save states, rewind, fast forward, real boot-ROM `DMG`/`CGB`/`AGB`/`SGB`/`SGB2`, `GB ADVANCE` / `AGB` GB/C compatibility profile for GBA Enhanced detection, Game Boy Printer, [Pokémon Pikachu Color](docs/info/INFRARED.md#pok%C3%A9mon-pikachu-color), [Custom GSC Mystery Gift IR Sender](docs/info/INFRARED.md#custom-gsc-mystery-gift-ir-sender) |
