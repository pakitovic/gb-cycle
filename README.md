...
Release versioning is a two-stage manual GitHub Actions flow so `main` can stay protected while the release itself remains autonomous:

1. Run `release-prepare` with the crate SemVer version, for example `0.1.8` or `v0.1.8`. It normalizes the version, rejects an existing tag or GitHub Release, checks out the selected workflow ref unless `source_ref` is provided, merges the latest `base_branch` (`main` by default), runs `scripts/bump-workspace-version.sh`, validates locked Cargo metadata and formatting, pushes `codex/release-<version>`, opens or updates a ready PR titled `chore(release): bump crates to <version>`, and explicitly dispatches `ci.yml`, `test-roms.yml`, and `test-roms-extra.yml` for that PR branch. Use `dry_run` to validate without pushing a branch or creating a PR, and disable `dispatch_checks` only when you intend to run checks yourself.
2. Merge that PR to `main` after the dispatched checks are green.
3. Run `release-version` from `main` with the same SemVer version. It rejects older versions or existing tags, verifies that every workspace crate and `Cargo.lock` already match the requested version, creates the annotated `v<version>` tag, creates the GitHub Release, and explicitly dispatches `release-linux.yml`, `release-macos.yml`, and `release-windows.yml` at that tag so their assets attach to the release. Use `dry_run` when you only want validation without creating the tag, GitHub Release, or package workflows. Prerelease versions such as `0.1.8-rc.1` create GitHub prereleases; SemVer build metadata is intentionally not accepted for crate-release automation.

The workflows need repository Actions permission to write contents, pull requests, and dispatch workflows. The prepare workflow dispatches checks explicitly because branches and pull requests created with `GITHUB_TOKEN` should not rely on implicit event recursion; the publish workflow dispatches package builds explicitly for the same reason. If you need a local fallback, `scripts/bump-workspace-version.sh 0.1.8` still updates every workspace crate package version plus internal workspace dependency requirements and `Cargo.lock`.
