...
fn build_direct_mode2_oam_access_program() -> Vec<u8> {
    let seed_bytes = [0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0, 0x5A, 0xA5];
    let mut program = ProgramBuilder::default();

    program.xor_a();
    program.ld_a16_from_a(0xFF40);
    seed_oam_bytes(&mut program, 0xFE30, &seed_bytes);
...
fn build_fea0_mode2_read_program() -> Vec<u8> {
    let seed_bytes = [
        0x57, 0x13, 0x68, 0x24, 0xAA, 0xAA, 0xBB, 0xBB, 0x0F, 0x0F, 0xCC, 0xCC, 0xDD, 0xDD, 0xEE,
        0xEE,
    ];
    let mut program = ProgramBuilder::default();
...
fn build_hli_hld_program() -> Vec<u8> {
    ...
    seed_oam_bytes(&mut program, 0xFE28, &phase_one_seed_bytes);
    program.ld_hl_imm(0xFE38);
    program.ld_a_imm(0x80);
    program.ld_a16_from_a(0xFF40);
    program.push_nops(5);
    program.ld_a_hli();
    ...
    seed_oam_bytes(&mut program, 0xFE50, &phase_two_seed_bytes);
    program.ld_hl_imm(0xFE58);
    program.ld_a_imm(0x80);
    program.ld_a16_from_a(0xFF40);
    program.ld_a_imm(0x99);
    program.push_nops(7);
...
fn build_stack_and_interrupt_service_program() -> Vec<u8> {
    let phase_one_seed_bytes = [
        ...
        0x30, 0x0F, 0x0F, 0x40, 0x40, 0x50, 0x50, 0x60, 0x60,
    ];
    let phase_two_seed_bytes = [
        0x78, 0x56, 0x11, 0x11, 0xAA, 0x00, 0x22, 0x22, 0x34, 0x12, 0x33, 0x33, 0x44, 0x44, 0x55,
        0x55, 0x0F, 0x0F, 0x66, 0x66, 0x77, 0x77, 0x88, 0x88,
    ];
    let mut program = ProgramBuilder::default();
    ...
    seed_oam_bytes(&mut program, 0xFE70, &phase_two_seed_bytes);
...
fn phase_4_direct_mode2_oam_access_rom_fixture_matches_expected_oam_state_and_trace() {
    ...
    apply_write_corruption(&mut expected, 7);

    assert_machine_rows(&mut machine, &expected, &[6, 7]);
}
...
fn phase_4_fea0_mode2_read_rom_fixture_matches_expected_oam_state_and_trace() {
    ...
    write_expected_row(
        &mut expected,
        7,
        [0x0F, 0x0F, 0xCC, 0xCC, 0xDD, 0xDD, 0xEE, 0xEE],
    );
    apply_read_corruption(&mut expected, 7);
...
fn phase_4_hli_hld_rom_fixture_matches_expected_oam_state_and_trace() {
    ...
    apply_read_with_incdec_corruption(&mut expected, 7);
    ...
    apply_write_corruption(&mut expected, 11);

    assert_machine_rows(&mut machine, &expected, &[5, 6, 7, 10, 11]);
}
...
fn phase_4_stack_and_interrupt_service_rom_fixture_matches_expected_oam_state_and_trace() {
    ...
    write_expected_row(
        &mut expected,
        7,
        [0x55, 0x55, 0x10, 0x10, 0x20, 0x20, 0x30, 0x30],
    );
    write_expected_row(
        &mut expected,
        8,
        [0x0F, 0x0F, 0x40, 0x40, 0x50, 0x50, 0x60, 0x60],
    );
    apply_write_corruption(&mut expected, 7);
    ...
    write_expected_row(
        &mut expected,
        14,
        [0x78, 0x56, 0x11, 0x11, 0xAA, 0x00, 0x22, 0x22],
    );
    write_expected_row(
        &mut expected,
        15,
        [0x34, 0x12, 0x33, 0x33, 0x44, 0x44, 0x55, 0x55],
    );
    write_expected_row(
        &mut expected,
        16,
        [0x0F, 0x0F, 0x66, 0x66, 0x77, 0x77, 0x88, 0x88],
    );
    apply_write_corruption(&mut expected, 15);
    apply_write_corruption(&mut expected, 16);

    assert_machine_rows(&mut machine, &expected, &[6, 7, 8, 14, 15, 16]);
}
