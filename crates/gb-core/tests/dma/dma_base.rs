...
fn dma_trace_shows_start_and_completion_points_with_progress_metadata() {
    ...
    assert!(trace.contains(
        "subsystem=dma level=trace message=\"t_cycle=648 phase=autonomous_peripheral_ticks console_model=GameBoy status=Ready transfer_state=Completed transfer_kind=Oam transfer_family=FullBurst block_size=1 advance_condition=EveryTCycle oam_speed_mode=Normal lcd_domain_duration_dots=648 first_byte_delay_t_cycles=8 first_byte_delay_remaining_t_cycles=0 cpu_bus_restriction_delay_t_cycles=5 cpu_bus_restriction_delay_remaining_t_cycles=0 cpu_bus_restriction_active=true elapsed_t_cycles=648 completed_bytes=160 remaining_bytes=0 completed_blocks=160 remaining_blocks=0 byte_phase_t_cycles=0 total_t_cycles=648 cpu_access_policy=Unrestricted active_region=None"
    ));
