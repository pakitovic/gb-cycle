...
where
    ...
    W: std::io::Write,
{
    gb_benchmark::bench::run_bench_command(arguments, output)
}
...
mod tests {
    ...
    fn run_help_succeeds() {
        ...
        assert!(output.contains("cargo rom-bench"));
    }
    ...
    fn exit_code_returns_one_and_writes_parse_errors() {
        ...
        let code = super::exit_code(["--template", "case.toml"], &mut output, &mut error);
        ...
        assert!(
            String::from_utf8(error)
                ...
                .contains("--template requires --generate-cases")
        );
    }
}
