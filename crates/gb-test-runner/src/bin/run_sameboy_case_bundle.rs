...
where
    ...
    W: std::io::Write,
{
    gb_test_runner::run_sameboy_case_bundle_command(arguments, output)
}
...
fn main() {
    let code = exit_code(env::args().skip(1), &mut io::stdout(), &mut io::stderr());
    if code != 0 {
        process::exit(code);
    }
}
...
mod tests {
    ...
    fn exit_code_returns_one_and_writes_parse_errors() {
        ...
        let code = super::exit_code(["--timeout-tcycles", "nope"], &mut output, &mut error);
        ...
        assert!(
            String::from_utf8(error)
                ...
                .contains("invalid --timeout-tcycles value")
        );
    }
}
