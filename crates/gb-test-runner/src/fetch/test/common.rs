...
use super::super::git::scrub_inherited_git_repository_context;
use super::super::manifest::{DATA_DIR, REPORTS_MANIFEST_PATH, Report};

pub(super) fn unique_temp_dir(label: &str) -> PathBuf {
    super::super::git::unique_temp_path(&format!("fetch-{label}"))
}
...
pub(super) fn write_source_manifest(workspace_root: &Path, relative_path: &str, text: &str) {
    let path = workspace_root.join(DATA_DIR).join(relative_path);
...
pub(super) fn write_basic_reports(workspace_root: &Path, source_path: &str) {
    write_reports(
        workspace_root,
        &format!(
            concat!(
