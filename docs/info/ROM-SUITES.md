...
- `GB_CYCLE_TEST_ROM_STARTUP` — legacy-only local startup override for `run_linked_session`; `cargo rom-suite` and `cargo rom-suite-link` never read it and use explicit `startup` manifest fields plus `--boot-rom-dir <dir>` instead.
