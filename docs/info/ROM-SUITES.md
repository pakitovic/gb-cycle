...
- The old single-machine manifest inventories have been retired; new external single-machine lanes must use `*.suite.toml` plus a report-local `sources.report.toml`. Linked-session manifests remain as the explicit exception until linked sessions get a report-local model; they live as `crates/gb-test-runner/data/docboy-dmg-linked.toml` and `crates/gb-test-runner/data/linked-*.toml` and are executed with `run_linked_session`. The root `crates/gb-test-runner/data/sources.toml` is retained only as a consultation/provenance list for the DocBoy DMG linked ROMs used by `docboy-dmg-linked.toml`; it is not a fetch entry point and must not be expanded for new single-machine lanes.
...
- Built-in single-machine external ROM suites live only as report-local `*.suite.toml` manifests under `crates/gb-test-runner/data/<report>/`, with materialization metadata in that report's `sources.report.toml`; the repo must not add legacy single-machine root manifests or report-local non-`.suite.toml` copies. Linked-session TOMLs are the only remaining non-suite external manifests and stay under `crates/gb-test-runner/data/` until a linked-session report model exists.
...
Use `cargo rom-suite` for report-local external ROM suites; it always writes failure artifacts under the selected report's `.artifacts/<suite>/<case>/`. The older `run_rom_suite` binary is not a repo external-data path anymore; keep it only for synthetic or local/private manifests outside `crates/gb-test-runner/data/`.
...
- `GB_CYCLE_TEST_ROM_ROOT` — store override for non-report runner paths such as `run_rom_suite` and linked-session cases that reference repo-managed `test/...` inputs. The new `cargo rom-fetch` and `cargo rom-suite` report paths materialize under the repository `test/` directory and do not read this variable.
