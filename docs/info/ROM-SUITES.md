...
| `mooneye`, `little-things-gb`, `turtle-tests`, `ashiepaws`, `nitro2k01`, `magen`, `mealybug-tearoom-tests`, `samesuite`, `rtc3test`, `mbc3-tester`, `gambatte` | `cargo rom-suite` | Standalone exploratory report channels; `mooneye`, `little-things-gb`, `turtle-tests`, `ashiepaws`, `mealybug-tearoom-tests`, `rtc3test`, `mbc3-tester`, and `gambatte` are archive-backed by c-sp `game-boy-test-roms`, `nitro2k01` is pinned to upstream nitro2k01 release assets with local placeholder framebuffer fixtures, and Little Things, TurtleTests, Ashiepaws, Mealybug, SameSuite, Nitro2k01, MBC3 Tester, and Gambatte stay out of `test-roms-extra` while their v7 inventories, manual fixtures, or exposed timing/admission gaps are validated manually. |
...
The standalone `gambatte` report materializes the c-sp `game-boy-test-roms` v7.0 `gambatte/` ROMs that have PNG framebuffer fixtures under `/test/gambatte/gambatte/`. It is split into one suite manifest per fixture-bearing upstream folder, runs final framebuffer checks after the upstream `15`-frame Gambatte test-runner window, uses `DMG` rows for `dmg08` and `.gb` no-suffix fixtures with alternate fixture arrays where both files describe the same ROM/model configuration, CPU-CGB-C rows with `source = "cgb"`, per-case revision suffixes only for `_cgb04c` fixtures, unlabeled CGB fixtures that run on CPU-CGB-C without exact revision report suffixes, and AGB rows with `source = "cgb"` for `_gba` fixtures while DMG/AGB rows avoid exact revision suffixes unless upstream names a concrete revision. The eight `_x*` fixtures stay listed as disabled rows until their upstream hardware-label semantics are confirmed, and CGB rows currently use the runner's palette-rank framebuffer oracle instead of Gambatte's exact RGB conversion formula.
...
- GitHub `test-roms-extra` runs explicitly promoted standalone report lanes with `cargo rom-suite <report>`; any promoted row that intentionally depends on manifest-declared RealBoot must first run `cargo rom-fetch --boot-rom <dir>` into runner-local temporary storage and then pass `--boot-rom-dir <dir>`. `gbmicrotest` is temporarily commented out while the c-sp v7 import is baselined, `rtc3test` remains the replacement for the removed standalone `ax6` report lane, `little-things-gb` is temporarily commented out during the c-sp v7 Telling LYs joypad IRQ entropy investigation, `turtle-tests` stays Pages-only while its c-sp v7 inventory is baselined, `ashiepaws` stays Pages-only while its c-sp v7 inventory is baselined, `nitro2k01` stays local while placeholder framebuffer fixtures are replaced with manual fixtures, standalone `mealybug-tearoom-tests` plus `samesuite` are temporarily commented out while their c-sp v7 inventories are validated manually, `mbc3-tester` stays out of this CI lane because it is intentionally Strict-red until the MBC30 ROM-only admission policy is resolved, and `gambatte` stays Pages-only while the fixture-backed subset and deferred Gambatte-specific oracle gaps are baselined.
