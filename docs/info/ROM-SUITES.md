...
- `crates/gb-test-runner/data/gb-emulator-shootout/sources.toml` is the source of truth for the promoted GB Emulator Shootout report catalog: upstream source IDs, pinned revisions, required file paths, SHA-256 hashes, source-family aliases, and materialized family names used by the stable fetch path; `cargo rom-fetch gb-emulator-shootout` and `cargo rom-suite gb-emulator-shootout` are the promoted report workflow.
- `crates/gb-test-runner/data/docboy/sources.toml` is the source of truth for the DocBoy single-machine report catalog: upstream source ID `docboy`, pinned revision, required ROM/result fixture paths, SHA-256 hashes, and materialized family names used by report-local DocBoy materialization.
- `crates/gb-test-runner/data/gbmicrotest/sources.toml` is the source of truth for the gbmicrotest report catalog: local source ID `gbmicrotest`, the pinned DocBoy revision, the 438 required ROM paths, SHA-256 hashes, and the flat `/test/gbmicrotest/<rom>` materialization used by `cargo rom-fetch gbmicrotest` and `cargo rom-suite gbmicrotest`.
...
- Built-in promoted GB Emulator Shootout manifests live under `crates/gb-test-runner/data/gb-emulator-shootout/*.toml` with report-local `*.suite.toml` copies for the new runner; built-in DocBoy single-machine manifests live under `crates/gb-test-runner/data/docboy/*.toml` with report-local `*.suite.toml` copies for `cargo rom-suite docboy`; the stable gbmicrotest manifest lives under `crates/gb-test-runner/data/gbmicrotest/gbmicrotest.toml` and its new runner manifest copy lives under `crates/gb-test-runner/data/gbmicrotest/gbmicrotest.suite.toml`; built-in legacy extra/internal manifests remain under `crates/gb-test-runner/data/*.toml`; local/private manifests passed with `--manifest` may live outside the repo.
...
Promoted `gb-emulator-shootout` and standalone `gbmicrotest` report lanes no longer have Makefile aggregate or member targets. Fetch them with `cargo rom-fetch <report> [family ...]` and run them with `cargo rom-suite <report> [--suite <suite>]`; `cargo rom-suite` keeps running later cases in a selected suite/report after earlier case failures and returns non-zero at the end if any selected case failed. Non-DocBoy extra/internal rows share the legacy `/test/test-report-extra.md`. Large DocBoy single-machine materialization owns the `docboy` report channel at `/test/docboy/`; its old Makefile aggregate was removed and the new runner executes the report-local `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext` `*.suite.toml` manifests through `cargo rom-suite docboy`. gbmicrotest rows share the `gbmicrotest` report channel at `/test/gbmicrotest/`. Legacy Makefile failure artifacts share `/test/.artifacts/`; `cargo rom-suite` failure artifacts live under the selected report's `.artifacts/<suite>/<case>/`. Linked-session rows such as `docboy-dmg-linked-extra` and `linked-cgb-ir-smoke` print participant-scoped status to stdout and retain failure artifacts, but they do not currently append Markdown report rows.
...
| `cargo rom-suite gb-emulator-shootout --suite samesuite` | `samesuite` | `samesuite`, `mooneye` | Consolidated promoted SameSuite lane: SGB command/multiplayer fixtures, CGB PPU/DMA framebuffer rows, and Mooneye CGB misc boot/DIV rows. |
...
Use `--failure-artifact-root` with the legacy `run_rom_suite` path whenever a failing row may need screenshots, memory text, snapshots, traces, or linked-session participant artifacts for diagnosis. Remaining Makefile ROM-suite targets go through the shared `RUN_ROM_SUITE` wrapper, default to `ROM_PROFILE=release-max`, and write legacy artifacts below `test/.artifacts/`; override locally with `ROM_PROFILE=release make <target>` only when iterating on compile time rather than final timing evidence. The new `cargo rom-suite` path always writes failure artifacts under the selected report's `.artifacts/<suite>/<case>/`.
...
- GitHub `test-roms` fans out promoted `gb-emulator-shootout` suites as matrix children; each child runs `cargo rom-fetch gb-emulator-shootout <family>` followed by `cargo rom-suite gb-emulator-shootout --suite <suite>`.
...
- GitHub `test-roms-gbmicrotest` runs `cargo rom-fetch gbmicrotest` followed by `cargo rom-suite gbmicrotest --suite gbmicrotest` as the standalone gbmicrotest report lane.
- DocBoy report lanes, RealBoot targets, private commercial manifests, and red/experimental local investigations stay outside GitHub ROM workflows unless promoted intentionally.
