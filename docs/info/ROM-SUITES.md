...
- `GB_CYCLE_TEST_ROM_ROOT` — store override for linked-session cases that reference repo-managed `test/...` inputs. The new `cargo rom-fetch` and `cargo rom-suite` report paths materialize under the repository `test/` directory and do not read this variable.
- `GB_CYCLE_TEST_ROM_STARTUP` — local startup override for `run_linked_session`; omit it to preserve manifest startup, use `skip-boot` or `custom-boot` for direct-start lanes, and use `real-boot` only with `GB_CYCLE_BOOT_ROM_ROOT`.
