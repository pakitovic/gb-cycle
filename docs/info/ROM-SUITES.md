...
- `crates/gb-test-runner/data/reports.toml` is the report registry for `cargo rom-fetch` and `cargo rom-suite`: it defines report IDs, store roots, inherited status/artifact/report file defaults, and optional report family order.
- Each report-local `sources.report.toml` is the source of truth for the new report fetch path: it groups pinned files by upstream source and family with explicit sparse checkout paths, `target_root`, per-file `target`, and SHA-256. Active report-local inventories are `gb-emulator-shootout/sources.report.toml`, `docboy/sources.report.toml`, `gbmicrotest/sources.report.toml`, `mooneye/sources.report.toml`, `ax6/sources.report.toml`, `little-things-gb/sources.report.toml`, `magen/sources.report.toml`, `mealybug-tearoom-tests/sources.report.toml`, and `samesuite/sources.report.toml`.
- The stable legacy fetch inventory remains in `crates/gb-test-runner/data/sources.toml` for the old `fetch_test_roms` path and currently-unpromoted linked assets such as `docboy-dmg-linked-extra`; do not add new report-local suites to the root legacy inventory unless the legacy runner still owns them.
- Active report ID `gb-emulator-shootout` uses upstream source ID `gbemu-shootout`; report IDs `docboy`, `gbmicrotest`, `little-things-gb`, `magen`, and `samesuite` use DocBoy rows where their `sources.report.toml` declares them; report IDs `mooneye`, `ax6`, and `mealybug-tearoom-tests` use GBEmulatorShootout rows. Fetching always uses temporary `git` checkouts of the pinned sources instead of user-supplied checkout roots.
...
`scripts/fetch.sh <report> <family> [family ...]` and `fetch_test_roms` remain the stable legacy fetch path. These direct fetches must name a report plus one or more explicit families; omitting the report, omitting families, passing the legacy `all` selector, or passing `null` is rejected. Use them only for legacy lanes that still depend on the old manifest model, such as linked assets, or when deliberately validating the old fetch path.

`cargo rom-fetch <report> [family ...]` is the local Cargo alias for `cargo run --release -q -p gb-test-runner --bin fetch -- <report> [family ...]`, the report fetch path for the report registry and source metadata contract. It reads `reports.toml`, then the selected report's `sources.report.toml`, selects all report families when no family is provided, validates any explicit family selection, sparse-checks out only the selected source-family roots, verifies every selected file hash, and copies each file to `test/<report-store>/<target_root>/<target>` while preserving the report's inherited `.status`, `.artifacts`, and Markdown report file. Reports may override the global `status_dir`, `artifact_dir`, or `report_file`; if a report omits `family_order`, `fetch` derives a deterministic alphabetical family order from the report-local `sources.report.toml`. The report registry intentionally excludes `legacy`; `cargo rom-suite` invokes this report fetch path automatically for any selected family with missing or stale materialized files.

The generated `/test/` store is gitignored. Each `cargo rom-suite` report owns `/test/<report-store>/.status/`, `/test/<report-store>/.artifacts/`, and `/test/<report-store>/test-report.md` if a Markdown report is later rendered for that channel. The promoted GB Emulator Shootout report stores families below `/test/gb-emulator-shootout/<family>/`; DocBoy stores its four single-machine target roots below `/test/docboy/{dmg,cgb,cgb-dmg,cgb-dmg-ext}/`; gbmicrotest materializes flat ROM paths directly below `/test/gbmicrotest/<rom>`; standalone exploratory reports store their single source family below `/test/mooneye/mooneye/`, `/test/ax6/ax6/`, `/test/little-things-gb/little-things-gb/`, `/test/magen/magen/`, `/test/mealybug-tearoom-tests/mealybug-tearoom-tests/`, or `/test/samesuite/samesuite/`. Upstream fixtures declared in `sources.report.toml` are materialized with the selected family; committed fixtures that are not shipped by the pinned upstream trees remain under `crates/gb-test-runner/data/**/fixtures/**` and are referenced from `*.suite.toml` with paths relative to each report-local `target_root`.

## Manifest rules

- Built-in promoted GB Emulator Shootout manifests live under `crates/gb-test-runner/data/gb-emulator-shootout/*.toml` with report-local `*.suite.toml` copies for the new runner; built-in DocBoy single-machine manifests live under `crates/gb-test-runner/data/docboy/*.toml` with report-local `*.suite.toml` copies for `cargo rom-suite docboy`; gbmicrotest lives under `crates/gb-test-runner/data/gbmicrotest/`; standalone exploratory reports live under `crates/gb-test-runner/data/{mooneye,ax6,little-things-gb,magen,mealybug-tearoom-tests,samesuite}/` with their own `sources.report.toml`; legacy root-manifest copies remain under `crates/gb-test-runner/data/*.toml` only for the old runner path while it still exists; local/private manifests passed with `--manifest` may live outside the repo.
...
- `cargo rom-suite` reads report-local `*.suite.toml` manifests; the promoted GB Emulator Shootout and DocBoy single-machine catalogs have `*.suite.toml` copies for every current report-local `*.toml` suite, with structured inline oracles and fixture paths relative to each source family `target_root`. Every `*.suite.toml` must declare `report = "<report-id>"`; the runner rejects manifests with a missing or mismatched report so a suite cannot be executed from the wrong report store.
...
- Use `disabled = true` only with a non-empty `comment = "..."`; disabled rows are for explicit overfit, duplicate, impossible, upstream-disabled, or CI-budget cases, not for quietly hiding a failing oracle. `cargo rom-suite` validates the comment and skips those rows, so migrated DocBoy disabled cases stay cataloged with their reason without becoming executable cases.
...
| `cargo run -p gb-test-runner --bin run_linked_session -- --suite linked-cgb-ir-smoke` | `linked-cgb-ir-smoke` | stdout/artifacts only | Internal synthetic CGB-to-CGB IR smoke; not part of GitHub ROM workflows. |
...
The new `cargo rom-suite` runner writes per-suite status TOML under `/test/<report-store>/.status/*.toml` and failure artifacts under `/test/<report-store>/.artifacts/<suite>/<case>/`; it does not render Markdown reports yet. Current status roots are `/test/gb-emulator-shootout/.status/`, `/test/docboy/.status/`, `/test/gbmicrotest/.status/`, `/test/mooneye/.status/`, `/test/ax6/.status/`, `/test/little-things-gb/.status/`, `/test/magen/.status/`, `/test/mealybug-tearoom-tests/.status/`, and `/test/samesuite/.status/`. Legacy report rendering still writes Markdown tables for old manifest-model lanes when that runner is used.
...
- `GB_CYCLE_TEST_ROM_ROOT` — legacy `fetch_test_roms` / `run_rom_suite` store override. The new `cargo rom-fetch` and `cargo rom-suite` report paths materialize under the repository `test/` directory and do not read this variable.
