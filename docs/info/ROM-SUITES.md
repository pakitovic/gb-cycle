...
- `crates/gb-test-runner/data/sources.toml` is the source of truth for upstream source IDs, pinned revisions, required file paths, SHA-256 hashes, source-family aliases, and materialized family names.
- Active upstream source IDs are `gbemu-shootout` for GBEmulatorShootout-hosted rows and `docboy` for DocBoy-hosted rows; use `GB_CYCLE_GBEMU_SHOOTOUT_ROOT` or `GB_CYCLE_DOCBOY_ROOT` only to point the fetcher at an existing local checkout instead of cloning a temporary one.
- Active materialized families are `acid`, `ashiepaws`, `ax6`, `blargg`, `cpp`, `daid`, `gbmicrotest`, `little-things-gb`, `magen`, `mealybug-tearoom-tests`, `mooneye`, `samesuite`, `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext`.
...
make fetch-test-roms FAMILIES="blargg acid samesuite"
```

`make fetch-test-roms` runs `fetch_test_roms`, verifies the pinned source files from `crates/gb-test-runner/data/sources.toml`, materializes the runnable `/test/` store, and removes temporary source checkouts. Every `make run-*` ROM target fetches its own required family before execution, so a direct target is self-contained.

The generated `/test/` store is gitignored and grouped by logical family, for example `/test/acid/`, `/test/blargg/`, `/test/docboy/dmg/`, `/test/docboy/cgb/`, `/test/docboy/cgb-dmg/`, `/test/docboy/cgb-dmg-ext/`, `/test/gbmicrotest/`, `/test/little-things-gb/`, `/test/magen/`, `/test/mealybug-tearoom-tests/`, `/test/mooneye/`, and `/test/samesuite/`. Long-lived committed fixtures and oracle assets stay under `crates/gb-test-runner/data/fixtures/**` instead of `/test/`.

## Manifest rules

- Built-in suite manifests live under `crates/gb-test-runner/data/*.toml`; local/private manifests passed with `--manifest` may live outside the repo.
...
- Prefer typed oracles (`serial-contains`, `mooneye-result`, `memory-byte-equals`, framebuffer fixtures, RGB555 framebuffer fixtures, trace fixtures, linked participant oracles) over manual visual inspection.
...
| --- | --- | --- | --- |
| `make test-roms` | Promoted local DMG/SGB aggregate | `/test/test-report.md` | Runs `acid`, Blargg chunks, `daid`, Mooneye chunks, `ashiepaws`, `cpp`, `cpp-sgb`, `mealybug`, and fixture-backed `samesuite-sgb`. |
| `make test-roms-real-boot` | Local RealBoot rerun for the promoted DMG subset | `/test/test-report.md` | Requires `GB_CYCLE_BOOT_ROM_ROOT`; excludes SGB targets until SGB/SGB2 RealBoot policy exists. |
...
| `make test-roms-cgb` | Promoted local CGB aggregate | `/test/test-report.md` | Runs `cgb-smoke`, `cgb-boot-div`, `cgb-speed`, `cgb-ppu-basic`, `cgb-ppu-hard`, `cgb-dma`, `cgb-audio-blargg`, `cgb-audio-samesuite`, and `cgb-rtc`. |
| `make test-roms-cgb-real-boot` | Local RealBoot rerun for the promoted CGB aggregate | `/test/test-report.md` | Requires `GB_CYCLE_BOOT_ROM_ROOT`; uses revision-derived private boot ROM filenames such as `cgb_boot.bin` or `cgbE_boot.bin`. |
...
Promoted DMG/SGB and promoted CGB rows share `/test/test-report.md`, so rerun the aggregate that matches the evidence you want before quoting a report count. Non-DocBoy extra/internal rows share `/test/test-report-extra.md`. Large DocBoy single-machine rows use `/test/test-report-docboy.md`. Linked-session rows such as `docboy-dmg-linked-extra` and `linked-cgb-ir-smoke` print participant-scoped status to stdout and retain failure artifacts, but they do not currently append Markdown report rows.
...
| `make run-blargg`, `make run-blargg-cpu-instrs`, `make run-blargg-dmg-sound`, `make run-blargg-timing-memory-oam` | `blargg-dmg-curated` chunks | `blargg` | CPU, timing, memory, OAM, and DMG sound chunks; chunk targets mirror CI matrix lanes. |
| `make run-daid` | `daid-dmg-curated` | `daid` | DMG framebuffer and compatibility smoke rows. |
| `make run-mooneye`, `make run-mooneye-acceptance`, `make run-mooneye-mbc1-mbc5`, `make run-mooneye-mbc2` | `mooneye-acceptance-dmg-curated` chunks | `mooneye` | DMG acceptance/manual plus MBC1/MBC5 and MBC2 chunks. |
...
cargo run -p gb-test-runner --bin run_rom_suite -- --suite cgb-dma --failure-artifact-root .artifacts/cgb-dma
cargo run -p gb-test-runner --bin run_rom_suite -- --suite mooneye-cgb-extra --case mooneye-cgb-ppu-intr-2-mode0-timing-sprites
cargo run -p gb-test-runner --bin run_linked_session -- --suite docboy-dmg-linked-extra --failure-artifact-root .artifacts/docboy-dmg-linked
cargo run -p gb-test-runner --bin run_rom_suite -- --manifest .artifacts/local-private-smoke.toml
```

Use `--failure-artifact-root` whenever a failing row may need screenshots, memory text, snapshots, traces, or linked-session participant artifacts for diagnosis. Heavy extra/internal targets default to `ROM_PROFILE=release-max`; override locally with `ROM_PROFILE=release make <target>` only when iterating on compile time rather than final timing evidence.
...
Keep commercial ROMs and private firmware outside the repo, outside `/test/`, and outside CI. For local-only smoke or audio/menu investigation, use a private manifest with `external_rom_root_key` and a developer-owned environment variable; do not standardize private paths in docs or commit generated private artifacts.
...
- `GB_CYCLE_TEST_ROM_ROOT` — optional override for the materialized ROM store; if unset, `gb-test-runner` uses the default `/test/` store.
...
- `GB_CYCLE_GBEMU_SHOOTOUT_ROOT` — optional existing GBEmulatorShootout checkout for `fetch_test_roms`.
- `GB_CYCLE_DOCBOY_ROOT` — optional existing DocBoy checkout for `fetch_test_roms`.
- `GB_CYCLE_PRIVATE_ROM_ROOT` — example private manifest root key; local manifests may choose a different developer-owned variable, but repo docs and CI must not depend on it.
