...
- The old single-machine manifest inventories have been retired; new external single-machine lanes must use `*.suite.toml` plus a report-local `sources.report.toml`. The only legacy linked-session executable manifest retained under `crates/gb-test-runner/data/` is `docboy-dmg-linked.toml` for `run_linked_session`; internal linked-session manifests now live only as local report drafts under `crates/gb-test-runner/data/linked/*.link.suite.toml` with `[[case]]`, `[[case.participant]]`, and structured linked oracles. The root `crates/gb-test-runner/data/sources.toml` is retained only as a consultation/provenance list for the DocBoy DMG linked ROMs used by `docboy-dmg-linked.toml`; it is not a fetch entry point and must not be expanded for new single-machine lanes.
...
- Built-in single-machine external ROM suites live only as report-local `*.suite.toml` manifests under `crates/gb-test-runner/data/<report>/`, with materialization metadata in that report's `sources.report.toml`; the repo must not add legacy single-machine root manifests or report-local non-`.suite.toml` copies. Linked-session legacy TOMLs are the only remaining executable non-suite external manifests; their new-model internal copies use `*.link.suite.toml` under local report `crates/gb-test-runner/data/linked/`, use `[[case]]` plus `[[case.participant]]`, and keep ROM/fixture paths relative to that linked report data directory.
