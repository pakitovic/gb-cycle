...
- `crates/gb-test-runner/data/gb-emulator-shootout/sources.toml` owns pinned external ROM and upstream oracle-fixture sources and hashes for the promoted GB Emulator Shootout report; `crates/gb-test-runner/data/docboy/sources.toml` owns pinned DocBoy single-machine source hashes; `crates/gb-test-runner/data/sources.toml` owns only the remaining legacy extra/internal source inventory. Update those files rather than documenting ad hoc source lists elsewhere.
- `/test/gb-emulator-shootout/test-report.md`, `/test/docboy/test-report.md`, and `/test/test-report-extra.md` are generated/local evidence channels; keep before/after copies when the mandatory external-ROM regression workflow applies.
