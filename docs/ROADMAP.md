...
- Phase `7` follow-up: CH1 / CH2 / CH3 / CH4 fast timers are still paused whenever `channel_active` is false. The docs already require fast per-channel timers to remain conceptually separate from frame-sequencer clocks, but the current inactive-channel pause policy has not been validated against a dedicated oracle or hardware evidence. Revisit that gating before treating pulse duty phase, CH3 sample position, or CH4 noise phase continuity as settled DMG behavior.
