...
### Current PPU checkpoint

- The broad PPU refactor is structurally landed: explicit visible and pipeline register snapshots, explicit `Mode 3` transfer/readiness/execution state, explicit push and fill ownership, startup-alignment seam, cached-slice ownership across `Push -> fill -> FIFO`, and typed cached-slice origins for the second and third visible post-startup BG tiles.
- The work stopped before closing the main oracle debt. The still-open families are the `Mode 3` live-write cases, especially `m3_lcdc_bg_map_change`, `m3_lcdc_tile_sel_change`, `m3_lcdc_tile_sel_win_change`, and `m3_scy_change`. Mooneye closure also remains incomplete in the LCD restart lane and in one sprite-coupled `STAT` timing case.
- The last stable exploratory cut for the left edge kept repo-local PPU tests, `acid/dmg-acid2.gb`, the materialized external suite, and `make ci` green while measuring roughly `m3_lcdc_bg_map_change: 674`, `m3_lcdc_tile_sel_change: 1410`, `m3_lcdc_tile_sel_win_change: 1232`, and `m3_scy_change: 7819`.
- The most useful positive result from the March 29 spike was not another startup rewrite; it was narrowing the remaining debt to cached background slices already waiting in `Push` or `fill.pending` after startup. Broad startup retuning, broad tilemap rereads, and broad cached-slice `LCDC.3` retargeting all regressed the oracle and should not be the first next step.
- If PPU work resumes, the highest-value next step is cached-slice live-write timing for the second and especially the third visible BG tile after startup (`x = 16..23`), using the current cached-slice origin and same-T-cycle window machinery instead of reopening the older generic startup or fetcher-shadow-state experiments.
...
- Temporary repo-owned coverage-gate relaxation: as of April 1, 2026, the gate now runs one shared `cargo llvm-cov --workspace --no-report` sweep, then enforces per-crate floors of `gb-core` `90/90/90`, `gb-test-runner` `84.22/84.63/78.87`, `gb-persistence` `84.84/79.37/84.44`, `gb-cli` `64.47/61.23/59.48`, and `gb-desktop` `45.24/48.05/48.76` for lines/regions/functions. That keeps `make ci` on one workspace test pass while the coverage thresholds remain crate-specific. Follow-up work should raise the temporary floors back toward the intended `90%` baseline and remove the relaxation once that coverage lands.
...
- [PPU][PHASE4-CHECKPOINT] Phase `4` PPU is no longer blocked on baseline architecture. The landed state already includes the routed video-bus contract, explicit `Mode 3` transfer/readiness/execution state, explicit push/fill ownership, startup-alignment state, cached-slice ownership across `Push -> fill -> FIFO`, delayed pipeline-register snapshots, and same-cycle CPU->PPU MMIO commit staging. If this lane is resumed, treat the remaining work as oracle closure, not as another general PPU refactor. The main remaining targets are `Mode 3` live-write families (`m3_lcdc_bg_map_change`, `m3_lcdc_tile_sel_change*`, `m3_lcdc_win_map_change`, `m3_scy_change`, some low-`X` sprite/live-OBJ cases, and window/live-`LCDC.5` cases) plus the remaining mooneye LCD restart / sprite-coupled `STAT` timing rows. The detailed bullets below are retained as implementation history; the actionable pause point is the cached-slice live-write timing of the second and especially third visible BG tile after startup.
