...
- The pinned upstream source inventory and per-file SHA-256 hashes are recorded in `crates/gb-test-runner/data/sources.toml`; most rows come from `GBEmulatorShootout`, while source-specific exceptions such as DocBoy SameSuite, little-things-gb, gbmicrotest, docboy, and docboy-cgb-dmg-ext rows declare their materialized family/ROM alias explicitly.
