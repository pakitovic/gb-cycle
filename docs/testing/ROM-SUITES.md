...
- `linked-cgb-ir-smoke` is an internal linked-session CGB IR smoke suite backed by repo-owned synthetic fixtures in `crates/gb-test-runner/data/fixtures/linked/cgb-ir/`; it uses topology `cgb-ir`, two native-CGB participants, and a participant-scoped serial oracle (`receiver` emits `$B2`) to validate only CGB-to-CGB `RP` emitter/sensor routing after the deterministic warmup threshold.
