...
The extra/internal CGB companion suite `mealybug-tearoom-cgb-extra` runs the same 24 ROM programs on `console = "cgb"` with `report_model_suffix = true`, `timeout_tcycles = 5000000`, and until-match framebuffer fixtures under `crates/gb-test-runner/data/fixtures/mealybug-cgb/`; it owns the byte-identical Mealybug coverage that used to appear under `docboy-cgb-dmg`, including `m3_lcdc_win_en_change_multiple_wx.gb` using the user-provided fixture `m3_lcdc_win_en_change_multiple_wx.png`, reports as `mealybug-tearoom-tests | ppu/*.gb (GBC)` in `/test/test-report-extra.md`, and had initial 2026-05-21 local evidence of `1/24` passing so it remains a red bring-up signal rather than promoted CGB closure evidence. The `m3_bgp_change_sprites.gb` row pins `startup = "custom-boot"` because the fixture exercises the boot-logo tile bytes through the sprite path while still avoiding a full private CGB boot ROM dependency.
