...
- The pinned upstream source inventory and per-file SHA-256 hashes are recorded in `crates/gb-test-runner/data/sources.toml`; most rows come from `GBEmulatorShootout`, while source-specific exceptions such as DocBoy SameSuite, little-things-gb, gbmicrotest, docboy, docboy-cgb-dmg, and docboy-cgb-dmg-ext rows declare their materialized family/ROM alias explicitly.
...
/.roms/test/daid/
/.roms/test/docboy/
/.roms/test/docboy-cgb-dmg/
/.roms/test/docboy-cgb-dmg-ext/
...
- `docboy-cgb-dmg-extra` is the extra/internal DocBoy CGB GB-compatible suite, not a promoted CGB closure lane; its suite definition is `crates/gb-test-runner/data/docboy-cgb-dmg.toml`, its membership source of truth is DocBoy `tests/config/cgb_dmg_mode.json` at `214905562590c35ba2bc41f36da3a5d636d99378`, it materializes the `504` unique rows from `505` JSON entries in `tests/roms/cgb_dmg_mode` into the separate `docboy-cgb-dmg` family, skips the one upstream-disabled Mealybug row, forces `console = "cgb"` with `execution_mode = "experimental"` so direct boot and RealBoot select CGB-family GB-compatible mode including DocBoy KEY0 bit-2 cases, checks `$FFF0 == $01` plus fail-fast `$FFF0 == $02` for memory rows, and uses committed DocBoy CGB DMG-mode PNG fixtures for framebuffer until-match rows. Initial local evidence on 2026-05-20 is `71/503` passing, so the target is expected to be a red extra/internal bring-up lane until compatibility-mode gaps are closed.
- `run-docboy-cgb-dmg` materializes its upstream family with `make fetch-test-roms FAMILIES=docboy-cgb-dmg` before invoking `cargo run --profile $(ROM_PROFILE) -q -p gb-test-runner --bin run_rom_suite -- --suite docboy-cgb-dmg-extra --failure-artifact-root .artifacts/docboy-cgb-dmg`; it is included in `make test-roms-cgb-extra` and the local RealBoot extra aggregate, but intentionally stays outside `make test-roms-cgb` and GitHub `test-roms` until promoted intentionally.
- `docboy-cgb-dmg-ext-extra` is the extra/internal DocBoy CGB DMG-ext suite, not a promoted CGB closure lane; its suite definition is `crates/gb-test-runner/data/docboy-cgb-dmg-ext.toml`, its membership source of truth is DocBoy `tests/config/cgb_dmg_ext_mode.json` at `214905562590c35ba2bc41f36da3a5d636d99378`, it materializes exactly 26 `docboy/...` rows from `tests/roms/cgb_dmg_ext_mode/docboy` into the separate `docboy-cgb-dmg-ext` family, forces `console = "cgb"` with `execution_mode = "experimental"`, checks `$FFF0 == $01` plus fail-fast `$FFF0 == $02`, and validates only the narrow experimental CGB DMG-ext register profile rather than full PGB/PSM behavior.
- `run-docboy-cgb-dmg-ext` materializes its upstream family with `make fetch-test-roms FAMILIES=docboy-cgb-dmg-ext` before invoking `cargo run --profile $(ROM_PROFILE) -q -p gb-test-runner --bin run_rom_suite -- --suite docboy-cgb-dmg-ext-extra --failure-artifact-root .artifacts/docboy-cgb-dmg-ext`; it is included in `make test-roms-cgb-extra` and the local RealBoot extra aggregate, but intentionally stays outside `make test-roms-cgb` and GitHub `test-roms` until promoted intentionally.
