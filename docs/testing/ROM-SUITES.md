...
- `gbmicrotest-dmg-extra` keeps ordinary rows startup-neutral so `make run-gbmicrotest` and `make test-roms-extra` use the default SkipBoot path, while `make test-roms-extra-real-boot` reruns the same manifest with `GB_CYCLE_TEST_ROM_STARTUP=real-boot`. Only the reset-facing `boot/poweron_*`, allowlisted SCX HBlank IF/nops seam, and `ppu/line_65_ly.gb` rows declare `startup = "custom-boot"`, using the core DMG-family CustomBoot PPU publication table or first-frame hidden Mode `0` IRQ phase instead of a runner-only PPU-profile workaround, while ordinary rows keep the same plain SkipBoot contract as gb-cli and gb-desktop.
