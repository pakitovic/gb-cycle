...
- `samesuite-sgb` is the promoted-report informational SameSuite SGB suite for Phase 11 SGB command/multiplayer bring-up; its suite definition is `crates/gb-test-runner/data/samesuite-sgb.toml`, it materializes `sgb/command_mlt_req.gb` and `sgb/command_mlt_req_1_incrementing.gb`, forces `console = "sgb"` so the runner uses the shared DMG core with `HostPlatform::Sgb`, captures framebuffers as `info-framebuffer`, emits SGB host RGB555 artifacts when available, and writes `ℹ️` rows to `/test/test-report.md` after `samesuite | ppu/blocking_bgpi_increase.gb | ...` by source order. After Slice 5, these ROMs exercise the implemented `MLT_REQ` controller-multiplexing path while remaining informational report rows until a non-framebuffer pass/fail oracle is promoted; use `make run-samesuite-sgb` to materialize `FAMILIES=samesuite` before invoking `run_rom_suite --suite samesuite-sgb`.
