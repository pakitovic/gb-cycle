...
- `cgb-ppu-basic` is the Phase `10` Slice `4` CGB PPU baseline promotion suite, not a repo-gated DMG closure lane; its ROM inventory is declared in `crates/gb-test-runner/data/sources.toml`, its suite definition is `crates/gb-test-runner/data/cgb-ppu-basic.toml`, and `make run-cgb-ppu-basic` fetches `samesuite daid` before invoking `run_rom_suite`.
- `cgb-ppu-basic` currently starts with two blocking rows in roadmap order: SameSuite `ppu/blocking_bgpi_increase.gb`, using a rank-normalized framebuffer fixture at `crates/gb-test-runner/data/fixtures/samesuite/ppu/blocking_bgpi_increase.png`, and Daid `ppu_scanline_bgp.gb (GBC)`, using the `framebuffer-rgb555-fixture` oracle against `crates/gb-test-runner/data/fixtures/daid/ppu_scanline_bgp.gbc.png`; later Slice `4` promotions should append `cgb-acid2.gbc` and Hacktix `bully.gb (GBC)` in the roadmap order instead of declaring strict Slice `4` closure from these first probes alone.
