...
make test-roms-extra   # fetch if needed + run the exploratory/internal extra DMG/SGB suites
...
make run-samesuite-sgb # fixture-backed SameSuite SGB command/multiplayer suite
make run-mooneye-sgb-boot-regs # extra Mooneye SGB/SGB2 boot-register suite
...
Each `make run-*` target is autosufficient and materializes its own curated family before execution. Heavy extra/internal or non-default-host targets (`run-ax6`, `run-samesuite`, `run-samesuite-cgb`, `run-samesuite-sgb`, `run-mooneye-sgb-boot-regs`, `run-magen-cgb`, `run-little-things-gb`, `run-little-things-gb-cgb`, `run-gbmicrotest`, `run-docboy-dmg`, `run-docboy-cgb`, `run-cgb-boot-hwio`, `run-mooneye-cgb`, `run-mealybug-cgb`, `run-docboy-cgb-dmg`, and `run-docboy-cgb-dmg-ext`) invoke the runner through `cargo run --profile $(ROM_PROFILE)` with default `ROM_PROFILE=release-max`; override locally with `ROM_PROFILE=release make <target>` when iterating on compile time rather than final timing evidence.
...
# Run the SameSuite SGB fixture-backed packet/multiplayer suite
...
- `samesuite-sgb` is the promoted-report fixture-backed SameSuite SGB suite for Phase 11 SGB command/multiplayer bring-up; its suite definition is `crates/gb-test-runner/data/samesuite-sgb.toml`, it materializes `sgb/command_mlt_req.gb` and `sgb/command_mlt_req_1_incrementing.gb`, forces `console = "sgb"` so the runner uses the shared DMG core with `HostPlatform::Sgb`, compares the raw DMG LCD framebuffer with `framebuffer-fixture` PNG fixtures under `crates/gb-test-runner/data/fixtures/samesuite/sgb/`, gives `command_mlt_req.gb` a `300`-frame budget because it sends the longer 43-packet MLT_REQ matrix before enabling LCD, emits retained framebuffer/snapshot artifacts on failure, and writes `✅` or `❌` rows to `/test/test-report.md` after `samesuite | ppu/blocking_bgpi_increase.gb | ...` by source order. The two PNG fixtures were copied from CasualPokePlayer/GBEmulatorShootout commit `0eab01ca7dc69d02c9892d7175eea62a73728b71` because the upstream gbdev/GBEmulatorShootout PR `91` is not merged yet; when the project refreshes its GBEmulatorShootout source fork, these should be switched back to the normal upstream-origin fixture materialization path. After Slice 5, these ROMs exercise the implemented `MLT_REQ` controller-multiplexing path through a visible pass/fail screen oracle; use `make run-samesuite-sgb` to materialize `FAMILIES=samesuite` before invoking `run_rom_suite --suite samesuite-sgb`.
- `mooneye-sgb-boot-regs-extra` is the extra/internal Mooneye SGB/SGB2 direct-start suite; its suite definition is `crates/gb-test-runner/data/mooneye-sgb-boot-regs.toml`, it materializes GBEmulatorShootout Mooneye `acceptance/boot_regs-sgb.gb` and `acceptance/boot_regs-sgb2.gb`, forces `console = "sgb"` and `console = "sgb2"`, uses the `mooneye-result` oracle, keeps default `SkipBoot`, and writes rows such as `mooneye | acceptance/boot_regs-sgb.gb | ✅` to `/test/test-report-extra.md`. These rows pin the public external oracle for original SGB `A=$01, C=$14` and SGB2 `A=$FF, C=$14` direct-start fingerprints without claiming SGB/SGB2 `RealBoot` host-shell closure.
- `run-mooneye-sgb-boot-regs` materializes the Mooneye family with `make fetch-test-roms FAMILIES=mooneye` before invoking `cargo run --profile $(ROM_PROFILE) -q -p gb-test-runner --bin run_rom_suite -- --suite mooneye-sgb-boot-regs-extra --failure-artifact-root .artifacts/mooneye-sgb-boot-regs`; the ROM hashes live in `crates/gb-test-runner/data/sources.toml`, while the suite contract lives in `crates/gb-test-runner/data/mooneye-sgb-boot-regs.toml`.
- `little-things-gb-dmg-extra` is the extra/internal DocBoy little-things-gb DMG suite, not a promoted DMG closure lane; its suite definition is `crates/gb-test-runner/data/little-things-gb.toml`, it materializes DocBoy `double-halt-cancel.gb` and `whichboot.gb`, forces `console = "dmg"`, keeps both report rows unsuffixed, compares against committed fixtures under `crates/gb-test-runner/data/fixtures/little-things-gb/`, marks `whichboot.gb` with `startup = "skip-boot"` so its boot-logo/map oracle sees the ordinary fast DMG boot-logo VRAM seed without enabling the reset-facing `CustomBoot` PPU publication phase, and writes rows such as `little-things-gb | whichboot.gb | ✅` to `/test/test-report-extra.md`.
...
- `gbmicrotest-dmg-extra` keeps ordinary rows startup-neutral so `make run-gbmicrotest` and `make test-roms-extra` use the default logo-seeded `SkipBoot` path, while `make test-roms-extra-real-boot` reruns the same manifest with `GB_CYCLE_TEST_ROM_STARTUP=real-boot`. The reset-facing `boot/poweron_*` rows plus the explicitly allowlisted PPU timing probes declare `startup = "custom-boot"` so they get the DMG boot-logo VRAM/map seed and reset-facing PPU CPU-bus publication instead of a runner-only PPU-profile workaround, while ordinary rows keep the same logo-seeded direct-start contract as gb-cli, gb-desktop, GBEmulatorShootout, and Mooneye `boot_hwio-dmgABCmgb`.
...
- `make test-roms` fetches the curated ROM store if needed and runs all local curated DMG/SGB suites currently wired in `Makefile`: `acid`, the full Blargg lane via the three `run-blargg-*` chunks, `daid`, `ashiepaws`, `cpp`, `cpp-sgb`, `mealybug-tearoom-tests`, the full Mooneye lane via the three `run-mooneye-*` chunks, and fixture-backed `samesuite-sgb`.
- GitHub uses three core validation workflows: `ci` for Rust checks plus coverage, `test-roms` for the promoted workflow-managed ROM subset (`acid`, `blargg-cpu-instrs`, `blargg-dmg-sound`, `blargg-timing-memory-oam`, `daid`, `ashiepaws`, `cpp`, `cpp-sgb`, `samesuite-sgb`, `mooneye-acceptance`, `mooneye-mbc1-mbc5`, `mooneye-mbc2`, `mealybug-tearoom-tests`, `cgb-smoke`, `cgb-boot-div`, `cgb-speed`, `cgb-ppu-basic`, `cgb-ppu-hard`, `cgb-dma`, `cgb-audio-blargg`, `cgb-audio-samesuite`, and `cgb-rtc`), and `test-roms-extra` for the green non-DocBoy extra/internal subset (`ax6`, `samesuite`, `little-things-gb`, `gbmicrotest`, `mooneye-sgb-boot-regs`, `cgb-boot-hwio`, `mooneye-cgb`, `samesuite-cgb`, `magen-cgb`, `mealybug-cgb`, and `little-things-gb-cgb`).
...
- `GB_CYCLE_TEST_ROM_STARTUP` — ROM-suite startup override for local ignored external harness runs and the `run_rom_suite` / `run_linked_session` CLIs; omit it to preserve each suite manifest's declared startup mode, use `skip-boot` to force direct boot with the console-family boot-logo VRAM seed, use `custom-boot` to force that same direct boot plus custom boot-facing phases, and use `real-boot` only with `GB_CYCLE_BOOT_ROM_ROOT` for clean boot-ROM execution that clears direct-start startup-memory and startup timer state.
