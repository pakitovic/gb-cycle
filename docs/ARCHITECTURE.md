...
- Rewind is layered over repeated in-memory `MachineSaveState` capture/restore. Phase 8.4 defines the core-only frame/subframe `MachineRewindBuffer` ring buffer and memory telemetry; `gb-desktop` Phase 8.6 owns the single-machine host integration by recording frame/subframe snapshots during normal runtime, exposing a root `REWIND` action plus a remappable hold hotkey (`Left Shift` by default), and resetting host input/audio/pacing/RTC/save baselines after restore. Multi-machine coordination, compression, deltas, and debugger-grade reverse T-cycle stepping remain outside the core contract.
