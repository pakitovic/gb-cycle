...
- Additive fields inside an existing subsystem save-state DTO must either carry explicit deserialization defaults with compatibility tests or deliberately bump/migrate the envelope format; they must not silently make same-version `.gbstate` payloads unreadable.
