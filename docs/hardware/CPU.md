...
- In native CGB mode, a `STOP` whose `KEY1` prepare bit is armed takes the speed-switch path instead of the DMG joypad-wake entry matrix: it fetches and discards the padding byte, requests the same shared divider reset, asks the speed controller to clear the prepare bit and toggle speed, and enters an explicit `SpeedSwitchPause` state for `8200` T-cycles.
- The CGB speed-switch path resumes from the post-padding `PC` after the pause expires. During the pause, the CPU performs no bus access and the scheduler treats the CPU as stop-active for timer, serial, DMA, and APU advancement, plus PPU-visible stop-state publication.
- CGB speed-switch eligibility is supplied by the machine-owned `SpeedController`; Non-CGB models and CGB-family `GbCompatible` mode leave `KEY1` unavailable, so their `STOP` behavior continues to follow the DMG-family matrix above.
...
- tests for the native CGB `KEY1`-armed `STOP` path: prepare-bit readback and clear, padding-byte fetch, divider reset, speed toggle, `8200` T-cycle pause, no bus traffic during the pause, and no regression to the DMG-family `STOP` wake matrix
...
The synthetic DMG boot ROM from Phase `2.4` remains useful as a narrow, deterministic `gb-core` integration target, but it is no longer the sole boot closure criterion. Production DMG-family closure in this repo depends on explicit local private-firmware RealBoot validation rather than legacy ignored `gb-test-runner` regressions, and any remaining differential-oracle or replay work now belongs to Phase `9` hardening rather than to a functional CPU/boot gap.
