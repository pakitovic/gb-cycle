...
Keep the hardware-visible register behavior separate from host input collection. Model the joypad as a matrix peripheral rather than as "a byte the frontend writes into `FF00`". Keep these layers distinct:
...
- For retained timing visibility in this repo baseline, scheduler traces should make the joypad-owned selection bits, hardware-facing pressed-mask state, visible low nibble, interrupt-request latch, and `STOP`-wake latch observable at the points where interrupt aggregation and CPU wake evaluation consume them.
...
- In the current baseline, let both `FF00` selection writes and hardware-facing button transitions feed that same visible-edge detector, while the resulting joypad request remains latched locally until the scheduler's interrupt aggregation step publishes it into `IF`.
...
- Keep the `STOP` wake latch separate from the joypad-interrupt latch even when one hardware-facing button press produces both on the same shared T-cycle; in the current baseline, the CPU consumes wake during scheduler phase `9` and only later reaches any interrupt-acceptance point through normal instruction flow.
