...
- For the current DMG / MGB `SkipBoot` baseline, the synthesized timer state should seed the internal system counter to `0xABC8`, not merely `0xAB00`, so the first post-boot `DIV` edges match Mooneye's DMG-family `boot_div` timing. The matching later-DMG / MGB `RealBoot` path seeds the timer with a `0x0024` power-on reset offset so executing the real boot ROM reaches the same `0xABC8` handoff phase instead of starting the divider from zero. For CGB direct start, the synthesized timer state is selected from the cartridge header: missing or DMG-compatible headers seed `0x2674`, not merely `0x2600`, so Mooneye `misc/boot_div-cgbABCDE.gb` remains the ABCDE baseline; the validated native CGB non-Nintendo old-licensee bucket seeds `0x1E84` to match gb-cycle's standard `cgb_boot.bin` handoff observation for Hacktix `bully.gb`; and the native CGB old-licensee `$33` plus binary-zero new-licensee bucket seeds `0x1E98` to match Nitro2k01 `whichboot.gb` without manifest-level startup timer overrides.
