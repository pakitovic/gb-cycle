...
- `ConsoleModel::GameBoyColor` and `ConsoleModel::GameBoyAdvance` publish that the non-blocked readback is revision-dependent; `CpuCgbE`, `CpuAgb0`, and `CpuAgbA` currently return an address-derived extra-OAM pattern `(low_address & 0xF0) | (low_address >> 4)`, so `FEA0` and `FEA1` read as `0xAA`, `FEB8` reads as `0xBB`, and `FEFF` reads as `0xFF`.
