...
- In that same DMG-first baseline, keep zero-context storage helpers out of the public `Bus` runtime facade; if a targeted harness needs partial access for fixture setup or storage inspection, that limitation must stay explicit in the API shape rather than being named like an ordinary bus transaction.
