...
- A `STOP` executed while `KEY1` is armed follows the CGB speed-switch path: fetch the padding byte, reset the shared divider through the speed-aware `DIV` write-effect path, clear the prepare bit, toggle normal/double speed, and enter the explicit `8200` T-cycle speed-switch pause before resuming from the post-padding `PC`.
- During the speed-switch pause, the scheduler treats the CPU as stop-active for the same frozen hardware domains as `STOP`; CPU bus traffic is absent, timer/serial/APU advancement is gated, and the PPU sees the stop-active contract that later CGB PPU work must preserve.
...
- LCD/PPU timing must continue to use its own scheduler-domain contract; double speed is not a generic frame, LY, STAT, or LCD-dot multiplier.
- CGB-family `STOP` forced blank fills the visible framebuffer with panel shade `3` (black), while DMG-family `STOP` keeps shade `0` (white); this is a small STOP/visible-output bridge for current monochrome framebuffer data, not the full Slice 4 CGB palette renderer.
- The Phase 10 `cgb-speed` ROM suite is manifest-backed; Daid `stop_instr.gb (GBC)` is a blocking absolute grayscale framebuffer fixture, `speed_switch_timing_div.gbc` is a blocking rank-normalized framebuffer fixture, and the remaining Daid and Blargg rows stay informational until promoted to strict trace, framebuffer, serial, or ROM-declared output oracles.
