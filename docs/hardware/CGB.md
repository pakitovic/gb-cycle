...
- Post-Slice 10 CGB IR v1 models only native CGB-to-CGB optical input: the sensor sees the peer emitter OR its own emitter, advances once per scheduler T-cycle with provisional warmup/threshold/fade constants cross-checked against SameBoy and GBE+, and the linked topology samples emitters during `ExternalEventIngress` so a CPU write to `RP` cannot be observed by the peer in the same CPU cycle.
