...
Own the Color Game Boy silicon-extension contract inside `gb-core`: CGB model axes, AGB GB/C compatibility mode as a CGB-family profile, boot-selected operating mode, CGB-only MMIO visibility, double-speed domains, VRAM/WRAM banking, RGB555 palette and framebuffer surfaces, GDMA/HDMA integration boundaries, CGB serial speed, model-gated infrared register ownership, CGB-family APU taps, CGB compatibility mode, and the narrow experimental CGB DMG-ext profile.
...
CGB is an extension of the shared Game Boy core, not a second emulator. `ConsoleModel::GameBoyAdvance` uses that CGB-family GB/C path with an AGB revision and boot fingerprint rather than invoking a native GBA core. Shared subsystems must keep DMG-family behavior, CGB-family silicon behavior, and active software-visible operating mode separate enough that CGB native mode, CGB compatibility mode, experimental CGB DMG-ext mode, and future host shells can reuse the same scheduler and bus architecture.
...
- `ConsoleModel::GameBoyColor` is the CGB-family handheld entry point with a physical CGB infrared port. The active revision set is `CpuCgb0`, `CpuCgbC`, `CpuCgbD`, and `CpuCgbE`, with `CpuCgbD` as the default and revision-derived firmware names owned by [`BOOT-ROM.md`](BOOT-ROM.md).
- `ConsoleModel::GameBoyAdvance` is the active AGB GB/C compatibility entry point. It is CGB-family for GB/C CPU/bus/PPU/APU behavior, supports explicit `HardwareRevision::CpuAgb0` and default `HardwareRevision::CpuAgbA`, selects `cgb_agb0_boot.bin` or `cgb_agb_boot.bin` for `RealBoot`, exposes the GBA-enhanced post-boot register fingerprint, and does not have a physical CGB infrared port.
...
- CGB and AGB CGB-compatible `RealBoot` map firmware through the non-contiguous CGB windows `0000-00FF` and `0200-08FF`, while `0100-01FF` remains cartridge/header-visible until boot unmaps.
- CGB and AGB CGB-compatible `RealBoot` do not preselect native or compatibility mode before firmware executes. The boot ROM writes `KEY0`; the `FF50` handoff locks the boot-written value and updates machine, bus, speed, and PPU operating-mode state together.
...
- Direct-start CGB-family mode selection uses the canonical parsed cartridge header and heuristic policy, not ad hoc reads of raw `0x0143` in unrelated subsystems. AGB direct start additionally sets `B.bit0` for GBA-enhanced software detection while keeping `C=0`.
...
- CGB infrared `RP` state is bus-owned and model-gated to `ConsoleModel::GameBoyColor`; AGB remains CGB-family for GB/C execution but has no physical IR port, so `RP` stays unavailable there. Two-console optical routing and single-accessory sessions are link-owned. The detailed topology and accessory contracts live in [`LINK.md`](LINK.md) and [`../info/INFRARED.md`](../info/INFRARED.md).
...
- Save states must include every CGB-family hidden state that affects continuation: model axes, locked `KEY0`, speed controller, bank selectors, banked WRAM/VRAM contents, palette RAM and index latches, OPRI latch/boot-selected priority mode, DMA/GDMA/HDMA state, CGB misc registers, model-gated infrared sensor state where physically present, serial high-speed state, and subsystem-specific CGB hidden phases.
...
- Use focused Rust tests for register routing, mode gates, readbacks, save-state continuation, speed-domain scheduling, bank selection, palette ports, DMA state, APU taps, serial speed, AGB detection fingerprints, and infrared sensor state before relying on broad visual ROMs.
...
- Additional AGS/SP and Game Boy Player UI profiles remain deferred. The current active AGB support is intentionally one visible `GameBoyAdvance` GB/C compatibility model with `CpuAgb0` and `CpuAgbA` revision profiles; `CpuAgb0` is active for explicit boot-ROM validation but currently shares the `CpuAgbA` direct-start behavior because no promoted evidence distinguishes a separate synthetic post-handoff state.
...
- Do not let frontend palette, audio, timing, AGB model labels, or IR UX mutate core hardware state implicitly.
