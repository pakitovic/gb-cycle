...
- `GbCompatible` and `CgbDmgExt` PPU Mode `3` live-write seams should be modeled as CGB-family DMG-software behavior: they ignore native CGB BG attributes but may still need CGB-specific startup phase tables, as with `LCDC.0`, `LCDC.3`, and `LCDC.4`, and must stay separate from native-only CGB glitches such as same-cycle tile-number substitution.
