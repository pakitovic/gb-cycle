...
`ConsoleModel` is the visible product model, not the CPU revision string and not the boot ROM filename. The CGB product row is `ConsoleModel::GameBoyColor`; it defaults to `BootRomKind::Cgb`, allows `Cgb0`, `Cgb`, and `CgbE` for `RealBoot`, and uses a centralized CGB `SkipBoot` startup profile aligned to standard `cgb_boot.bin` handoff for Slice 6-owned deterministic state. CPU revisions such as `CPU CGB 0`, `CPU CGB A`, `CPU CGB C`, `CPU CGB D`, and `CPU CGB E` are documented hardware profiles only in this phase and must not enter behavior gates until a tested revision-specific difference is intentionally modeled.
...
- Do not claim functional closure for CGB-only special cartridges before the base CGB implementation can boot and validate CGB-only software end to end. `MBC30` is implemented after that base gate as an explicit `MBC3`-family variant, and `MBC7` now has a dedicated sensor / EEPROM runtime path with focused internal validation; `MBC6` still requires future cartridge-local runtime work rather than a fallback mapper approximation.
...
- The Slice 2 CGB direct-boot timer baseline seeds visible `DIV` to `0x26` and the hidden timer counter to `0x2674`, matching Mooneye `misc/boot_div-cgbABCDE.gb`; Slice 6 keeps `cgb-boot-div` on canonical `cgb_boot.bin` RealBoot by seeding CGB RealBoot power-on counter phase to `0xFFFB`, so executed firmware reaches the same centralized CGB `SkipBoot` handoff timer contract.
...
- Native CGB palette MMIO is enabled only when `ConsoleModel::GameBoyColor` is running `OperatingMode::Cgb`; DMG-family models and CGB-family `GbCompatible` mode keep these CGB-only palette data/index registers unavailable, while compatibility-mode rendering uses internally seeded CGB palette RAM through the Slice 4F adapter.
...
- functional support for remaining CGB-only special cartridges beyond the now-supported `MBC30` and focused MBC7 sensor / EEPROM path, such as `MBC6`
