...
The current implementation provides only a deterministic gb-cycle-owned generic visual HLE default border for presentation compatibility. The border is loaded immediately for SGB/SGB2 hosts and remains stable for DMG-only and otherwise non-SGB-enhanced cartridges because they do not send accepted SGB border commands. For SGB-enhanced cartridges, the host snapshots the fallback and starts fading it out as soon as a border transfer begins through `CHR_TRN` or `PCT_TRN`; if the fade-out reaches black before the cartridge completes its `PCT_TRN`, the host holds the border area black while the LCD window remains live. When `PCT_TRN` completes, the host fades from black to the cartridge border, then normal dynamic border composition owns presentation.
