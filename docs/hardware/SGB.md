...
The current public SGB/SGB2 implementation covers the Phase 11 slices 0-6 milestone plus post-slice hardening: architecture and save-state shape, profile-aware `SkipBoot`/`CustomBoot` direct-start registers, `RealBoot` asset selection, JOYP packet transport, SGB-header unlock policy, deterministic packet busy/suppression gating, base palette commands, BIOS title/default palette seeding for DMG-only titles, `_TRN` transfer scheduling and five-frame capture state, static/dynamic borders, `MASK_EN`, advanced screen coloring, transferred palette/attribute files, `PAL_PRI`, `MLT_REQ` multiplayer, `ATRC_EN`, `TEST_EN`, `ICON_EN`, `OBJ_TRN` state, SGB NTSC/PAL and SGB2 NTSC profile timing, desktop/CLI model exposure, `BORDER AUTO/OFF` presentation for SGB-family and handheld models, SGB2 physical Game Link routing, and a deterministic gb-cycle-owned generic visual-only HLE default border fallback that loads for every SGB/SGB2 host presentation, remains stable for DMG/non-SGB-enhanced cartridges, and fades to a cartridge border when an SGB-enhanced game completes `PCT_TRN`.

The core code mirrors these ownership boundaries with `crates/gb-core/src/sgb.rs` as a public façade. `sgb/host.rs` and `sgb/protocol.rs` are internal façades over functional submodules: host lifecycle/backend/transport/command/video/multiplayer state stays save-state-visible, protocol constants/requests/packet/transfer/color/palette/attribute/border/OBJ data stays flatly reexported to the SGB façade, `sgb/shell.rs` owns the visual HLE fallback transition, and `sgb/border.rs` owns the gb-cycle fallback asset constants; SGB unit tests use `sgb/test.rs` as a façade over functional files under `sgb/test/`.

Slices 7 and 8 remain not planned for the current `gb-cycle` architecture. Experiments for general SGB special audio through `SOUND`/`SOU_TRN` and SNES-side data transfer / 16-bit execution through `DATA_SND`/`DATA_TRN`/`JUMP` were inconclusive and showed that strict compatibility requires a complete SNES/SFC emulator with 65C816 execution, S-APU/S-DSP audio, host firmware behavior, and presentation state. The current default-border fallback and fade are explicitly HLE and presentation-focused: they do not execute the real SNES/SFC firmware, do not implement the real startup intro/animation, and must not be treated as real SGB special audio or SNES-side execution.
...
SGB/SGB2 profiles do not expose `dmg-cpu-0` even though the hosted GB core uses the shared `ConsoleModel::GameBoy` family. DMG-CPU 0 direct-start and real-boot handoff corrections are handheld DMG validation rules; SGB/SGB2 startup state, real-boot asset selection, and packet handoff behavior must come from `SgbHostProfile`.
...
Current SGB/SGB2 `RealBoot` executes only this 256-byte GB-side boot ROM on the shared DMG-compatible CPU/bus path. Separately, the deterministic SGB host seeds an HLE default border immediately for SGB/SGB2 presentation; DMG/non-SGB-enhanced cartridges keep it as a stable frame, while SGB-enhanced cartridges keep it until their first border transfer command, then the host fades fallback-to-black over `96` host frames, holds black if the cartridge border is not ready yet, and fades black-to-game-border over another `96` host frames after `PCT_TRN` completes. This HLE fallback is core-owned, save-state-visible, and presentation-only. The real-hardware Super Game Boy logo animation and built-in "Game Boy screen" border remain SNES-side host-firmware behavior that is not contained in `sgb_boot.bin` / `sgb2_boot.bin`, and `.sfc` firmware files are not active assets in the current implementation.
...
Slice 3 adds the shared 4 KiB `_TRN` capture seam, and post-Slice-6 hardening gives it a deterministic multi-frame transfer timeline. `CHR_TRN`, `PCT_TRN`, `PAL_TRN`, `ATTR_TRN`, `SOU_TRN`, and `DATA_TRN` are still normal one-packet SGB commands, but completing the packet only schedules a host-side transfer request and opens a five-frame busy window. Across the documented capture window the machine reconstructs the 4 KiB payload from the prepared GB transfer display when the transfer layout remains valid (BG enabled, no scroll, identity `BGP`, and the selected tilemap/tiledata mode), including signed-tiledata transfer screens such as Donkey Kong (GB). Once display-order capture is established, the host also latches the last visible display-order payload and reuses that payload for LCD-disabled tail frames such as Space Invaders, because the SGB host can only see the previously presented transfer screen after the GB LCD stops driving even if VRAM is immediately repurposed; otherwise it uses a deterministic raw fallback for HLE tests. The machine-level path advances this host-frame timeline once per `456×154` T-cycles, so repeated scheduler observations of the same PPU coordinate cannot compress transfer timing or shell fades. The partial payload, display-order latch, phase, frame count, target, and final payload are all save-state-visible. The direct one-shot capture helper is crate-internal only and exists as a deterministic HLE/test seam, so public callers cannot bypass the production multi-frame path. This is not a CPU VRAM read, not CGB VRAM behavior, and not frontend-only postprocessing.
...
## Borrowed handheld border presentation

`BORDER AUTO` for handheld models is a presentation bridge, not SGB hardware emulation on those models. The live `GAME BOY`, `GB POCKET`, `GB LIGHT`, `GB COLOR`, and `GB ADVANCE` machines keep `HostPlatform::Handheld`, reject SGB command side effects as handheld hardware, and expose only their real DMG-family palette or CGB-family RGB555 LCD output.

When a loaded handheld ROM has the SGB acceptance header (`$0146 = $03` and `$014B = $33`), the frontend may ask `gb-core` to run a temporary `SGB NTSC` machine for at most `600` frames, record the first completed `PCT_TRN` border transfer, keep stepping only until the borrowed border presentation state remains unchanged for a short settle window, clone the `SgbBorderState` plus the current application backdrop color needed for transparent border pixels, and then destroy the temporary machine. That cloned presentation state is attached as a borrowed border sidecar and is intentionally not part of save states, the active machine's `SgbHost`, or the active machine's model axes.

Borrowed composition draws only the 256×224 border area outside the 160×144 aperture. The aperture remains the live handheld output: frontend palette shading for DMG/MGB/LGB and CGB-family RGB555 for CGB/AGB. This v1 path extracts only the initial border and does not implement later dynamic SGB border updates, SGB palettes/attributes/masks, multiplayer, SGB audio, or any other SGB command feature on handheld models.

## HLE default border fallback

The real SNES/SFC-side startup shell owns the Super Game Boy logo animation, startup transfer presentation, built-in generic border, and other host-side presentation behavior shown before cartridge-side SGB border transfers. These effects are not contained in `sgb_boot.bin` / `sgb2_boot.bin`; those files are only GB-side low-window boot ROM images.

The current implementation provides only a deterministic gb-cycle-owned generic visual HLE default border for presentation compatibility. The border is loaded immediately for SGB/SGB2 hosts and remains stable for DMG-only and otherwise non-SGB-enhanced cartridges because they do not send accepted SGB border commands. For SGB-enhanced cartridges, the host snapshots the fallback and starts fading it out as soon as a border transfer begins through `CHR_TRN` or `PCT_TRN`; transparent LCD-window pixels fade with the shell instead of bypassing it, and if the fade-out reaches black before the cartridge completes its `PCT_TRN`, the host holds the whole 256×224 presentation black, including the LCD window. When `PCT_TRN` completes, the host fades from black to the cartridge border and LCD composition, then normal dynamic border composition owns presentation.

The fallback art reserves color index 0 only for the exact 160×144 GB LCD aperture at `(48, 40)` in the 256×224 host frame; the scanline above the aperture, the scanline below it, and the final host-frame scanline remain opaque border art so startup presentation matches the same LCD placement used by cartridge-transferred borders.

This HLE fallback and transition must remain core-owned and save-state-visible, not a frontend-only overlay. It does not execute real SNES/SFC firmware, does not implement the real startup intro/animation, does not make `.sfc` firmware files active assets, and does not change the GB-side `RealBoot` asset contract.
...
For the current `SkipBoot` SGB/SGB2 startup baseline, the cartridge-entry timer phase is synthesized from the boot-ROM JOYP header-transfer cost rather than from the handheld DMG direct-start counter. The modeled transfer covers cartridge bytes `$0100..$014F`; a set bit is one M-cycle (`4` T-cycles) faster than a reset bit, so gb-cycle uses all-zero-header phase `$DCA0` minus `4 * popcount(header)` to preserve the `boot_div-S` / `boot_div2-S` distinction without a title-specific duration override.
...
- Mooneye `acceptance/boot_regs-sgb.gb`, `acceptance/boot_regs-sgb2.gb`, `acceptance/boot_div-S.gb`, `acceptance/boot_div2-S.gb`, and `acceptance/boot_hwio-S.gb` run in the extra report as SGB/SGB2 `SkipBoot` public oracles for direct-start register fingerprints, SGB-specific JOYP/audio HWIO, and the header-bit-dependent SGB boot DIV phase; they do not claim SNES/SFC host startup-shell or SGB/SGB2 `RealBoot` closure.
