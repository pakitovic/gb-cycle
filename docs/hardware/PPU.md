...
- [inference] The deterministic DMG `CustomBoot` reset-facing lane keeps the internal synthetic first frame at `LY=0` for scheduler/test determinism, but bus-visible `FF44` readback lags that first frame by one line until VBlank so boot-handoff-style LY probes still observe the verified direct-boot phase without changing ordinary `SkipBoot` HWIO readback.
- [inference] During the first DMG-family cartridge-entry CPU bus probes, `gbmicrotest` exposes a narrower boot-facing publication table before the general first-frame `FF44` lag: CPU MMIO reads see `LY=0` through delay `119`, `LY=1` through delay `233`, and `LY=2` from delay `234`; this is a CPU-bus readback overlay only, with a `CustomBoot` reset-facing frame-origin base and a separate `RealBoot` handoff base, not a change to the internal synthetic raster, ordinary `SkipBoot` HWIO readback, or the verified direct-boot `LY=153` snapshot.
