...
- Keep the DMG BG palette-output model split from the raw current-scanline color pipeline. The CPU-path `BGP` model should keep three behaviors explicit and separate: delayed pipeline-visible writes, a narrow previous-line boundary repaint seam fed only by that delayed class, and retroactive panel recolor when either the first visible-line write lands at `visible_pixels_output == 0` / `current_transfer_x == 0` with no selected sprites or the already-visible BG tail is entirely color `0`. When a BG dot was already presented as `LCDC.0`-forced white, that retroactive recolor must leave the panel dot white instead of remapping it through `BGP`. Historical panel repaints triggered by `LCDC.0`/`LCDC.1` live writes must not reuse the active `BGP` output-delay override that feeds future pipeline pixels; repaint palettes come from the latch-level historical `BGP`, except explicitly modeled CGB-family `LCDC.1` visible-hold seams.
...
- [inference] On DMG-family timing, the bus-facing `FF44` readback can advance to the next scanline from visible-line dot `451` (`DOTS_PER_SCANLINE - 5`) before the full raster wrap completes; do not force bus-visible `LY` reads to be identical to the internal raster/comparison line at every dot, and keep dot `450` on the current visible line.
...
- [inference] For non-extended Mode `3 -> 0` seams, the exact-boundary CPU-visible `STAT.mode = HBlank` override is a Mode `0` IRQ-publication seam, not a universal readback rule; nonzero-`SCX` Mode `2`-only probes at the same internal Mode `0` start dot can still see published Drawing on the CPU bus, while `SCX=0` keeps the exact-boundary HBlank publication used by Mooneye.
- [inference] Sprite-extended Mode `3` lines publish CPU-visible `STAT.mode` through the same one-dot readback lag and the same Mode `0` boundary override as non-sprite lines; once the per-sprite penalty above lands the internal Mode `3 -> 0` boundary on the hardware-true dot, no per-layout early-publication aperture is needed. The earlier model carried per-sprite-layout publication tables (saturated ten-OBJ step-8 tails, single-sprite tail apertures) to compensate an under-counted penalty; those are removed.
...
- [inference] Steady-state Mode `0` HALT wake can also lag the Mode `0` `IF` pretrigger across the four-dot aperture immediately before HBlank; CGB-family applies that wake gate for all `SCX` low bits, while DMG-family `hblank_ly_scx_timing-GS` currently requires it only for `SCX&7` in `{1,2,5,6}`. Keep this as halted-CPU wake gating for Mode `0`-enabled STAT rather than moving the PPU request edge or applying it to Mode `2`-only paths.
...
- [inference] CGB-family DMG-software mode has its own line `143 -> 144` Mode `2` STAT-only source at the final HBlank pretrigger dot; keep it separate from the DMG VBlank-priority interrupt-service deferral so Mooneye `misc/ppu/vblank_stat_intr-C.gb` can observe LCD STAT before the dedicated VBlank request.
...
- [hardware fact] On an object match the fetch first stalls while the BG fetcher has not yet reached the point where its fetched slice is ready (`TileDataHigh` second dot or later) or the BG FIFO is empty; each stall dot advances the BG fetcher. Once that alignment is satisfied the sprite is fetched in a fixed `6`-dot sequence (`Startup`, `TileDataLow`, `TileDataHigh`, two dots each) with the object-row overlay applied on the final dot. There is no separate object `Push` stage.
- [hardware fact] The alignment stall is paid once per BG tile reached by the fetcher, not once per sprite: the first object that matches inside a given `8`-pixel BG tile pays the alignment cost and later objects matching the same tile pay only the `6`-dot fetch. The per-tile key uses the fetcher pixel including the `SCX` fine-scroll discard.
- [hardware fact] Single-sprite Mode 3 cost by BG-fetcher alignment phase `0..7` at the match is `11/10/9/8/7/6/6/6` dots (`6` fixed plus `max(0, 5 - phase)` alignment), where the phase is shifted by the `SCX & 7` fine-scroll discard. A sprite at `X = 0` always pays the full `11` because rendering and the `SCX` discard cannot start while an `X = 0` object is pending.
- [inference] On CGB-family hidden-lane fetches (`current_transfer_x < 8`) the alignment phase uses the raw transfer position without the `SCX` discard shift; DMG-family and visible-lane fetches use the discard-shifted phase.
- [hardware fact] A sprite whose effective match position is `X >= 168` never matches and adds no Mode 3 cost; there is no separate offscreen-right penalty adjustment.
...
- [hardware fact] The `SCX & 7 > 0` together with `X = 0` case is covered by the alignment model above: the `X = 0` object blocks rendering and the `SCX` discard until its fetch completes, so it pays the full alignment stall regardless of `SCX & 7`. This is no longer a separately approximated penalty.
...
- [inference] The `WX = 0 && (SCX & 7) == 3` terminal seam keeps CPU-visible `STAT.mode` in Mode `3` for the one-dot preterminal read used by `gbmicrotest` before publishing HBlank on the following probe. The one-dot readback lag now produces this directly: the preterminal read sees the prior dot's Drawing and the boundary override publishes HBlank only at the Mode `0` start dot, so the dedicated keep-drawing guard that the earlier per-layout publication model needed is no longer required.
