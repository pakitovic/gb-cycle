...
- [inference] SameBoy's research-backed reference implementation also mirrors the read-corruption first-word result into the previous row's first word; keep this as a read-only refinement of the Pan Docs formula and do not apply the mirror to write-triggered corruption.
