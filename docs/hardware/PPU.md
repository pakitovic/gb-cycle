...
- Keep the same distinction for Mode `2` at LCD restart, Mode `0` edges, and ordinary line starts: a pretriggered or edge-aligned LCD STAT request may be aggregated in the same T-cycle and wake the CPU, but a CPU instruction reading `IF` earlier in that T-cycle must still see the previously aggregated `IF` value rather than the PPU's unaggregated pending bit.
