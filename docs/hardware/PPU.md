...
- CGB compatibility RGB555 output should still use CGB palette RAM instead of the DMG grayscale path: BG/window color indices are first remapped through the pipeline-visible `BGP` register and then look up BG palette `0`, OBJ color indices are first remapped through `OBP0` or `OBP1` and then look up OBJ palette `0` or `1`, and CGB-only palette MMIO remains unavailable to compatibility-mode software.
...
- [inference] The deterministic DMG `SkipBoot` continuity path keeps the internal synthetic first frame at `LY=0` for scheduler/test determinism, but bus-visible `FF44` readback lags that first frame by one line until VBlank so boot-handoff-style LY probes still observe the verified direct-boot phase.
- [inference] During the first DMG-family cartridge-entry CPU bus probes, `gbmicrotest` exposes a narrower boot-facing publication table before the general `SkipBoot` first-frame `FF44` lag: CPU MMIO reads see `LY=0` through delay `119`, `LY=1` through delay `233`, and `LY=2` from delay `234`; this is a CPU-bus readback overlay only, with a `SkipBoot` frame-origin base and a separate `RealBoot` handoff base, not a change to the internal synthetic raster or the verified direct-boot `LY=153` snapshot.
...
- [inference] DMG line `153` `LYC=0` STAT has a narrower STAT-only pretrigger: it may request LCD STAT at dot `8` so a running CPU can take the interrupt before the next `DI`, but readable coincidence remains low until dot `12` and a same-dot CPU write that changes `LYC`/`STAT` cancels the unaggregated request.
