...
- For exploratory ROM-driven work, preserve baseline and final `/.roms/test/test-report.md` snapshots for promoted suites, `/.roms/test/test-report-extra.md` snapshots for non-DocBoy extra/internal suites, or `/.roms/test/test-report-docboy.md` snapshots for the large DocBoy suites, compare them before keeping the change, and isolate one failing family at a time.
