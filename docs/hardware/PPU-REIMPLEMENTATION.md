...
- When OBJ startup helpers need the remaining hidden pops before the first visible pixel, derive that from the logical startup-visible X (`current_transfer_x + initial_scx_discard`) rather than from `current_transfer_x < 8` alone; the raw-X shortcut reopens the curated `m3_lcdc_obj_size_change_scx` seam.
