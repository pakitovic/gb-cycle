...
- For exploratory ROM-driven work, preserve baseline and final `/test/gb-emulator-shootout/test-report.md` snapshots for promoted GB Emulator Shootout suites, `/test/test-report-extra.md` snapshots for non-DocBoy extra/internal suites, `/test/docboy/test-report.md` snapshots for the large DocBoy suites, or `/test/gbmicrotest/test-report.md` snapshots for gbmicrotest, compare them before keeping the change, and isolate one failing family at a time.
