...
- Native-CGB shift-zero CH1 sweep clocks immediately after a trigger honor a revision-scoped sweep-restart hold before refreshing the unshifted addend or scheduling overflow; the promoted public SameSuite `channel_1_sweep_restart_2.gb` CGB-E oracle requires the longer `9` scheduler T-cycle hold, so `CpuCgbE` currently shares the same hold as the standard CGB profile instead of the earlier short `5` hold inferred from the DocBoy-only `channel_1_sweep_restart_2-cgbE` variant. Once the hold expires, the unshifted delayed overflow check resolves after `4` scheduler T-cycles, and the latch is serialized with CH1 sweep state.
