...
- Runtime `NR52` power-on must consume the timer-owned current `DIV-APU` signal without cloning divider logic inside the APU: DMG-family hardware resets the frame sequencer to step `0`; when power-on occurs while the selected `DIV-APU` source is high, the first following frame-sequencer edge is suppressed and `NRx4` length-enable writes in that pending window observe the effective next step as `1`, while CGB-family hardware enters the high-half phase (`7`) so the next high-to-low transition clocks the envelope phase instead of delivering the step-0 length/sweep clocks. SameSuite `div_write_trigger_10`, `div_write_trigger_volume_10`, and `div_trigger_volume_10` cover the CGB path through repeated `DIV` writes and natural `DIV-APU` events after enabling APU at visible `DIV=$10`, while the DMG path is currently anchored by DocBoy APU length-timer cases and focused `gb-core` frame-sequencer regressions.
...
- Writes to `NR13` or `NR14` should not change the currently playing sample instantly; the new period should normally take effect after the current sample ends. The native-CGB pulse core has one important DocBoy-observed seam: if a CH1/CH2 sample has just completed, an `NRx3` write or an `NRx4` non-trigger period-high write reloads the generation timer from the new register period during a short three-generation-clock `just_sampled` window. SameSuite/DocBoy `channel_1_freq_change_timing-cgbDE` covers this in both normal and double speed.
...
- On CH1 trigger:
  - native-CGB should copy the current period into `sweep_shadow_period`; the current DMG-family baseline resets the public shadow-period field and reloads the live period through the timed recalculation pipeline instead of using a one-instant trigger calculation
  - the sweep timer should reset
  - `sweep_enabled` should become true if sweep pace or sweep shift are non-zero, false otherwise
  - if sweep shift is non-zero, sweep calculation should remain explicit; native CGB routes trigger-time overflow through the delayed sweep-pipeline latch, while DMG-family trigger and post-writeback checks currently run through the repo-local T-cycle/M-cycle recalculation countdown validated by DocBoy CH1 sweep rows
...
- On a sweep clock while sweep is enabled and pace is non-zero:
  - calculate the new period and perform overflow check through the model's active hardware pipeline
  - if the result is in range and shift is non-zero, write it back to `sweep_shadow_period`, `NR13`, and `NR14`
  - then run the second overflow check using the new shadow period without writing that second result back; on native CGB this is a short delayed pipeline check, and on the current DMG-family baseline it is deferred through the explicit reload/countdown window rather than being collapsed into the next ordinary sweep tick
- Keep this second overflow check explicit; do not fold it into a generic "next tick will catch it" simplification.
- Native-CGB keeps trigger-time and post-writeback addition-overflow checks as explicit delayed sweep-pipeline calculations rather than disabling CH1 in the same frame-sequencer instant; SameSuite `channel_1_sweep` and `channel_1_sweep_restart` require the channel to remain active for the short low-shift-derived delay after the first calculation writes `NR13`/`NR14` or after a CGB trigger initializes sweep, while the DMG-family baseline keeps its explicit DocBoy-observed recalculation/reload pipeline covered by unit tests.
...
- Native-CGB sweep writebacks do not add a second active-retrigger delay on top of the shared pulse-core delay; SameSuite/DocBoy `channel_1_sweep_restart-cgbE` expects the decreasing-writeback retrigger seam to remain on the base active-retrigger timing rather than a CH1-local extension.
- Native-CGB shift-zero CH1 sweep clocks immediately after a trigger honor a revision-scoped sweep-restart hold before refreshing the unshifted addend or scheduling overflow; the standard CGB-C profile keeps the longer `9` scheduler T-cycle hold required by the promoted SameSuite `channel_1_sweep_restart_2.gb` fixture, while the CGB-E profile selected by `MachineConfig::revision = CpuCgbE` uses the short `5` scheduler T-cycle hold required by SameSuite/DocBoy `channel_1_sweep_restart_2-cgbE`. Once the hold expires, the unshifted delayed overflow check resolves after `4` scheduler T-cycles, and the latch is serialized with CH1 sweep state.
- Repo-local DMG-family sweep baseline: the implementation keeps the DocBoy-observed CH1 sweep recalculation/reload pipeline as explicit serialized state (`recalculation`, `period_increment`, reload countdown, trigger-window counters, and restart delay), including `NR10`/`NR13`/`NR14` writes inside the trigger/reload windows; this is intentionally localized in CH1 sweep state rather than encoded as a ROM-specific pass condition.
...
- `SkipBoot` should keep all currently synthesized hidden APU startup state explicit. In the current DMG baseline that means at least the shared-divider-derived `DIV-APU` / frame-sequencer phase, powered state, visible `NRxx` ownership, wave-RAM startup policy, and the live channel-active state reconstructed from `NR52`; runtime-only subcycle helpers such as the APU fast-clock phase and pending DMG NR52 high-half edge skip must be cleared when applying a full startup state, while fuller hidden-state continuity such as HPF history, pulse duty phase, CH3 buffered-sample position, and CH4 LFSR/noise-timer continuation remains deferred and must not be described as already solved.
