...
- The CGB-family direct-start PCM-silence mask is an explicit hidden-state seam: it preserves the published `NR52` active bits and CGB `PCM12`/`PCM34` tap ownership while preventing a synthetic pulse phase from leaking nonzero boot audio into no-boot ROM probes until software performs a real channel trigger or powers the APU through an ordinary transition.
