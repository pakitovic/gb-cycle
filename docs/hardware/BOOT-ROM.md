...
The current active set is intentionally narrower than the modeled enum: DMG exposes `DmgCpu0` and `DmgCpuC`, MGB/LGB expose only `CpuMgb`, CGB exposes `CpuCgbC`, `CpuCgbD`, and `CpuCgbE`, and AGB exposes only `CpuAgbA`. DMG-CPU 0 is active for targeted boot validation while `DmgCpuC` remains the default ordinary Game Boy profile; other earlier DMG and CGB revision variants remain modeled so real-boot assets, save-state metadata, and future hardware validation do not need another rename when they become active; earlier AGB0, AGS, and Game Boy Player rows remain model-axis documentation until they have distinct core behavior to expose.
