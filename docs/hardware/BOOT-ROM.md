...
| `GameBoyAdvance` | `CpuAgbA` | `CpuAgbA` | `cgb_agb_boot.bin` | AGB CGB-compatible post-boot profile with GBA-enhanced detection register fingerprint and no CGB infrared port |
...
The current active set is intentionally narrower than the modeled enum: handheld DMG exposes `DmgCpu0` and `DmgCpuC`, SGB/SGB2 profiles expose only the profile-backed `DmgCpuC` GB core, MGB/LGB expose only `CpuMgb`, CGB exposes `CpuCgb0`, `CpuCgbC`, `CpuCgbD`, and `CpuCgbE`, and AGB exposes only `CpuAgbA`. DMG-CPU 0 and CGB-CPU 0 are active for targeted handheld boot validation while `DmgCpuC` remains the default ordinary Game Boy and SGB-family GB-core profile and `CpuCgbE` remains the default ordinary Game Boy Color profile; other earlier DMG and CGB revision variants remain modeled so real-boot assets, save-state metadata, and future hardware validation do not need another rename when they become active; earlier AGB0, AGS, and Game Boy Player rows remain model-axis documentation until they have distinct core behavior to expose.
...
- For the current deterministic DMG-family direct-start continuity baseline, that coherence includes one explicit hidden divider phase instead of only the visible `DIV` byte. The current model seeds the shared timer system counter to `0xABC8` for DMG-family direct boot so Mooneye's DMG-family `boot_div` cadence lines up with the expected first post-boot `DIV` edges. CGB-family direct boot is header-aware for validated buckets: missing and DMG-compatible CGB headers keep the `0x2674` CGB ABCDE baseline used by Mooneye `misc/boot_div-cgbABCDE.gb`, AGB DMG-compatible headers use `0x2678` so Mooneye `misc/boot_div-A.gb` matches the verified `cgb_agb_boot.bin` RealBoot handoff phase, native CGB non-Nintendo old-licensee headers currently use `0x1E84` so Ashiepaws `bully.gb` sees the same initial-`DIV` family as gb-cycle's standard `cgb_boot.bin` RealBoot handoff, and native CGB old-licensee `$33` headers with binary-zero new-licensee bytes use `0x1E98` so Nitro2k01 `whichboot.gb` sees the hardware-facing GBC entry fingerprint without runner-only timer overlays.
...
- tests that AGB `RealBoot` resolves `cgb_agb_boot.bin` through the same loader/fingerprint path instead of aliasing to CGB-E or native GBA BIOS assets
