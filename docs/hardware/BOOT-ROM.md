...
- The deterministic CGB `Machine::SkipBoot` continuity snapshot is aligned to the validated CGB direct-start handoff fields owned by Slice 6: CGB native headers enter with `AF=$1180`, `BC=$0000`, `DE=$FF56`, `HL=$000D`, `SP=$FFFE`, and `PC=$0100`, while DMG-only compatibility headers enter with `AF=$1180`, `BC=$0000`, `DE=$0008`, `HL=$007C`, `SP=$FFFE`, and `PC=$0100`; both CGB paths seed released joypad state with unselected rows so the visible `P1` handoff readback is `$FF`. The AGB direct-start profile uses the same CGB-family fields except for the GBA-enhanced detection fingerprint: native CGB headers enter with `AF=$1100`, `BC=$0100`, `DE=$FF56`, `HL=$000D`, and DMG-compatible headers enter with `AF=$1180`, `BC=$0100`, `DE=$0008`, `HL=$007C`; in both cases `B.bit0` is set for software that distinguishes GBA-compatible hardware from CGB.
