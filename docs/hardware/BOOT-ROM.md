...
- `CustomBoot` must initialize the same direct CPU, visible I/O, and hidden-state baseline as `SkipBoot`, but overlays boot-logo-visible VRAM according to the selected console family: DMG-family custom boot writes the DMG boot-logo tile bytes and logo tilemap, while CGB custom boot preserves the CGB direct-start memory base and writes only the boot-logo tile bytes so the `$9904/$9924` tilemap residue remains clear.
...
- For the current deterministic `Machine::SkipBoot` continuity baseline, that coherence includes one explicit hidden divider phase instead of only the visible `DIV` byte. The current model seeds the shared timer system counter to `0xABC8` for DMG-family direct boot so Mooneye's DMG-family `boot_div` cadence lines up with the expected first post-boot `DIV` edges. CGB direct boot is header-aware for validated buckets: missing and DMG-compatible headers keep the `0x2674` CGB ABCDE baseline used by Mooneye `misc/boot_div-cgbABCDE.gb`, while native CGB non-Nintendo old-licensee headers currently use `0x1E84` so Hacktix `bully.gb` sees the same initial-`DIV` family as gb-cycle's standard `cgb_boot.bin` RealBoot handoff.
...
- The standard CGB `RealBoot` power-on path seeds the hidden timer counter to `0xFFFB` so executing canonical `cgb_boot.bin` reaches the cartridge-header-dependent post-handoff phase used by the centralized CGB direct-start contract: `0x2674` for the Mooneye `boot_div-cgbABCDE` DMG-compatible baseline and `0x1E84` for the currently validated Hacktix `bully.gb` native CGB non-Nintendo bucket.
