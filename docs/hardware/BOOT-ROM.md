...
`ConsoleModel` is the visible handheld console product exposed to frontends and persisted settings. `HardwareRevision` is the CPU/revision profile for that handheld model; handheld `RealBoot` derives the boot ROM filename, expected size, and expected SHA-256 from it. The active `GameBoyAdvance` profile models the AGB GB/C compatibility path only: `CpuAgb0` selects `cgb_agb0_boot.bin` and `CpuAgbA` selects `cgb_agb_boot.bin`, but neither revision selects a native `gba_bios.bin` because the GBA BIOS is outside this GB/C core startup path. SGB/SGB2 frontend machine profiles are resolved into the same DMG-compatible `ConsoleModel`/`HardwareRevision` core plus an SGB host profile, and that host profile derives `sgb_boot.bin` or `sgb2_boot.bin`. A separate boot-ROM-kind axis is not public configuration: the internal asset descriptor is derived from `HardwareRevision` for handheld profiles or from `SgbHostProfile` for SGB/SGB2 profiles, and frontends/manifests should continue to expose machine profiles rather than arbitrary boot-ROM kinds. `SkipBoot` and `CustomBoot` use the selected revision/profile for silicon/direct-start behavior but do not load or emulate boot-ROM bytes.

| ConsoleModel | Active revisions | Default revision | RealBoot filename from default | Direct-start profile |
|---|---|---|---|---|
| `GameBoy` | `DmgCpu0`, `DmgCpuC` | `DmgCpuC` | `dmg_boot.bin` | DMG0 or standard DMG post-boot profile selected by revision |
...
| `GameBoyColor` | `CpuCgb0`, `CpuCgbC`, `CpuCgbD`, `CpuCgbE` | `CpuCgbD` | `cgb_boot.bin` | CGB0, standard CGB, or explicit CGB-E post-boot profile selected by revision and aligned to validated CGB direct-start handoff fields |
| `GameBoyAdvance` | `CpuAgb0`, `CpuAgbA` | `CpuAgbA` | `cgb_agb_boot.bin` | AGB CGB-compatible post-boot profile with GBA-enhanced detection register fingerprint and no CGB infrared port; `CpuAgb0` currently differs only by explicit revision identity and `cgb_agb0_boot.bin` RealBoot asset |

| HardwareRevision | RealBoot filename | Expected size | Expected SHA-256 profile |
|---|---|---:|---|
| `DmgCpu0` | `dmg0_boot.bin` | `256` | DMG0 |
| `DmgCpuA` / `DmgCpuB` / `DmgCpuC` | `dmg_boot.bin` | `256` | standard DMG |
| `CpuMgb` | `mgb_boot.bin` | `256` | MGB |
| `CpuCgb0` | `cgb0_boot.bin` | `2304` | CGB0 |
| `CpuCgbA` / `CpuCgbB` / `CpuCgbC` / `CpuCgbD` | `cgb_boot.bin` | `2304` | standard CGB |
| `CpuCgbE` | `cgbE_boot.bin` | `2304` | CGB-E |
| `CpuAgb0` | `cgb_agb0_boot.bin` | `2304` | AGB0 CGB-compatible, SHA-256 `fe2d45405531756d87622abde6127c804bd675cb968081b2c052497a470ffeb2` |
| `CpuAgbA` | `cgb_agb_boot.bin` | `2304` | AGB CGB-compatible, SHA-256 `fe3cceb79930c4cb6c6f62f742c2562fd4c96b827584ef8ea89d49b387bd6860` |

The current active set is intentionally narrower than the modeled enum: handheld DMG exposes `DmgCpu0` and `DmgCpuC`, SGB/SGB2 profiles expose only the profile-backed `DmgCpuC` GB core, MGB/LGB expose only `CpuMgb`, CGB exposes `CpuCgb0`, `CpuCgbC`, `CpuCgbD`, and `CpuCgbE`, and AGB exposes `CpuAgb0` and `CpuAgbA`. DMG-CPU 0, CGB-CPU 0, and AGB0 are active for targeted handheld boot validation while `DmgCpuC` remains the default ordinary Game Boy and SGB-family GB-core profile, `CpuCgbD` remains the default ordinary Game Boy Color profile, and `CpuAgbA` remains the default ordinary Game Boy Advance GB/C profile; other earlier DMG and CGB revision variants remain modeled so real-boot assets, save-state metadata, and future hardware validation do not need another rename when they become active; AGS and Game Boy Player rows remain model-axis documentation until they have distinct core behavior to expose.
...
- CGB and AGB CGB-compatible real boot publish both the low window and the upper boot-ROM window at `0200-08FF` through the same contract, without changing bus-state shape.
...
- Strict RealBoot validation for explicit `CpuCgb0` requires canonical `cgb0_boot.bin` as a `2304`-byte sparse asset with SHA-256 `3a307a41689bee99a9a32ea021bf45136906c86b2e4f06c806738398e4f92e45`; explicit `CpuCgbC` or default/explicit `CpuCgbD` requires canonical `cgb_boot.bin` with SHA-256 `b4f2e416a35eef52cba161b159c7c8523a92594facb924b3ede0d722867c50c7`, explicit `CpuCgbE` selects `cgbE_boot.bin` with SHA-256 `c56299bedd56debdbf36442238636bf5887a65c5173b33995682052353804da9` automatically, the AGB0 `CpuAgb0` profile selects `cgb_agb0_boot.bin` with SHA-256 `fe2d45405531756d87622abde6127c804bd675cb968081b2c052497a470ffeb2` automatically, and the AGB `CpuAgbA` profile selects `cgb_agb_boot.bin` with SHA-256 `fe3cceb79930c4cb6c6f62f742c2562fd4c96b827584ef8ea89d49b387bd6860` automatically. `gba_bios.bin` is not part of this GB/C boot-ROM asset contract.
- SGB and SGB2 real-boot assets are DMG-sized `0x100`-byte low-window GB-side images selected by the SGB host profile: original SGB profiles select `sgb_boot.bin`, while `Sgb2Ntsc` selects `sgb2_boot.bin`. The core still uses the existing DMG-compatible CPU/bus model for executing that GB-side image, but boot-ROM loading, fingerprints, and strict verification use the SGB asset identity. This asset selection does not by itself execute the SNES/SFC-side Super Game Boy host firmware. The deterministic SGB host separately provides a gb-cycle-owned generic HLE default border for SGB/SGB2 presentation, fades that fallback out when an SGB-enhanced cartridge begins a border transfer, and fades in the cartridge border after `PCT_TRN` completes; this remains host state, not boot-ROM bytes, and `.sfc` Super Game Boy firmware files are not active boot assets in the current implementation.
...
- `SkipBoot` must initialize a model-specific post-boot state directly, start execution at `0x0100`, leave boot ROM mapping disabled from the beginning, and seed the boot-logo VRAM residue used by fast no-boot tooling.
- `CustomBoot` must remain an explicit direct-start path rather than a partial boot-ROM execution: it starts from the same direct CPU, visible I/O, hidden-state, and boot-logo memory baseline as `SkipBoot`, then adds the reset-facing DMG boot PPU CPU-bus publication phase; CGB custom boot preserves the CGB direct-start CPU/memory base, applies the header-aware cartridge-entry timer/raster phase for boot-sensitive CGB fixtures, and keeps the same logo-tile-only CGB memory residue as `SkipBoot`.
...
- In the current repo baseline, keep centralized direct-start contracts explicit instead of conflating them: DMG-family `Machine::SkipBoot` keeps the deterministic direct CPU/I/O continuity profile used by core/fixture tests and Mooneye HWIO checks while seeding the DocBoy-like DMG boot-logo tile/map VRAM residue, `Machine::CustomBoot` adds only the reset-facing PPU CPU-bus publication overlay to that same direct baseline, CGB `Machine::SkipBoot` derives the direct handoff from the cartridge header where that evidence is currently validated while seeding only the DMG logo tile bytes, CGB `Machine::CustomBoot` preserves the same CGB CPU/memory direct-start base while using the core cartridge-entry timer/raster phase, and `BootController::direct_boot_state()` owns the verified DMG-family cartridge-entry snapshot used to validate real `FF50` handoff behavior.
...
- DMG-family boot ROM selection should remain explicit through `HardwareRevision` values covering at least `DmgCpu0`, `DmgCpuA/B/C`, and `CpuMgb`, with the concrete filename derived rather than independently configured.
...
- The deterministic later-DMG-family `Machine::SkipBoot` continuity snapshot currently uses `P1=0xCF`, `SB=0x00`, `SC=0x7E`, `DIV=0xAB`, `TIMA=0x00`, `TMA=0x00`, `TAC=0xF8`, `IF=0xE1`, `LCDC=0x91`, `STAT=0x85`, `SCY=0x00`, `SCX=0x00`, `LY=0x00`, `LYC=0x00`, `DMA=0xFF`, `BGP=0xFC`, `WY=0x00`, `WX=0x00`, and `IE=0x00`.
- The deterministic DMG0 direct-start snapshot is revision-specific: CPU registers use the DMG-CPU 0 table above, visible boot HWIO uses `P1=0xCF`, `SB=0x00`, `SC=0x7E`, `DIV=0x18`, `TIMA=0x00`, `TMA=0x00`, `TAC=0xF8`, `IF=0xE1`, `LCDC=0x91`, `STAT=0x83`, `SCY=0x00`, `SCX=0x00`, `LY=0x01`, `LYC=0x00`, `DMA=0xFF`, `BGP=0xFC`, `WY=0x00`, `WX=0x00`, and `IE=0x00`, with the hidden timer counter seeded to `$182C` so Mooneye `boot_div-dmg0` and `boot_hwio-dmg0` observe the same cartridge-entry timing in `SkipBoot` and `CustomBoot`.
- The SGB/SGB2 `Machine::SkipBoot` direct-start profile keeps the SGB/SGB2 CPU register fingerprints separate from handheld later-DMG, leaves `P1=$FF` with both JOYP rows unselected, uses the SGB-family `NR52=$F0` audio readback, and derives the hidden timer counter from the SGB boot-ROM header-transfer timing: the all-zero-header phase is `$DCA0`, then each set bit in cartridge bytes `$0100..$014F` subtracts one M-cycle (`4` T-cycles), matching Mooneye `boot_div-S`, `boot_div2-S`, and `boot_hwio-S` without hardcoding a single boot duration.
- The later-DMG/MGB `Machine::SkipBoot` continuity snapshot uses the same CPU, visible I/O, hidden subsystem state, and plain PPU readback as before, while its memory policy now writes the DMG boot-logo tile stream at even VRAM addresses starting at `$8010` and the logo tilemap at `$9904..$992F`, including the second tilemap row at `$9924..$992F`; this is the fast no-boot logo-memory contract used by DocBoy-like tooling and is not evidence that reset-facing PPU publication belongs in ordinary `SkipBoot`. The later-DMG/MGB `Machine::CustomBoot` continuity snapshot starts from the same direct baseline and additionally arms the reset-facing boot PPU CPU-bus publication overlay; DMG0 arms its revision-specific boot publication phase in both `SkipBoot` and `CustomBoot` because Mooneye DMG0 boot HWIO exposes that cartridge-entry publication contract. The CGB `Machine::SkipBoot` and `Machine::CustomBoot` memory policies keep the CGB direct-start WRAM/HRAM/VRAM base and write the same logo tile stream without the `$9904..$992F` tilemap overlay, matching the CGB no-boot fixture distinction between logo tiles and clear tilemap residue; only `CustomBoot` applies the CGB custom cartridge-entry raster phase.
- The DMG0 direct-boot PPU publication phase is a one-shot cartridge-entry overlay: it may extend past the ordinary startup clear point so the first frame matches DMG0 boot HWIO observations, but it must clear when VBlank starts at `LY=144` so the modulo frame-dot publication table cannot reappear on later frames.
- The deterministic CGB `Machine::SkipBoot` continuity snapshot is aligned to the validated CGB direct-start handoff fields owned by Slice 6: CGB native headers enter with `AF=$1180`, `BC=$0000`, `DE=$FF56`, `HL=$000D`, `SP=$FFFE`, and `PC=$0100`, while DMG-only compatibility headers enter with `A=$11`, `F=$80`, `B=<Nintendo title checksum or $00>`, `C=$00`, `DE=$0008`, `HL=$991A` when `B` is `$43` or `$58` and `HL=$007C` otherwise, `SP=$FFFE`, and `PC=$0100`; both CGB paths seed released joypad state with unselected rows so the visible `P1` handoff readback is `$FF`. The AGB direct-start profile uses the same CGB-family fields except for the GBA-enhanced detection fingerprint: native CGB headers enter with `AF=$1100`, `BC=$0100`, `DE=$FF56`, `HL=$000D`, and DMG-compatible headers enter with `A=$11`, `B=<CGB DMG-mode B + 1>`, `C=$00`, `DE=$0008`, `F` from the final `INC B` flags with `N/C` clear, and `HL=$991A` when `B` is `$44` or `$59` and `HL=$007C` otherwise; native CGB headers keep `B=$01` for software that distinguishes GBA-compatible hardware from CGB.
- The same CGB/AGB `Machine::SkipBoot` path keeps the CGB-family DIV/timer state coherent through a header-aware direct-start predictor: CGB0 uses the early-boot-ROM bucket (`DIV=$28` with hidden timer counter `$2880`) validated by Mooneye `misc/boot_div-cgb0.gb` against `cgb0_boot.bin`, missing cartridges and DMG-compatible CGB C/D/E headers keep the Slice 2 CGB ABCDE baseline (`DIV=$26` with hidden timer counter `$2674`) validated by Mooneye `misc/boot_div-cgbABCDE.gb`, AGB-family DMG-compatible headers use the AGB boot-ROM handoff phase (`DIV=$26` with hidden timer counter `$2678`) validated by Mooneye `misc/boot_div-A.gb` against `cgb_agb_boot.bin`, `CpuAgb0` shares that `CpuAgbA` direct-start timer/raster bucket until evidence shows a separate AGB0 synthetic handoff, the currently validated native CGB non-Nintendo old-licensee bucket seeds `DIV=$1E` with hidden timer counter `$1E84` for Ashiepaws `bully.gb`, and the native CGB old-licensee `$33` plus binary-zero new-licensee bucket seeds `DIV=$1E` with hidden timer counter `$1E98` for Nitro2k01 `whichboot.gb`. The same path also mirrors the boot-owned RealBoot handoff state that is deterministic in this repo: locked `KEY0`-derived `OperatingMode`, `KEY1`/bank/MMIO readbacks for the selected mode, CGB compatibility-mode `BCPS=$C8` and `OCPS=$D0` index readbacks, CGB palette RAM seed and `OPRI` state where exposed by the PPU model, alternating CGB wave RAM bytes for standard CGB and AGB-family boot ROMs, zeroed CGB0 direct-start wave RAM matching the boot ROM that does not initialize wave RAM, and boot-owned visible memory prefixes in VRAM/WRAM/HRAM.
- The verified DMG-family cartridge-entry snapshot owned by `BootController::direct_boot_state()` now tracks the real-handoff-visible fields used by the repo-local regression matrix: `P1=0xCF`, `DIV=0xAB`, `STAT=0x81`, and `LY=153` for `DMG` / `MGB`, while `DMG0` uses its own direct-entry snapshot and an explicit `RealBoot` handoff correction at the `FF50` unmap edge so `skip-boot`, `custom-boot`, and private-firmware `real-boot` converge on the same DMG-CPU 0 boot-register and DIV publication contract.
...
- For the current deterministic DMG-family direct-start continuity baseline, that coherence includes one explicit hidden divider phase instead of only the visible `DIV` byte. The current model seeds the shared timer system counter to `0xABC8` for DMG-family direct boot so Mooneye's DMG-family `boot_div` cadence lines up with the expected first post-boot `DIV` edges. CGB-family direct boot is header-aware for validated buckets: `CpuCgb0` uses its dedicated `0x2880` CGB0 bucket, missing CGB-family cartridges and DMG-compatible CGB headers keep the `0x2674` CGB ABCDE baseline used by Mooneye `misc/boot_div-cgbABCDE.gb`, AGB-family DMG-compatible headers use `0x2678` so Mooneye `misc/boot_div-A.gb` matches the verified `cgb_agb_boot.bin` RealBoot handoff phase, `CpuAgb0` shares the `CpuAgbA` AGB-family direct-start buckets until evidence shows otherwise, native CGB non-Nintendo old-licensee headers currently use `0x1E84` so Ashiepaws `bully.gb` sees the same initial-`DIV` family as gb-cycle's standard `cgb_boot.bin` RealBoot handoff, and native CGB old-licensee `$33` headers with binary-zero new-licensee bytes use `0x1E98` so Nitro2k01 `whichboot.gb` sees the hardware-facing GBC entry fingerprint without runner-only timer overlays.
...
- The explicit standard CGB and AGB CGB-compatible `RealBoot` power-on paths seed the hidden timer counter to `0xFFFB` so executing canonical `cgb_boot.bin` preserves the Mooneye `boot_div-cgbABCDE` DMG-compatible `0x2674` bucket, while executing canonical `cgb_agb0_boot.bin` or `cgb_agb_boot.bin` uses the AGB-family DMG-compatible `0x2678` bucket and preserves the Ashiepaws `bully.gb` native CGB non-Nintendo `0x1E84` bucket; the native CGB old-licensee `$33` plus binary-zero new-licensee bucket used by Nitro2k01 `whichboot.gb` applies a narrow `24` T-cycle handoff correction at the real `FF50` edge, advancing timer and CGB raster together to the validated cartridge-entry phase (`DIV=$1E` hidden counter `$1E98`, `LY=$90`, line dot `173`) without changing the global CGB power-on seed.
...
- For the current deterministic DMG-family direct-start continuity baseline in this repo, that serial hidden phase is seeded independently to `0xABCC` at `PC = 0x0100`; the later-DMG / MGB `RealBoot` power-on path seeds the serial free-running counter with its own `0x0068` reset offset so the same `0xABCC` phase is reached at the verified `FF50` handoff. It must not be synthesized by reusing the timer's `0xABC8` divider phase because Mooneye's `boot_sclk_align` timing depends on that separation.
...
- For the current deterministic DMG-family `Machine::CustomBoot` reset-facing lane, PPU coherence includes a first-frame hidden Mode `0` STAT IRQ phase in addition to the visible `LY=0` / `STAT=$85` snapshot, so startup HBlank interrupt tests can distinguish readable `IF`, same-cycle interrupt wake, and the ordinary post-startup STAT edge without contaminating ordinary `Machine::SkipBoot` HWIO probes.
- For the same deterministic DMG-family `Machine::CustomBoot` reset-facing lane, PPU coherence also includes a boot-facing CPU-bus publication overlay for the earliest cartridge-entry probes: `FF44`, `FF41`, OAM accessibility, and VRAM accessibility follow the `gbmicrotest poweron_*` delay table while the internal raster remains synthetic `LY=0`; this hidden startup phase must stay separate from both ordinary `Machine::SkipBoot` HWIO snapshots and the verified `BootController::direct_boot_state()` `LY=153` snapshot.
...
- In the current repo baseline, direct boot only guarantees the APU state that is already represented in the centralized startup snapshot and the APU-owned reconstruction path: visible `NRxx` ownership, powered state, wave-RAM startup policy, channel-active reconstruction from `NR52`, direct-start PCM-silence masking for CGB-family boot-owned active channels, and `DIV-APU` seeded from the chosen shared-divider preset.
- The CGB-family direct-start PCM-silence mask is an explicit hidden-state seam: it preserves the published `NR52` active bits and CGB `PCM12`/`PCM34` tap ownership while excluding masked boot-owned channels from the digital taps, DAC conversion, mixer path, recorded channel taps, and host output until software performs a real channel trigger or powers the APU through an ordinary transition.
- Remaining hidden audio state such as HPF capacitor/history, exact pulse duty-step continuation, CH3 sample-buffer/sample-index continuation, and CH4 LFSR/noise-timer continuation is still reconstructed from repo-local defaults rather than from verified boot-handoff evidence; document that gap explicitly instead of calling the whole APU startup path coherent today.
...
- In the current repo baseline, DMG-family `SkipBoot` and `CustomBoot` use an explicit deterministic patterned policy for WRAM and HRAM so continuity tests stay reproducible without claiming those bytes are hardware constants, while both overlay the DMG boot-logo tile and tilemap bytes. CGB and AGB `SkipBoot` use the explicit validated CGB-family handoff memory policy for deterministic comparison against RealBoot: zeroed WRAM, zeroed HRAM with the boot logo prefix, and zeroed VRAM with the boot logo tile prefix, then overlays only the DMG boot-logo tile bytes; CGB and AGB `CustomBoot` preserve that same CGB-family memory base, leaving the logo tilemap clear for CGB framebuffer oracles that do not inspect DMG tilemap residue.
...
- The repo should keep the deterministic synthetic `Machine::SkipBoot` startup state, including its tooling-oriented logo-memory seed, and the verified direct-boot cartridge-entry snapshot explicit and separately documented instead of letting one silently stand in for the other.
...
- tests for CGB and AGB boot-overlay windows at `0000-00FF` and `0200-08FF`, including compact `0x800` and sparse `0x900` images, and tests that `0100-01FF` routes to cartridge/header bytes while boot ROM is still mapped
- tests that AGB `RealBoot` resolves `cgb_agb0_boot.bin` for `CpuAgb0` and `cgb_agb_boot.bin` for `CpuAgbA` through independent loader/fingerprint slots instead of aliasing to CGB-E or native GBA BIOS assets
...
- direct-boot CPU-register tests for `DMG0`, DMG with checksum `0x00`, DMG with checksum not `0x00`, MGB, and AGB native/compatibility CGB headers
...
- The synthetic DMG boot ROM from Phase `2.4` remains a useful narrow integration target inside `gb-core`, but production DMG-family boot closure in this repo depends on explicit local private-firmware RealBoot validation through the real CPU, bus, scheduler, and `FF50` handoff path while keeping invalid DMG logo/checksum/header cases on the non-handoff side.
