...
- For the current later-DMG `RealBoot` path, executing the boot ROM seeds the first LCD enable with an observed hidden dot phase of `92` dots and, at the real `FF50` handoff, arms a handoff-frame Mode `0` STAT seam for `SCX&7 == 3` and `SCX&7 == 7`; keep those as explicit PPU hidden state instead of changing the visible `FF50` mapping contract.
