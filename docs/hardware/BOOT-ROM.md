...
- The DMG-family `Machine::SkipBoot` continuity snapshot uses the same CPU, visible I/O, hidden subsystem state, and plain PPU readback as before, while its memory policy now writes the DMG boot-logo tile stream at even VRAM addresses starting at `$8010` and the logo tilemap at `$9904..$992F`, including the second tilemap row at `$9924..$992F`; this is the fast no-boot logo-memory contract used by DocBoy-like tooling and is not evidence that reset-facing PPU publication belongs in ordinary `SkipBoot`. The DMG-family `Machine::CustomBoot` continuity snapshot starts from the same direct baseline and additionally arms the reset-facing boot PPU CPU-bus publication overlay. The CGB `Machine::SkipBoot` and `Machine::CustomBoot` memory policies keep the CGB direct-start WRAM/HRAM/VRAM base and write the same logo tile stream without the `$9904..$992F` tilemap overlay, matching the CGB no-boot fixture distinction between logo tiles and clear tilemap residue; only `CustomBoot` applies the CGB custom cartridge-entry raster phase.
