...
- [APU][CH4-NR43-LIVE-GLITCH-MATRIX] CH4 live `NR43` writes now reload the explicit noise timer immediately and also keep a hidden noise-counter phase plus `countdown_reloaded` seam state, which lets the DMG-oriented subset model one seam step, a SameBoy-style single-intermediate `old_bit / glitch_bit / new_bit` decision path, some low-shift two-step / feedback-corruption cases, and a small repo-local narrow-staircase Zelda-tail tuning layer. That layer currently suppresses the remaining `0x5C -> 0x4C` mid-band descent, the leading `0x5C -> 0x6C` ascent, and the late `0x4C -> 0x3C` rise, leaving `0x7C -> 0x6C` as the only active narrow return edge in the final clink staircase. The resolver is also split into explicit category-1/category-2/rising-edge/follow-up paths, and those categories now resolve through an explicit LFSR-action seam (`none`, `plain step`, `forced-short step`, `forced-short step + low-shift corruption`) recorded in the CH4 trace/debug snapshot together with the chosen `glitch_value`, a repo-local staged-upper-nibble **second intermediate candidate** for bit7-stable writes, and their classifier bits. That richer trace is now the intended oracle for deciding whether a true two-intermediate behavior change is warranted, because SameBoy still models a much richer revision-specific immediate LFSR glitch matrix, including additional intermediate values, broader corruption patterns, and more precise reload-timing cases. Keep that deeper behavior deferred until the core grows stronger hardware-backed evidence than the current DMG/CGB-C-oriented subset.
