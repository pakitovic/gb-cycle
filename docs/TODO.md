...
- [CARTRIDGE][HUC1-ORACLE-HOST-IR] `HuC1` now has a real runtime baseline (dedicated classification, RAM-vs-IR mode switching, ROM/RAM banking, IR register semantics, and its own persistent RAM payload), but the current implementation still hardwires the IR sensor to the documented no-light readback unless tests mutate the internal state directly. Do not call point `4` fully closed until one commercial HuC1 title has been validated end to end and the cartridge/device boundary grows an explicit host-side light-input seam for GB Kiss behavior.
