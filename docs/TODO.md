...
- [APU][FAST-TIMER-INACTIVE-GATING] CH1 / CH2 / CH3 / CH4 currently gate their fast waveform/sample/noise timers on `channel_active`. Pan Docs keeps those timers conceptually separate from the frame sequencer and from DAC/output audibility, so do not treat the current inactive-channel pause behavior as hardware-proven DMG behavior until a stronger oracle or hardware-backed model confirms whether phase/timer progression should continue while a channel is logically inactive or silent.
