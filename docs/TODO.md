...
- [BOOT][CGB-DIRECT-DIV-PREDICTOR] CGB direct-start currently has validated header-aware timer buckets for missing/DMG-compatible headers (`0x2674`, locked by Mooneye `boot_div-cgbABCDE`), native CGB non-Nintendo old-licensee headers (`0x1E84`, locked by Ashiepaws `bully.gb` and gb-cycle's standard `cgb_boot.bin` handoff observation), and native CGB old-licensee `$33` headers with binary-zero new-licensee bytes (`0x1E98`, locked by Nitro2k01 `whichboot.gb` plus canonical `cgb_boot.bin` RealBoot with the core handoff correction). A full DocBoy-style CGB direct-start DIV predictor for other Nintendo, new-licensee, and DMG-compatibility checksum-table buckets remains deferred; re-entry should start from generated RealBoot measurements with canonical `cgb_boot.bin`, not by adding ROM-specific runner timer overrides or blindly copying DocBoy's phase constants.
