...
- **Highest-value next step:** keep `daid/ppu_scanline_bgp.gb` as the active early raster gate, but use testcase `1` of `intr_2_mode0_timing_sprites.gb` as the active mooneye oracle. The exact copied ROM-path probe now matches the real `STAT` arm at `ly = 67`, `line_dot = 136`, `pc = 0x0BFD`, so the next useful move is a narrow post-arm probe, not another pre-arm setup retune or a return to the older testcase-`13` hidden-lane probes.
...
- [PPU][MOONEYE-STAT-TIMING] `hblank_ly_scx_timing-GS`, `intr_2_0_timing`, `vblank_stat_intr-GS` are now green. Remaining open case: `ppu/intr_2_mode0_timing_sprites` — sprite-coupled Mode `2 -> 0` timing is not fully closed.
  - Current re-entry state for `intr_2_mode0_timing_sprites.gb`:
    ...
    - the current full-ROM failure signature is back at testcase `1`, not testcase `13`: `real_mooneye_intr_2_mode0_timing_sprites_logs_failure_testcase_id` now reports `failure_testcase_id = 1`, `mode0_start_dot = 252`, `selected_sprites_len = 0`, and `visible_pixels_output = 157`.
    - the exact copied ROM-path probe for testcase `1` now matches the real setup seam through the `STAT` arm itself: it arms at `ly = 67`, `line_dot = 136`, `mode = Drawing`, `pc = 0x0BFD`, which closes the earlier `16`-dot mismatch before `FF41 = 0x20`.
    - the remaining local mismatch is now strictly post-arm: the copied ROM-path probe still does not reach the first `FF41` read inside the short post-arm window, so the next gap is in the IRQ/return/caller path after arming `STAT`, not in the pre-arm setup writes.
  - Do not retry these two directions first:
    ...
    - do not resume from the older testcase-`13` hidden-lane probes first; they are now superseded by the exact testcase-`1` ROM-path oracle.
  - Highest-value next step from this checkpoint:
    - keep testcase `1` as the active oracle and add a narrower post-arm probe on top of the copied ROM-path setup so it captures the first `FF41` read or the return path immediately after the exact `STAT` arm. The next useful move is to reproduce the post-arm caller/IRQ seam, not to retune pre-arm setup timing or reopen hidden-lane testcase-`13` work first.
...
- Treat cached background slices already in `Push`, `fill.pending`, or the visible FIFO as a likely suspect only for the remaining live-write families. Do not let that heuristic override a better active oracle such as the current post-arm `case1` seam in `intr_2_mode0_timing_sprites.gb`.
