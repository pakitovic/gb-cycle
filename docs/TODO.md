...
- [APU][HOST-RESAMPLER-BANDLIMITED] The host-facing capture path now does interval-accurate downsampling of the post-HPF T-cycle signal instead of picking the last T-cycle value, which removes the worst zero-order-hold aliasing from the current desktop output. However, SameBoy still does higher-quality edge-aware band-limited synthesis in its host audio path, and high-pitched commercial-game content may remain harsher here until the explicit core-to-host resampler grows a comparable band-limited mode without feeding host sample-rate concerns back into core APU timing.
