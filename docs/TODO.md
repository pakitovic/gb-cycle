...
- [APU][CH4-NR43-LIVE-GLITCH-MATRIX] CH4 live `NR43` writes now reload the explicit noise timer immediately and also keep a hidden noise-counter phase plus `countdown_reloaded` seam state, which lets the DMG-oriented subset model one seam step, an `old -> 0xFF -> new` intermediate-value path, some low-shift two-step / feedback-corruption cases, and a repo-local suppression guard for the high-shift narrow `old -> 0xFF` category-2 step that over-drove Zelda's final tail in the simpler model. However, SameBoy still models a much richer revision-specific immediate LFSR glitch matrix, including additional intermediate values, broader corruption patterns, and more precise reload-timing cases; keep that deeper behavior deferred until the core grows a stronger hardware-backed oracle than the current subset.
