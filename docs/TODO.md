...
- [TESTING][LINKED-SESSION-MARKDOWN-REPORT] `run-docboy` now executes the DocBoy serial-link rows through `run_linked_session` and retains participant failure artifacts, but linked-session outcomes are still CLI-output-only and do not update `.roms/test/test-report-extra.md`; if linked-session rows need aggregate report persistence, design participant-aware markdown status rows instead of flattening them into single-machine ROM rows.
...
- [BOOT][CGB-DIRECT-DIV-PREDICTOR] CGB direct-start currently has validated header-aware timer buckets for missing/DMG-compatible headers (`0x2674`, locked by Mooneye `boot_div-cgbABCDE`) and native CGB non-Nintendo old-licensee headers (`0x1E84`, locked by Hacktix `bully.gb` and gb-cycle's standard `cgb_boot.bin` handoff observation). A full DocBoy-style CGB direct-start DIV predictor for Nintendo, new-licensee, and DMG-compatibility checksum-table buckets remains deferred; re-entry should start from generated RealBoot measurements with canonical `cgb_boot.bin`, not by adding ROM-specific runner timer overrides or blindly copying DocBoy's phase constants.
