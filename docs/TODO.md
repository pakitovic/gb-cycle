...
- [REWIND][COMPRESSION-MULTI] Phase 8.7 keeps desktop rewind single-machine, full-snapshot-only, and in-memory while adding persistent capture/capacity/playback-speed settings plus HUD telemetry backed by core deep-size payload accounting. Multi-machine coordination, CLI rewind, compression, and delta encoding remain future work; use `MachineRewindStats.estimated_bytes` as accounted snapshot payload bytes (not RSS) before choosing a compression/delta strategy.
