...
- [SAVESTATE][PHASE8-SCOPE] Phase 8.5 exposes single-machine `.gbstate` I/O in CLI and desktop slots. Save states for linked `DMG-04` 2-player Game Link and `DMG-07` 4-Player Adapter sessions are intentionally unsupported and are not a Phase 8 follow-up. Manual `.gbstate` file dialogs and persisted desktop slot selection remain optional host UX work, not save-state correctness blockers.
- [REWIND][PHASE8-SCOPE] Phase 8.7 keeps desktop rewind single-machine, full-snapshot-only, and in-memory while adding persistent capture/capacity/playback-speed settings plus HUD telemetry backed by core deep-size payload accounting. CLI rewind is intentionally not planned because manual CLI rewind is not a target workflow and rewind behavior is covered by core/desktop tests. Linked-session rewind for `DMG-04` 2-player Game Link and `DMG-07` 4-Player Adapter is also intentionally unsupported. Compression and delta encoding remain optional future optimizations; use `MachineRewindStats.estimated_bytes` as accounted snapshot payload bytes (not RSS) before choosing a compression/delta strategy.
