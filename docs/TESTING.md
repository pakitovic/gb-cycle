...
- Keep local boot ROM images under the repo-managed gitignored `/.roms/bootrom/` store, using the canonical filenames from `gb-core` (`dmg0_boot.bin`, `dmg_boot.bin`, `mgb_boot.bin`, and the default CGB `cgb_boot.bin`) so local real-boot runs do not depend on ad hoc per-machine paths.
