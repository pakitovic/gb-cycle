...
- Curated family runs should update `/.roms/test/test-report.md` with a simple per-ROM status table so repo-managed `PASS` / `FAIL` / `INFO` state stays visible without re-reading logs; extra/internal suites render to `/.roms/test/test-report-extra.md` instead, currently `ax6-dmg-extra`, `samesuite-dmg-extra`, and `cgb-boot-hwio`, so exploratory rows do not pollute the promoted aggregate report. Both markdown views should render those states as `✅`, `❌`, and `ℹ️` rather than repeating the raw persisted strings.
...
- Within each populated family, rows should follow the pinned GBEmulatorShootout source order from `crates/gb-test-runner/data/sources.toml`, with same-ROM model variants ordered DMG before GBC and manifest order used only as the fallback for cases without a pinned source path.
...
- The local `make test-roms-real-boot` lane runs the same DMG curated suites as `make test-roms` by applying `GB_CYCLE_TEST_ROM_STARTUP=real-boot` to the normal `run-*` targets, requires `GB_CYCLE_BOOT_ROM_ROOT`, starts each case timeout after the `FF50` handoff, clears synthetic `SkipBoot` startup-memory profiles, and writes the same `.roms/test/test-report.md` rows as the SkipBoot lane; rerun `make test-roms` to restore the default SkipBoot report signal.
- The local `make test-roms-extra` lane currently runs the extra/internal `ax6-dmg-extra` and `samesuite-dmg-extra` suites and writes their rows to `.roms/test/test-report-extra.md`; `make test-roms-extra-real-boot` uses the same `GB_CYCLE_BOOT_ROM_ROOT` and `GB_CYCLE_TEST_ROM_STARTUP=real-boot` RealBoot contract for extra DMG comparison evidence without adding those rows to `.roms/test/test-report.md`.
...
- The `samesuite-dmg-extra` manifest runs SameSuite APU `div_write_trigger.gb` and `div_write_trigger_10.gb` on the DMG model with `report_model_suffix = true` and committed DMG grayscale fixtures `crates/gb-test-runner/data/fixtures/samesuite/apu/div_write_trigger*.dmg.png`; the upstream SameSuite PNG references remain the CGB RGB555 oracle fixtures for `cgb-audio-samesuite`.
- The local `make test-roms-cgb-real-boot` lane runs the promoted-green CGB aggregate through strict canonical `cgb_boot.bin` `RealBoot`, requires `GB_CYCLE_BOOT_ROM_ROOT`, applies `GB_CYCLE_TEST_ROM_STARTUP=real-boot` through the `run_rom_suite` CLI for CGB suites that normally default to `SkipBoot`, and clears synthetic startup-memory and startup-timer profiles so the executed boot firmware owns the handoff state; rerun `make test-roms-cgb` to restore the default CGB aggregate report signal.
...
- For direct `run_rom_suite` invocations, leaving `GB_CYCLE_TEST_ROM_STARTUP` unset preserves each manifest's declared startup mode, `GB_CYCLE_TEST_ROM_STARTUP=skip-boot` explicitly forces `SkipBoot` even for manifests that default to `RealBoot`, and `GB_CYCLE_TEST_ROM_STARTUP=real-boot` explicitly forces verified `RealBoot`.
...
- Phase `10` starts with one manifest-backed exploratory CGB catalog suite under `make run-cgb-smoke`; it runs `mooneye/misc/boot_regs-cgb.gb` and Acid `which.gb` on `ConsoleModel::GameBoyColor`, uses the default centralized CGB `SkipBoot` handoff, uses `mooneye-result` plus informational framebuffer capture, and stays outside the DMG `make test-roms` and GitHub `test-roms` workflow until the suite is promoted intentionally.
...
- Phase `10` Slice `7B` adds the manifest-backed `cgb-audio-samesuite` suite under `make run-cgb-audio-samesuite`; it materializes upstream SameSuite APU ROMs and PNG fixtures from `GBEmulatorShootout`, runs all `61` rows on `ConsoleModel::GameBoyColor` with blocking `framebuffer-rgb555-fixture` oracles, and joins both `make test-roms-cgb` and the local boot-ROM-backed `make test-roms-cgb-real-boot` aggregate now that the shared APU primitives are green enough for the target to be closure evidence.
...
- The GitHub `test-roms` workflow currently runs the same workflow-managed non-CGB DMG families as the local `make test-roms` aggregator: `acid`, the three Blargg chunks `blargg-cpu-instrs`, `blargg-dmg-sound`, and `blargg-timing-memory-oam` covering `cpu_instrs 01..11`, `halt_bug`, `instr_timing`, `mem_timing 01..03`, `mem_timing-2 01..03`, `oam_bug 1..6,8`, and `dmg_sound 01..12`, `daid`, `hacktix`, `cpp`, the three Mooneye chunks `mooneye-acceptance`, `mooneye-mbc1-mbc5`, and `mooneye-mbc2`, and `mealybug-tearoom-tests`.
