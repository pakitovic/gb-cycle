...
- Phase `10` Slice `2` adds the manifest-backed `cgb-speed` suite under `make run-cgb-speed`; it runs Daid `STOP` / speed-switch timing ROMs plus Blargg `interrupt_time.gb` on `ConsoleModel::GameBoyColor`. Daid `stop_instr.gb (GBC)` is a blocking final `framebuffer-grayscale-fixture` against `crates/gb-test-runner/data/fixtures/daid/stop_instr.gbc.png` so the expected solid black STOP screen is compared as absolute grayscale, while `stop_instr_gbc_mode3.gb` is a blocking rank-normalized framebuffer fixture against `crates/gb-test-runner/data/fixtures/daid/stop_instr_gbc_mode3.png` so the CGB Mode `3` STOP PASS text remains visible; `speed_switch_timing_div.gbc` and `speed_switch_timing_ly.gbc` are blocking rank-normalized framebuffer fixtures against `crates/gb-test-runner/data/fixtures/daid/speed_switch_timing_div.png` and `crates/gb-test-runner/data/fixtures/daid/speed_switch_timing_ly.png`; Blargg `interrupt_time.gb` is a blocking BG-map console-text oracle that expects `Passed` after the CGB run reaches the ROM-declared screen console result. The remaining cases retain informational framebuffer artifacts, so the suite must not be counted as a strict CGB speed closure signal until all targeted cases have blocking trace/framebuffer/serial or ROM-declared output oracles.
