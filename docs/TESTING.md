...
- Phase `10` Slice `7B` adds the standalone manifest-backed `cgb-audio-samesuite` suite under `make run-cgb-audio-samesuite`; it materializes upstream SameSuite APU ROMs and PNG fixtures from `GBEmulatorShootout`, runs all `61` rows on `ConsoleModel::GameBoyColor` with blocking `framebuffer-rgb555-fixture` oracles, and intentionally stays outside `make test-roms-cgb` until the advanced shared APU primitives are green enough for this red exploratory target to become closure evidence.
