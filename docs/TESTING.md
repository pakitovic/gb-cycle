...
Automated determinism and save/load continuation coverage lives in cargo tests close to the core and runner code; there is no standalone determinism runner. Those tests should compare independent replays, final `MachineSaveState` payloads, serial output, mid-run save/restore continuation, and rejection of incompatible restore metadata.
