...
- For the current DMG-family and default CGB stores, `gb-test-runner` treats those boot ROM assets as pinned local inputs rather than arbitrary filenames: strict-mode `RealBoot` verifies the observed SHA-256 against the expected `dmg0/dmg/mgb/cgb` hashes before execution so local bring-up does not silently proceed on missing or wrong firmware bytes. Revision-only CGB assets such as `cgb0_boot.bin` and `cgbE_boot.bin` remain future local validation inputs rather than required repo gates.
- The local `make test-roms-real-boot` lane runs the same DMG curated suites as `make test-roms` through clean `RealBoot`, requires `GB_CYCLE_BOOT_ROM_ROOT`, starts each case timeout after the `FF50` handoff, and writes the same `.roms/test/test-report.md` rows as the SkipBoot lane; rerun `make test-roms` to restore the default SkipBoot report signal.
...
- Keep the ignored real-boot regression matrix in `crates/gb-test-runner/tests/external.rs`, with verified `dmg0`, `dmg`, and `mgb` valid-handoff cases, a default `cgb` handoff smoke, plus DMG invalid-logo, invalid-checksum, and FF-filled-header no-handoff cases. These tests are optional local evidence because the firmware dumps are non-redistributable; the DMG-family valid path should assert the real `FF50` handoff, compare the resulting cartridge-entry state against the centralized direct-boot snapshot (`BootController::direct_boot_state()`) for the same model, and emit one exact serial fingerprint artifact so failures leave a legible entry-state record without requiring firmware dumps in CI, while the CGB smoke is intentionally limited to verified asset loading plus the `FF50` handoff until Slice 6 owns CGB RealBoot equivalence.
