...
- Phase `10` Slice `4` starts the manifest-backed `cgb-ppu-basic` promotion under `make run-cgb-ppu-basic`; its first blocking row is SameSuite `ppu/blocking_bgpi_increase.gb` on `ConsoleModel::GameBoyColor` with a rank-normalized framebuffer fixture at `crates/gb-test-runner/data/fixtures/samesuite/ppu/blocking_bgpi_increase.png`, so the gate begins with the palette-MMIO blocking probe before later Daid, Acid, and Hacktix visual rows are promoted.
