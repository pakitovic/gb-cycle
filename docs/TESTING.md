...
Include MBC5 tests for RAM-enable behavior, disabled-RAM open-bus reads plus ignored writes, linear `8 KiB` bank selection for standard `8 KiB`, `32 KiB`, `64 KiB`, and `128 KiB` SRAM configurations, the absence of any MBC1-style dual banking mode, and the rule that header variants without RAM do not expose fake SRAM semantics merely because the RAM-bank register exists.
Include rumble-capable MBC5 tests that prove `bit 3` of the control register in `0x4000-0x5FFF` updates observable `rumble_on`, that the state remains active until software clears it, that rumble handling stays distinct from effective RAM-bank selection rather than collapsing both meanings into one opaque integer, and that rumble-capable variants can still reach all effective RAM banks for accepted `8 KiB`, `32 KiB`, and `64 KiB` SRAM declarations.
Include MBC5 validation tests for ROM sizes above `8 MiB`, impossible RAM declarations, no-RAM header variants with nonzero `0x0149`, and the failure case where a rumble-capable header requests `128 KiB` SRAM even though `bit 3` is reserved for the motor.
...
Include `MBC3` persistence tests that serialize live RTC state plus elapsed-time bookkeeping, restore powered-off advancement from an injected deterministic clock, do not confuse the latched RTC snapshot with the persistent live clock, and clear any stale runtime-local latch snapshot after a hot persistence restore.
