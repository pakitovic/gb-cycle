...
- The local `make test-roms-real-boot` lane runs the same DMG curated suites as `make test-roms` through clean `RealBoot`, requires `GB_CYCLE_BOOT_ROM_ROOT`, starts each case timeout after the `FF50` handoff, clears synthetic `SkipBoot` startup-memory profiles through `GB_CYCLE_TEST_ROM_STARTUP=real-boot`, and writes the same `.roms/test/test-report.md` rows as the SkipBoot lane; rerun `make test-roms` to restore the default SkipBoot report signal.
...
- Slice `2` also wires the manifest-backed `cgb-boot-div` suite under `make run-cgb-boot-div`; it materializes `mooneye/misc/boot_div-cgbABCDE.gb`, runs it on `ConsoleModel::GameBoyColor`, and uses the blocking `mooneye-result` oracle. This keeps the CGB boot/DIV timing check explicit in Slice `2` while the deeper `RealBoot` equivalence work remains owned by Slice `6`.
