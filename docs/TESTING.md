...
- After that single coverage sweep, the GitHub workflow may run the per-crate report-only `cargo cov-check-*` gates concurrently, but each gate must keep a separate log and exit status so failures stay attributable to one crate.
