...
- `Cartridge`: repo gate present for the primary mapper-oracle slice. Current evidence: unit and integration coverage for `NoMbc`, `Mbc1`, `Mbc2`, `Mbc3`, `Mbc30`, `Mbc5`, hardware-style persistence, the built-in `phase-6-cartridge-oracle` suite, LibSameBoy `case-bundle` differential artifacts locally materialized and compared green on April 27, 2026, the Phase `9` save/load determinism smoke covering that suite, and the Phase `10` Slice `8` CGB `cgb-rtc` lane plus focused CGB-mode mapper regressions for MBC1/MBC2/MBC3/MBC5.
