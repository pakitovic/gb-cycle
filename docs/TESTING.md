...
- A manifest case should set `report_model_suffix = true` only when the GBEmulatorShootout row label itself carries a model suffix such as `(DMG)` or `(GBC)`; `console = "dmg"` or `console = "cgb"` selects the runner model, while `report_model_suffix` controls whether that model appears in `.roms/test/test-report.md`.
