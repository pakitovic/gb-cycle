...
- After that single coverage sweep, the GitHub workflow may run the per-crate report-only `cargo cov-check-*` gates concurrently only after one serialized report has materialized cargo-llvm-cov's shared `.profdata`; each gate must keep a separate log and exit status so failures stay attributable to one crate without corrupting the shared coverage profile.
