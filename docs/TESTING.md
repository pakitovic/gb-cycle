...
- `Cartridge`: repo gate present for the primary mapper-oracle slice. Current evidence: unit and integration coverage for `NoMbc`, `Mbc1`, `Mbc2`, `Mbc3`, `Mbc30`, `Mbc5`, `Mbc6`, hardware-style persistence, the built-in `phase-6-cartridge-oracle` suite, LibSameBoy `case-bundle` differential artifacts locally materialized and compared green on April 27, 2026, the Phase `9` save/load determinism smoke covering that suite, and the Phase `10` Slice `8` CGB `cgb-rtc` lane plus focused CGB-mode mapper regressions for MBC1/MBC2/MBC3/MBC5.
...
- The current built-in cartridge lane for that differential path is `phase-6-cartridge-oracle`, which reuses the retained synthetic Phase `6` `MBC1`, `MBC2`, `MBC3`, and `MBC5` ROM fixtures under a stable `TestSubsystem::Cartridge` suite contract. That lane uses the `case-bundle` layout because the `MBC3` case needs explicit pre-run RTC advancement and the relevant compared artifact is a portable `serial_hex.txt` payload; MBC6 currently has focused unit/persistence coverage because its writable flash behavior needs a dedicated synthetic or commercial oracle before joining the shared differential lane.
