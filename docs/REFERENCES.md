...
- GBEmulatorShootout/DocBoy-sourced family names actively used by manifests include `acid`, `ashiepaws`, `ax6`, `blargg`, `cpp`, `daid`, `gbmicrotest`, `little-things-gb`, `magen`, `mealybug-tearoom-tests`, `mooneye`, `samesuite`, `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext`; pinned source revisions and file hashes for the promoted GB Emulator Shootout report live in `crates/gb-test-runner/data/gb-emulator-shootout/sources.toml`, DocBoy single-machine rows use `crates/gb-test-runner/data/docboy/sources.toml`, gbmicrotest uses `crates/gb-test-runner/data/gbmicrotest/sources.toml`, and remaining legacy extra/internal lanes use the pruned legacy-only inventory in `crates/gb-test-runner/data/sources.toml`. The report fetch metadata mirrors only the non-legacy report source pins through `crates/gb-test-runner/data/reports.toml` and report-local `sources.report.toml` files so future report metadata can be validated before the stable fetch path changes.
- [CasualPokePlayer/GBEmulatorShootout](https://github.com/CasualPokePlayer/GBEmulatorShootout) and [CasualPokePlayer/test-roms](https://github.com/CasualPokePlayer/test-roms) — temporary SGB fixture/source references for SameSuite SGB and cpp SGB rows until the normal upstream materialization path owns them.
