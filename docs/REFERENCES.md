...
- [c-sp/game-boy-test-roms](https://github.com/c-sp/game-boy-test-roms) — release-ZIP aggregate used for the archive-backed Wilbertpol Mooneye-derived ROM channel, including independently pinned ROM and fixture hashes.
...
- GBEmulatorShootout/DocBoy/c-sp-sourced family names actively used by manifests include `acid`, `ashiepaws`, `ax6`, `blargg`, `cpp`, `daid`, `gbmicrotest`, `little-things-gb`, `magen`, `mealybug-tearoom-tests`, `mooneye`, `samesuite`, `wilbertpol`, `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext`; pinned source revisions or archive hashes plus file hashes for report-local cargo lanes live in `crates/gb-test-runner/data/reports.toml` plus each report-local `sources.report.toml`; linked-session manifests use the report-local `*.link.suite.toml` model and share the same report registry.
