...
- GBEmulatorShootout/DocBoy-sourced family names actively used by manifests include `acid`, `ashiepaws`, `ax6`, `blargg`, `cpp`, `daid`, `gbmicrotest`, `little-things-gb`, `magen`, `mealybug-tearoom-tests`, `mooneye`, `samesuite`, `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext`; pinned source revisions and file hashes for report-local cargo lanes live in `crates/gb-test-runner/data/reports.toml` plus each report-local `sources.report.toml`; the legacy root inventory in `crates/gb-test-runner/data/sources.toml` remains only for old manifest-model fetches and currently-unpromoted linked assets.
