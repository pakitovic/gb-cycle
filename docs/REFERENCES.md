...
- GBEmulatorShootout/DocBoy-sourced family names actively used by manifests include `acid`, `ashiepaws`, `ax6`, `blargg`, `cpp`, `daid`, `gbmicrotest`, `little-things-gb`, `magen`, `mealybug-tearoom-tests`, `mooneye`, `samesuite`, `docboy-dmg`, `docboy-cgb`, `docboy-cgb-dmg`, and `docboy-cgb-dmg-ext`; pinned source revisions and file hashes for the promoted GB Emulator Shootout report live in `crates/gb-test-runner/data/gb-emulator-shootout/sources.toml`, while legacy extra/DocBoy lanes use the pruned legacy-only inventory in `crates/gb-test-runner/data/sources.toml`.
