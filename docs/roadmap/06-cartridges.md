...
4. Enforce controlled handling for documented special hardware.
   ...
   Status: done in the current branch baseline. `HuC1`, `HuC-3`, `M161`, and `Pocket Camera` now load through their own dedicated supported families, while `MBC6`, `MBC7`, `Bandai TAMA5`, and generic `UnknownCode` values still fail with explicit typed diagnostics and without silent fallback to nearby supported mappers.
