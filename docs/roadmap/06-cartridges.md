...
10. `MBC6` runtime support — done in the current branch baseline as a dedicated cartridge-local device; see `docs/hardware/CARTRIDGES-MBC.md`

The purpose of that split is to close the remaining DMG-relevant special-cartridge work first while keeping CGB-only specials explicit, typed, and impossible to misclassify. `MBC30` now has functional runtime support after the base CGB gate, and `MBC6` now has the dedicated split-window SRAM/ROM/flash runtime and typed persistence needed to avoid a misleading fallback mapper. `MBC7` may grow diagnostics, validation hooks, persistence shapes, or device skeletons, but it must not be counted as a functionally supported runtime target until a dedicated implementation exists.
