...
4. Enforce controlled handling for documented special hardware.
   ...
   Status: done in the current branch baseline. `HuC1`, `HuC-3`, `M161`, `Pocket Camera`, and `MBC6` now load through their own dedicated supported families, while `MBC7`, `Bandai TAMA5`, and generic `UnknownCode` values still fail with explicit typed diagnostics and without silent fallback to nearby supported mappers.
...
The purpose of that split is to close the remaining DMG-relevant special-cartridge work first while keeping CGB-only specials explicit, typed, and impossible to misclassify. `MBC30` now has functional runtime support after the base CGB gate, and `MBC6` now has the dedicated split-window SRAM/ROM/flash runtime, typed persistence, a repo-owned synthetic MBC6 oracle, and manual commercial validation needed to avoid a misleading fallback mapper. `MBC7` may grow diagnostics, validation hooks, persistence shapes, or device skeletons, but it must not be counted as a functionally supported runtime target until a dedicated implementation exists.
