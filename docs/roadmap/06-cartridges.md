...
8. `MBC30` runtime support — done after point `7`; see `docs/hardware/CGB.md` and `docs/hardware/CARTRIDGES-MBC.md`
9. `MBC7` runtime support — done after point `7` with focused sensor / EEPROM coverage and no invented runtime rumble; see `docs/hardware/CGB.md` and `docs/hardware/CARTRIDGES-MBC.md`
10. `MBC6` runtime support — done in the current branch baseline as a dedicated cartridge-local device with focused unit/persistence coverage, the synthetic `phase-6-mbc6-oracle`, and manual commercial smoke validation of `Net de Get: Minigame @ 100`; see `docs/hardware/CARTRIDGES-MBC.md`

The purpose of that split is to close the remaining DMG-relevant special-cartridge work first while keeping CGB-only specials explicit, typed, and impossible to misclassify. `MBC30` now has functional runtime support after the base CGB gate, `MBC6` now has the dedicated split-window SRAM/ROM/flash runtime, typed persistence, a repo-owned synthetic MBC6 oracle, and manual commercial validation needed to avoid a misleading fallback mapper, and `MBC7` now has focused runtime support for ROM banking, accelerometer latching, serial EEPROM, persistence, save-state, and bus descriptors while keeping runtime rumble disabled until hardware evidence identifies a control path.
