...
4. Enforce controlled handling for documented special hardware.
   ...
   Status: done in the current branch baseline. `HuC1`, `HuC-3`, `M161`, `Pocket Camera`, and `MBC7` now load through their own dedicated supported families, while `MBC6`, `Bandai TAMA5`, and generic `UnknownCode` values still fail with explicit typed diagnostics and without silent fallback to nearby supported mappers. MBC7 keeps the official `$22` header name `MBC7+SENSOR+RUMBLE+RAM+BATTERY` but exposes no runtime rumble control until hardware evidence identifies one.
...
9. `MBC7` runtime support — done after point `7`; see `docs/hardware/CGB.md` and `docs/hardware/CARTRIDGES-MBC.md`
10. `MBC6` runtime support — after point `7`; see `docs/hardware/CGB.md` and `docs/hardware/CARTRIDGES-MBC.md`

The purpose of that split is to close the remaining DMG-relevant special-cartridge work first while keeping CGB-only specials explicit, typed, and impossible to misclassify. `MBC30` now has functional runtime support after the base CGB gate; `MBC7` now has focused runtime support for ROM banking, accelerometer latching, serial EEPROM, persistence, save-state, and bus descriptors; `MBC6` may grow diagnostics, validation hooks, persistence shapes, or device skeletons, but it must not be counted as a functionally supported runtime target until a dedicated implementation exists.
