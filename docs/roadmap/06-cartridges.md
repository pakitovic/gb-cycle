...
2. Implement standard MBC3 ROM and RAM banking.
   Scope: fixed low ROM bank `0`, switchable high ROM bank `0x01..=0x7F`, raw `7`-bit ROM-bank register, real-size masking, standard external-RAM banking up to `32 KiB`, and explicit MBC30 variant separation.
   Acceptance criteria: MBC3 supports up to `2 MiB` ROM, the switchable region honors raw `0 -> 1` while still masking by real ROM size, banks `0x20`, `0x40`, and `0x60` are reachable unlike MBC1, RAM banking is masked by real RAM size, and `64 KiB` SRAM configurations route to the explicit MBC30 variant instead of being treated as standard MBC3.
   Status: done in the current branch baseline. Standard `MBC3` now supports ROM banking up to `2 MiB`, keeps banks `0x20`, `0x40`, and `0x60` reachable, masks RAM banking by real size up to `32 KiB`, and keeps MBC30-like `64 KiB` SRAM declarations out of ordinary MBC3.
...
6. Close with dedicated MBC3 tests and validation follow-up.
   Scope: header-type coverage, RAM-versus-RTC selector behavior, latch sequencing, halt/carry/day overflow, stable snapshots, optional fine-delay research, and explicit MBC30 variant tracking.
   ...
   Status: done in the current branch baseline. Unit tests, integration tests, and one retained synthetic Phase `6` ROM now cover header types `0x0F`, `0x10`, `0x11`, `0x12`, and `0x13`, raw ROM-bank `0 -> 1`, banks `0x20`, `0x40`, and `0x60`, RAM-versus-RTC selector behavior, latch sequencing, stable snapshots, halt / carry / day overflow, and MBC30 variant diagnostics. The built-in `phase-6-cartridge-oracle` suite now carries this `MBC3` case with explicit pre-run RTC advancement through typed runner metadata, and historical SameBoy comparison recorded matched portable `serial_hex` output for the selected banking and RTC observables. The `16`-T-cycle / `4 us` RTC access-spacing recommendation is now represented explicitly as timed cartridge state (`rtc_access_ready_at`) on RTC-register accesses, but still remains advisory rather than enforced behavior pending stronger validation. Current compatibility notes: the curated external `cpp/latch-rtc-test.gb` framebuffer oracle from `GBEmulatorShootout` stays green under a deliberate legacy relatch rule that accepts follow-up non-zero writes after one valid RTC snapshot exists, even though the stricter `Pan Docs` reading would require re-arming with `0x00`. The same curated source also keeps `cpp/rtc-invalid-banks-test.gb` green only while standard MBC3 selector values `0x04..=0x07` remain explicit reserved states rather than widened SRAM banks, despite the broader `$00-$07` wording in current `Pan Docs`; MBC30 gives those selectors the separate extended-SRAM-bank meaning.
...
6. Close with dedicated MBC5 tests and oracle comparisons.
   ...
   Status: done in the current branch baseline. Unit tests, integration tests, and one retained synthetic Phase `6` ROM now cover header types `0x19..=0x1E`, bank `0` visibility in the switchable region, bank `0x1FF`, the `0xFF -> 0x100` boundary, RAM-enable behavior, SRAM behavior for `32 KiB` and `128 KiB`, RAM-bank masking, rumble on/off, and size-validation diagnostics. The built-in `phase-6-cartridge-oracle` suite now includes the retained `MBC5` rumble/banking case under the shared cartridge oracle lane, and historical SameBoy comparison recorded matched portable `serial_hex` output for the selected bank-selection and rumble observables.
...
2. Add explicit `MBC30` detection.
   Scope: detect the `MBC3`-family plus `64 KiB` SRAM case as `MBC30`, return a typed supported variant entry point instead of ordinary standard `MBC3`, and route matching persistence / banking through the MBC30 variant.
   Acceptance criteria: `MBC3 + 64 KiB SRAM` never falls through to standard `MBC3`, loader diagnostics name `MBC30` explicitly, and runtime ROM banking, RAM banking, RTC behavior, and persistence all use a concrete `MBC30` implementation path.
   Status: done in the current branch baseline. `MBC3`-family headers with `64 KiB` SRAM now classify and load as the explicit supported `MBC30` variant, preserving standard MBC3 behavior while enabling MBC30 `4 MiB` ROM / `64 KiB` SRAM banking and persistence.
3. Add multicart and near-variant classification.
   ...
   Status: done in the current branch baseline. Header-coded `MMM01` variants classify as supported multicart hardware, standard `MBC1` still reserves a distinct internal future `MBC1M` variant instead of merging those rules into ordinary `MBC1`, and the known Mani `4-in-1` multicart now classifies explicitly as supported `M161` hardware through a deliberate signature path instead of falling through to ordinary `NoMbc` or generic unknown handling.
4. Enforce controlled handling for documented special hardware.
   Scope: explicit diagnostics and dedicated paths for `HuC1`, `HuC-3`, `M161`, `Pocket Camera`, `MBC6`, `MBC7`, and `Bandai TAMA5`, plus a hard rule against automatic fallback to `MBC1`, `MBC3`, `MBC5`, or other nearby supported mappers.
   Acceptance criteria: supported special types enter their dedicated hardware paths or fail explicitly on malformed images, unsupported special types fail with clear messages naming the exact detected cartridge, `UnknownCode` reports the raw `0x0147` byte, and no silent degradations or fake "best effort" mapper substitutions remain.
   Status: done in the current branch baseline. `HuC1`, `HuC-3`, `M161`, `Pocket Camera`, `MBC6`, and `MBC7` now load through their own dedicated supported families, while `Bandai TAMA5` and generic `UnknownCode` values still fail with explicit typed diagnostics and without silent fallback to nearby supported mappers. MBC7 keeps the official `$22` header name `MBC7+SENSOR+RUMBLE+RAM+BATTERY` but exposes no runtime rumble control until hardware evidence identifies one.
...
7. base CGB bring-up gate — outside Phase `6`; see [`docs/hardware/CGB.md`](../hardware/CGB.md)
8. `MBC30` runtime support — done after point `7`; see [`docs/hardware/CGB.md`](../hardware/CGB.md) and [`docs/hardware/CARTRIDGES-MBC.md`](../hardware/CARTRIDGES-MBC.md)
9. `MBC7` runtime support — done after point `7` with focused sensor / EEPROM coverage and no invented runtime rumble; see [`docs/hardware/CGB.md`](../hardware/CGB.md) and [`docs/hardware/CARTRIDGES-MBC.md`](../hardware/CARTRIDGES-MBC.md)
10. `MBC6` runtime support — done in the current branch baseline as a dedicated cartridge-local device with focused unit/persistence coverage, automated cargo-test coverage for the synthetic split-window/flash fixture, and manual commercial smoke validation of `Net de Get: Minigame @ 100`; see [`docs/hardware/CARTRIDGES-MBC.md`](../hardware/CARTRIDGES-MBC.md)

The purpose of that split is to close the remaining DMG-relevant special-cartridge work first while keeping CGB-only specials explicit, typed, and impossible to misclassify. `MBC30` now has functional runtime support after the base CGB gate, `MBC6` now has the dedicated split-window SRAM/ROM/flash runtime, typed persistence, repo-owned synthetic MBC6 fixture coverage in cargo tests, and manual commercial validation needed to avoid a misleading fallback mapper, and `MBC7` now has focused runtime support for ROM banking, accelerometer latching, serial EEPROM, persistence, save-state, and bus descriptors while keeping runtime rumble disabled until hardware evidence identifies a control path.
...
Current status for point `9`: done for the documented MBC7 runtime subset. `MBC7` now loads as a dedicated supported cartridge family with `7`-bit ROM banking, the two documented enable gates, `A000-AFFF` accelerometer / EEPROM registers, `B000-BFFF` `$FF` reads, deterministic host accelerometer input, `93LC56`-style serial EEPROM commands backed by a raw `256`-byte persistent payload, save-state coverage, and explicit bus descriptors. Header `$22` still preserves the official `MBC7+SENSOR+RUMBLE+RAM+BATTERY` name, but `has_rumble()` remains false because no documented runtime control route for MBC7 rumble is currently known.
...
3. Integrate battery-gated hardware persistence.
   ...
   Acceptance criteria: only battery-backed cartridges and documented persistent EEPROM cartridges generate hardware-style persistence by default, `MBC2` restores nibble RAM correctly, `MBC3` restores SRAM plus RTC correctly, `MBC7` restores raw EEPROM correctly, and non-persistent cartridges do not silently produce hardware-save payloads unless an explicit non-faithful option is added.
   Status: done in the current branch baseline. `gb-persistence` now exposes explicit host-side hardware-persistence helpers that gate default cartridge save/load behavior through validated cartridge capability metadata instead of filename heuristics or RAM-size guesses. The default helper path persists battery-backed `PersistentRam`, `PersistentRtc`, or `PersistentRamAndRtc` profiles plus documented `PersistentEeprom` media such as MBC7, skips `NonPersistentRam` cartridges without creating hardware-save files, and covers `MBC2` nibble RAM, `MBC3` SRAM+RTC, and `MBC7` raw EEPROM round-trips through the public helper layer. Powered-off RTC elapsed-time application and higher-level flush triggers remain sequenced under the later persistence subblocks.
...
- standard MBC3 behavior is modeled inside cartridge devices with explicit RAM-bank versus RTC-register selection and live-versus-latched RTC state, while MBC30 stays on an explicit supported variant path for extended ROM / RAM banking
...
- MBC7 behavior is modeled inside a dedicated cartridge device with explicit ROM banking, sensor latch state, EEPROM serial state, raw EEPROM persistence, and no invented runtime rumble path
...
- battery-backed cartridges and documented persistent EEPROM cartridges auto-persist by default, while full emulator save states remain a separate system
