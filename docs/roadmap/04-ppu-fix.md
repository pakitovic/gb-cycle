...
- **M3 — Observation tables deleted.** Each table family (smallest regression surface first; SCY last) replaced by live reads + uniform write-latency; unit tests rewritten to assert live-read behavior. Guardrail doc line 57 updated. *Gate: full §7 green at every sub-step, with `cargo rom-report mealybug-tearoom-tests` (both suites) green before deleting the next table.*
  ...
  **M2 STEP-2 RE-DIAGNOSED — it is NOT the fetcher/obj-stall; it is the CGB SCY write-OBSERVATION latency (= roadmap §4 M2-step-1, never implemented). (2026-06-14, decisive measurements, tree green at seed-fix 121px.)** Internal traces (ephemeral, reverted) of the BG GetTile0 (`advance_bg_fetcher_tile_index_dot1`) and the CPU SCY writes (`write_register` Scy) settled it:
    - gb-cycle's startup BG fetcher **DOES** obj-stall (the earlier "obj=Idle through startup" claim was WRONG): ly24 obj fetch dot87-93, BG held at TileIndex/0 dot88-95, VisibleTile2 GetTile0 at dot96.
    - **The CPU writes SCY at IDENTICAL line_dots in gb-DMG and gb-CGB**: `dot80→0, 88→1, 96→2, 104→3, 112→4, 120→3...` (rampa cada 8 desde dot80), measured both ways. gb-DMG **passes** m3_scy_change with exactly these writes. So gb's CPU-PPU timing is correct and console-consistent — NOT a CGB CPU-PPU misalignment.
    - **DocBoy's OBSERVED scy (the global `scy`, post-`pending_write`) for ly0 is completely different**: `0` held until **dot102**, then `3,4,3,2,1,0,1,2,3,4...` (after the start it tracks ~2 dots ahead of gb's raw triangle). Since the CPU writes are identical and gb-DMG passes, **the only remaining difference is CGB's SCY write-observation behavior** — DocBoy's `write_scy` sets `pending_write.scy.countdown=2` (ppu.cpp:3526) so the PPU sees SCY writes delayed/filtered; gb-cycle has NO such model (it reads raw `self.scy` live everywhere in mode3).
    - ⇒ **The fix is the CGB SCY write-observation latency in the register layer (M2-step-1), NOT the obj-stall/GetTile0 timing (M2-step-2).** The earlier "obj-stall-delayed GetTile0" framing was chasing a symptom. Both the seed (closed via GetTile0 capture, which happens to land in the stable scy=0 region) and the VisibleTile2/Tile3 bands are the SAME root cause: gb sees the raw SCY triangle from dot80 while CGB hardware observes the `pending_write`-filtered schedule.
    - **ROOT CAUSE NAILED (2026-06-14): a ~4-dot CGB CPU↔PPU phase offset, NOT a PPU-mode3 problem.** Instrumented DocBoy's `write_scy` in DocBoy-`dots` and gb's SCY write handler in `line_dot`, with the PPU origin pinned the same way (mode3/DUMMY @80 in both, verified this run). Result: **the CPU writes SCY 4 dots earlier (relative to the PPU) on CGB hardware than in gb-cycle CGB** — DocBoy writes val=0 @dots76 (4 dots BEFORE mode3@80), gb writes val=0 @line_dot80 (AT mode3). The full schedules: gb writes `@80→0,88→1,96→2,104→3,112→4...`; DocBoy writes `@76→0,84→1,92→2,100→3,108→4...` (gb = DocBoy + 4). With CGB's `pending_write` (+2) on top, DocBoy's PPU OBSERVES `@78,86,94,102...` = gb−2. Since gb-DMG writes @80 and PASSES (DMG fixture reflects @80 = DMG hardware), **CGB hardware runs the CPU ~4 dots ahead of the PPU vs DMG, and gb-cycle CGB uses the DMG phase** — almost certainly the CGB **SkipBoot** CPU↔PPU phase (the CGB boot ROM is longer; the skip-boot handoff state doesn't replicate the CGB raster phase) or a CGB mode2/line-length detail.
    - **Implications for the fix:** modelling CGB's SCY write-observation latency (`pending_write` countdown=2) ALONE is NOT enough and pushes the wrong way (it delays observation by +2, but gb needs −2 net); the dominant component is the −4 CPU↔PPU phase. The real close needs (a) the CGB CPU↔PPU phase corrected (~4 dots) AND (b) the CGB SCY 2-dot observation latency. Both are **fundamental console-timing changes outside the PPU mode3 fetcher** — high regression surface across ALL CGB ROMs (mooneye/wilbertpol CGB).

  **CGB CPU↔PPU PHASE — LOCALIZED to the CGB LCD re-enable/restart alignment (2026-06-14, the phase investigation). NOT SkipBoot handoff, NOT line153, NOT the fetcher.** Drilled the ~4-dot phase to its exact origin by tracing the ROM's CPU accesses:
    - The ROM sync is **not** STAT/LY polling at the wrap (no LY reads in ly152/153) and **not** `CGB_LINE_153_LY_READ_ZERO_DOT` (=8 vs DMG 4 — a red herring, the ROM never reads LY there). The ROM writes SCY as an **unrolled `ld (hl),a` sequence** (pc 0x065C++, one write per 8 dots); the alignment is whatever CPU↔PPU phase holds at that point.
    - The LCD is **ON at the SkipBoot handoff** (`apply_startup_state`, `line_dot=0`); the ROM **disables LCD @~tcycle65732 then re-enables it**. So the phase that matters is set at the **LCD re-enable / restart**, not the boot handoff — confirmed by experiment: adding `apply_cgb_boot_handoff_raster_correction(4)` on the CGB SkipBoot path changed NOTHING (the restart resets the phase). Reverted.
    - ⇒ The −4-dot CPU↔PPU offset lives in gb-cycle's **CGB LCD-re-enable restart raster alignment** (`LCD_REENABLE_LINE0_*` in ppu.rs:61-67: the re-enable line0 is 8 dots short, mode3 starts at `MODE2_DOTS-8`, etc.). gb-cycle uses the same restart timing for DMG and CGB; real CGB's post-restart CPU↔PPU phase differs ~4 dots (the DMG re-enable passes, the CGB one is 4 off). These constants are pinned by the wilbertpol `lcd_restart`/`intr_2_*` suite, so retuning them for CGB risks that whole suite.
    - **VERDICT (updated): the CGB phase is NOT movable by any single timing lever — it is entangled console-timing debt.** The LCD-restart workstream ran 5 decisive experiments (all reverted, tree green at 121px): **(1)** `apply_cgb_boot_handoff_raster_correction(4)` on the CGB SkipBoot path → SCY writes stay @80 (no effect; the LCD is on at handoff but the ROM disables+re-enables, and/or the restart resets it). **(2)** CGB LCD-enable `CPU_LCDC_ENABLE_EFFECT_DELAY_T_CYCLES + 4` → no effect on SCY-write dots. **(3)** `CGB_LINE_153_LY_READ_ZERO_DOT` 8→4 → no effect (the ROM never reads LY at the wrap; it writes SCY as an unrolled `ld(hl),a` run). **(4)** `CGB_DEFAULT_DIRECT_BOOT_SYSTEM_COUNTER` 0x2674→0x2678 (+4, the suspicious vs-AGB delta) → no effect on SCY-write dots. **(5)** feed the recompute the 1-dot-delayed `pipeline.scy` for CGB (observation-latency direction) → **228px (worse)**: the recompute is already too LATE (output-dot scy), so delaying it more hurts. ⇒ The SCY writes land @80,88,96,104 robustly regardless of handoff/enable/line153/system-counter, AND the only latency that helps would have to move the *fetch sample EARLIER* (toward the obj-stall-delayed GetTile0), which the GetTile0-capture experiments already showed is wrong for low-x sprites. **The −4-dot CPU↔PPU phase + the SCY observation are entangled with the fetcher startup timing in a way no single lever resolves; closing m3_scy_change CGB requires a coordinated CGB timing redesign (CPU↔PPU phase + fetcher startup + SCY observation), validated against the full CGB suite — beyond a scoped fix. The full design plan for that workstream is [`12-cgb-timing-redesign.md`](12-cgb-timing-redesign.md) (scope, refuted experiments, phasing P0–P4, risks, validation).** The seed fix (landed) works only because the seed sits in the stable scy=0 region. Instrumentation to reproduce: DocBoy `CPUWR-SCY-DOCBOY` in `write_scy` + gb `CPUWR-SCY` in the SCY handler (gated GBTRACE_LY/ly==0); revert at M5.

    - **DESIGN-DOC HARDENED 2026-06-15 (anti-seam gates).** `12-cgb-timing-redesign.md` was reviewed against the explicit goal of moving AWAY from tables/manual seams and tightened so the redesign cannot itself become a new seam. Three changes, now hard gates in that doc: (1) **the −4 phase must EMERGE from a CGB enable→PPU-start model** (a CGB constant family mirroring the existing DMG `DMG0_DIRECT_BOOT_HANDOFF_PPU_PHASE_BASE_OFFSET_DOTS`/`DMG_REAL_BOOT_POWER_ON_LCD_ENABLE_INITIAL_LINE_DOT` at ppu.rs:58/62 — gb-cycle has zero CGB equivalents today, that absence IS the defect), verified at BOTH the boot handoff and the LCD re-enable; a bare `is_cgb_family()`-gated `+4`/`correction(4)` literal is FORBIDDEN (curve-fit in a phase-model costume). (2) **The SCY observation latency must port DocBoy's UNIFORM `pending_write` register** (lcdc/scy/scx/wx/stat, ppu.cpp:703-726, serialized) as the single mechanism — sized to also feed the M2/M3 LCDC-table deletions — and COLLAPSE gb-cycle's 3 overlapping SCY mechanisms (2-deep latch shift, raw-live recompute, seed-fix frozen row) into it, not add a 4th. (3) **P3 has hard exit criteria**: ROM 24/24 AND the `cgb_dmg_software_startup_visible_tile2/3_*` retarget tables + the recompute override + the seed fix (`cgb_startup_seed_get_tile_scy_row`/`cgb_startup_frozen_tile_row`) all DELETED — green-with-seams is declared an explicit failure, and P4 gates on net PPU seam count being LOWER than at campaign start. Every P-step must REPLACE the mechanism it supersedes, never sit beside it.
