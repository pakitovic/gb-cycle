...
- **M3 — Observation tables deleted.** Each table family (smallest regression surface first; SCY last) replaced by live reads + uniform write-latency; unit tests rewritten to assert live-read behavior. Guardrail doc line 57 updated. *Gate: full §7 green at every sub-step, with `cargo rom-report mealybug-tearoom-tests` (both suites) green before deleting the next table.*
  ...
  **M2 STEP-2 RE-DIAGNOSED — it is NOT the fetcher/obj-stall; it is the CGB SCY write-OBSERVATION latency (= roadmap §4 M2-step-1, never implemented). (2026-06-14, decisive measurements, tree green at seed-fix 121px.)** Internal traces (ephemeral, reverted) of the BG GetTile0 (`advance_bg_fetcher_tile_index_dot1`) and the CPU SCY writes (`write_register` Scy) settled it:
    ...
    - **Implications for the fix:** modelling CGB's SCY write-observation latency (`pending_write` countdown=2) ALONE is NOT enough and pushes the wrong way (it delays observation by +2, but gb needs −2 net); the dominant component is the −4 CPU↔PPU phase. The real close needs (a) the CGB CPU↔PPU phase corrected (~4 dots, look at SkipBoot handoff timing / CGB mode2) AND (b) the CGB SCY 2-dot observation latency. Both are **fundamental console-timing changes outside the PPU mode3 fetcher** — high regression surface across ALL CGB ROMs (mooneye/wilbertpol CGB), so any change must be proven against the full CGB timing suite, not just mealybug. The seed fix (GetTile0 capture) works only because the seed lands in the stable scy=0 region where the ±4/±2 offsets don't cross a write boundary. **Verdict: closing CGB m3_scy_change is gated on a CGB CPU↔PPU phase investigation (SkipBoot), a separate workstream from PPU-mode3 hardening. Instrumentation to reproduce: DocBoy `build-trace-cgb` `CPUWR-SCY-DOCBOY` emit in `write_scy` + gb `CPUWR-SCY` in the SCY write handler, both gated GBTRACE_LY/ly==0; revert at M5.**
