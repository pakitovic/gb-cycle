...
- **M3 — Observation tables deleted.** Each table family (smallest regression surface first; SCY last) replaced by live reads + uniform write-latency; unit tests rewritten to assert live-read behavior. Guardrail doc line 57 updated. *Gate: full §7 green at every sub-step, with `cargo rom-report mealybug-tearoom-tests` (both suites) green before deleting the next table.*
  ...
  **M2 STEP-2 RE-DIAGNOSED — it is NOT the fetcher/obj-stall; it is the CGB SCY write-OBSERVATION latency (= roadmap §4 M2-step-1, never implemented). (2026-06-14, decisive measurements, tree green at seed-fix 121px.)** Internal traces (ephemeral, reverted) of the BG GetTile0 (`advance_bg_fetcher_tile_index_dot1`) and the CPU SCY writes (`write_register` Scy) settled it:
    ...
    - **OPEN NUANCE for the implementer:** a naive 2-dot delay applied to the raw writes (`@82,90,98,106...`) does NOT reproduce DocBoy's observed schedule (`0`-until-102 then jump to 3) — DocBoy *skips* the intermediate 1@~86 and 2@~94. This is either DocBoy's `pending_write` countdown interacting with the exact CPU-write sub-dot timing, or a DocBoy-vs-gb dots-origin offset (~2-4 dots) that M0 pinned only loosely. NEXT STEP: instrument DocBoy's CPU SCY writes in DocBoy-`dots` (not just the observed global) to nail the origin offset, then model CGB's `pending_write` (countdown=2) for SCY in gb-cycle's register layer and verify the observed schedule matches. Risk: a CGB SCY observation model touches every mode3 SCY read — must keep the steady path (correct today) and all CGB mooneye/wilbertpol green. The seed fix (GetTile0 capture) likely becomes redundant once the latency is modelled.
