...
#### 24.25.6 Cut 3 ATTEMPT — facet (c) DECISIVELY SPLIT: the readback half is closed cleanly (89→94), the IRQ-edge half is PROVEN Cut-1-gated; user pivots to Cut 1 (2026-06-18)

Re-applied the reproducible base {`enable+1` + vblank-wrap `+1`} (89/105 wilbertpol, reproduced exactly) and implemented the
**readback half of Cut 3** on top. Two clean, canon-positive mechanisms (both reuse the existing `readback_reference()` post-tick
helper that LY readback already uses — the SameBoy single-`ly_for_comparison`-feeds-both model, NOT new tables):
- **(c1) LYC coincidence readback through `readback_reference()`** — parametrised `live_ly_for_lyc_compare()` →
  `ly_for_lyc_compare_at(ly, line_dot)` (body byte-identical to `main`'s, which the branch already shares); the IRQ path keeps
  the live pre-tick raster, the CPU readback evaluates at the post-tick `readback_reference`. Closes `ly_lyc-GS` + `ly_lyc_144-GS`
  (89→91). The single CPU-observable divergence each (`ly_lyc-GS` pc=0x0647 `0xC4`→`0xC0`; `ly_lyc_144` pc=0x0667) is purely the
  reorder reading the coincidence one tick behind `main`, exactly the A′ mode-readback issue never migrated to the LYC path.
- **(c2) frame-wrap mode readback** — `current_published_stat_line_dot` returns `None` (→ line-start mode) when the readback
  reference wraps the frame (`self.ly==153 && readback_reference().0==0`), and `published_stat_mode_at_line_start` keys the VBlank
  decision on `ref_ly` (so DMG→HBlank, CGB→VBlank via the existing vblank-wrap delay). Closes `ly_lyc_0-GS` (its residual after
  c1, pc=0x0681 `0x81`/VBlank→`0x80`/HBlank) **AND** `ly00_mode0_2-GS` + `ly00_mode1_0-GS` (facet (b) — same root: the ly=153→0
  mode read pre-tick on line 153). 91→**94**. So facet (b) is NOT "more ly=0 readback dots" — it is the SAME frame-wrap mode read.

**Decisive oracle finding (refines §24.25.5's facet map): facet (c) `ly_lyc_*` is TWO facets, not one "LYC readback phase".**
Per-ROM differential trace (branch+base vs `../gb-cycle-main`, all 8 (c) ROMs + the 2 (b), CPU-observable `(addr,val,pc)` only):
- **Readback half (3 ROMs: `ly_lyc-GS`, `ly_lyc_144-GS`, `ly_lyc_0-GS`)** — single STAT-bit read divergence at a line/frame
  boundary. CLOSED by (c1)+(c2). Clean, mirrors A′, reuses `readback_reference`, net seams neutral-to-down.
- **IRQ-edge half (5 ROMs: `ly_lyc_write-{GS,C}`, `ly_lyc_0_write-{GS,C}`, `ly_lyc_153_write-C`)** — these PASSED on the clean
  baseline and only broke under `enable+1`; their LY/mode **readbacks are byte-identical to `main`** (0 divergence). The failure is
  the **STAT IRQ becoming VISIBLE in IF one read-position late** in the frame after an LCD re-enable (`ly_lyc_write-GS` pc=0x4843:
  branch sets STAT IF bit 0x02 at ly=2 dot=1, `main` by ly=2 dot=0). Same root as the **mooneye `lcdon-write-timing-gs` regression**
  (clean mooneye-acceptance 73/75 → with the cut 72/75; `lcdon-timing-gs` + `boot-hwio-dmg0` pre-existing). Two bounded fixes
  REFUTED: (i) restoring `main`'s `regular_line_dot0_compare_window → ly==lyc` for the IRQ None case = **zero gate effect** (so it is
  NOT the dot0 LYC edge); (ii) shortening the re-enable line-0 length would desync the readback, which already matches — so it is
  NOT the line-0 length. The edge is the IF-visibility phase (hide-family/Cut A territory) balanced against the restart timing.
- **Mechanism (why Cut-1-gated):** DocBoy's canonical LYC IRQ uses `last_ly`/`last_lyc` captured at end-of-tick AND `ly` that
  increments mid-line (dot 453), so at line_dot 455 `last_ly` already leads to the next line and the dot0 edge lands on time.
  gb-cycle increments `ly` only at the end-of-line wrap, so its `last_ly` lags ~3 dots; the E1 dot0-defer seam compensates but is
  balanced against the (clean) restart-1-early. `enable+1` removes that balance and exposes the lag. The canonical fix (delete E1 +
  `last_ly`/`last_lyc` for the IRQ) is impossible without `self.ly` leading mid-line = **Cut 1**.

⇒ **Facet (c) cannot close standalone: its IRQ-edge half genuinely requires Cut 1 first**, reversing §24.25.5's "Cut 3 first" back
to the original §24.18/§24.25 sequencing (Cut 1 → then Cut 3's IRQ half). Net WIP {base + c1 + c2} = wilbertpol 94 (break-even
count; restart-MODE family fixed, 5 write ROMs broken) + mooneye 72 (−1) ⇒ net-negative, REVERTED to committed clean 94/105 + Cut A
per the established pattern, this finding captured. **The reproducible cut for re-application after Cut 1** = {`enable+1` (registers.rs:99
`+1`); vblank-wrap `+1` (published_stat.rs); LYC readback via `readback_reference()` (irq.rs `ly_for_lyc_compare_at` + readback eval);
frame-wrap mode (published_stat.rs `readback_at_frame_wrap` + `published_stat_mode_at_line_start` on `ref_ly`)}. Probes kept
(`#[ignore]`): `irq_trace_env` (env `GB_TRACE_ROM`/`GB_TRACE_MODEL`/`GB_TRACE_CYCLES`) for per-ROM branch-vs-main differential traces.
**USER CHOSE: pivot to Cut 1 now** (§24.18 pt 1 + re-anchor groups 1-6; ground the exact `self.ly` increment dot vs the oracle).

#### 24.25.7 Cut 1 grounding — `enable+1` PROVEN to be the WRONG tool for the restart-MODE family; the write ROMs are an IRQ-cycle-count shift, not an edge; a readback-only restart-MODE fix (NO enable+1) is the promising path (2026-06-18)

Started Cut 1. Two decisive groundings before touching the 100+ `self.ly` sites:
- **`self.ly`-leads scope:** the literal "make `self.ly` lead" (§24.18 pt1) touches **114 `self.ly` sites** (irq 33, api 18, registers 17, mode2 12, …) + a new `PpuSaveState` field (the `raster_ly()` reconstruction needs a stored "led-this-line" flag — line143-led vs line144-natural and line152 vs 153 are ambiguous from `(self.ly, line_dot)` alone). The increment dot is **450** (the branch's own `read_ly` leads from `self.line_dot 450` via `ref_dot>=451`, preserved when `read_ly` becomes `self.ly`). NOTE the FUNCTIONAL goal (one leading value feeding `read_ly` + Cut 3's `last_ly`) is reachable via a `lead_ly()` helper WITHOUT mutating `self.ly` (zero churn to the 114 sites) — `self.ly`-leads is the doc's chosen means, not the end.
- **Write-ROM source PINNED + the enable+1 verdict.** Added a temp `stat_irq_source_debug()` mask probe (reverted). `ly_lyc_write-GS` (branch+`enable+1`): the late STAT IF edge is `src=0x81` = **LYC coincidence** (LYC=2 → STAT IRQ at LY==2), firing ly=2 dot=1 vs `main` dot=0. BUT restoring `main`'s `regular_line_dot0_compare_window → ly==lyc` (fire at dot0) **moved the edge to dot0 AND made the CPU-observable trace byte-identical to `main` through 1.5M — yet the ROM STILL FAILS (85/105, all 5 write ROMs red).** ⇒ **the write-ROM failure is NOT the dot0 LYC edge timing**; it is that `enable+1` shifts the ENTIRE post-re-enable frame by 1 cycle, so the STAT-IRQ-relative-to-CPU-instruction-count these ROMs measure (their pass signature is at the terminal, invisible to the FF41/FF44/FF0F read probe) is off by one. The readback (LY/mode) matches because the reorder readback compensates; the IRQ cycle-count does not.
- **STRATEGIC REFRAME (supersedes the enable+1 base): `enable+1` is the WRONG tool for the restart-MODE family.** It fixes the mode timeline by delaying the restart 1 cycle, but that 1-cycle frame shift is exactly what breaks the write ROMs' IRQ count (and mooneye `lcdon-write`). The restart-MODE fails (`ly143_144_*`/`lcdon`) are a mode **READBACK** boundary divergence (Cut R0: one FF41 mode0-vs-mode3 read). So they should be closed by a **readback-only** compensation on the restart line (the A′ family, evaluated at the post-tick reference) that leaves the restart's wall-clock timing UNTOUCHED — keeping the (early) restart's IRQ count intact (write ROMs stay green) AND the mode read correct. On the clean tree `ly143/lcdon` FAIL while the write ROMs PASS, so a readback-only `ly143/lcdon` fix is a candidate **net-positive that AVOIDS `enable+1` entirely** (then c1+c2 close the ly_lyc/ly00 readback on top). NEXT: prototype the readback-only restart-MODE fix (no enable+1), gate `ly143_144_*`+`lcdon` vs the write ROMs + mooneye `lcdon`; if it holds, the whole `enable+1`-based reproducible cut + the 114-site Cut 1 are unnecessary for the restart family. Tree reverted to committed clean 94/105 + Cut A; probes kept (`#[ignore]`).

#### 24.25.8 BREAKTHROUGH — the readback-only blank-frame un-shift lands 94→104/105 (NO enable+1, NO Cut 1); residual 1 ROM is the non-uniform restart-early offset (2026-06-18)

The readback-only hypothesis is CONFIRMED and large. ROOT (pinned with a register+past-halt differential trace, `cut1` probes): every committed-94 restart-family fail is a **blank-frame mode READBACK over-shift**. A′'s uniform `+1` reference (`current_published_stat_line_dot = line_dot`) is correct in the steady frame but during the post-re-enable **blank frame** the branch's internal raster is one dot early (Cut R0's restart-1-early), which already cancels the reorder's pre-tick read — so A′'s `+1` over-shoots by one and the CPU reads the mode boundary one dot off. `ly143_144_mode3_0` diverges at exactly one FF41 read (ly=143 dot=252, blank=true: branch `0x80`/HBlank vs `main` `0x83`/Drawing), after which the control flow (hence the terminal fibonacci signature) diverges — invisible to a reads-only diff, visible only with register/past-halt capture.
- **FIX (one line, NO enable+1, NO 114-site Cut 1):** `current_published_stat_line_dot` returns `line_dot - 1` when `blank_frame_active` (undo A′'s `+1` for the blank frame only; steady `+1` intact). **Gate: wilbertpol 94→104/105, mooneye 73→73 (recovers `ppu-lcdon-timing-gs`), blargg 58/58.** Recovers ALL 11 committed-94 fails (ly143_144×4, lcdon, the 5 sprites-scx-nops "§245 targets", hblank-ly-scx) — the §24.25.1/.2 "Cut 2 targets" + the restart-MODE family TOGETHER. The write ROMs stay green (no enable+1 → no IRQ-count shift). This is the clean realization of the readback-only path and refutes the need for `enable+1` / the 114-site `self.ly`-lead for the entire restart+§245-readback cluster.
- **Residual (1 ROM): `intr_2_mode0_timing_sprites`** (both wb+mn) regresses. Differential trace (register+past-halt): its measuring read is ALSO a blank-frame mode0-boundary read (ly=68 dot=263, m0s=263 sprite-extended) but there the restart-early offset is **0**, not `+1` (branch pre-tick line_dot=263 = `main` post-tick 264 − 1 = pure reorder, no early-restart), so A′ (`line_dot`) is correct and the un-shift over-corrects (reads Drawing where `main` reads HBlank). ⇒ **the restart-early offset is NON-UNIFORM across the blank frame** (present for the lines the 10 recovered ROMs read, absent at this read — it depends on the per-ROM re-enable phase within the line, not just "blank frame"). A uniform blank-frame `line_dot-1` is therefore too blunt for the last ROM.
- **NEXT:** make the un-shift offset-precise (superseded — see §24.25.9: the offset is uniform; the residual is IRQ-dispatch, not readback). Tree HELD at the 104/105 WIP (one-line blank un-shift in `published_stat.rs`); `#[ignore]` probes enriched (register + `GB_TRACE_NOHALT` + `GB_TRACE_RASTER`).

#### 24.25.9 105 residual is IRQ-DISPATCH timing, NOT readback — the blank-frame `+1` offset is UNIFORM; `intr_2_sprites` needs the dispatch/offset eliminated (Cut 1) (2026-06-18)

Per-cycle internal-raster trace (`GB_TRACE_RASTER`, branch-WIP vs `main`, aligned by `@cycle`) corrects §24.25.8's "non-uniform offset" guess: the branch's blank-frame internal `line_dot` is **uniformly `+1` ahead of `main`** at every dot of every blank-frame line (both ROMs). So the readback un-shift is the correct readback model everywhere in the blank frame. `intr_2_mode0_timing_sprites` is NOT a readback failure — its measuring read is **post-HALT**, and the decisive `@cycle` comparison shows the branch DISPATCHES the wake STAT IRQ one wall-clock cycle EARLY: the mode2 STAT IF is pending at the SAME `@cycle` on both (`@604975`), but the branch services it `@604978` vs `main` `@604979`. That one-cycle-early dispatch shifts the post-halt measuring read one cycle early (branch reads FF41 `@605242` line_dot 263→Drawing; `main` `@605243` line_dot 264→HBlank), so the (correct) un-shift then reads the boundary one dot early. ⇒ the residual is the **interrupt-DISPATCH/IF-visibility timing under the CPU-first reorder + the blank `+1` offset**, a CPU-IF-side effect (hide-family territory), NOT the PPU readback. The write ROMs are INSENSITIVE to the 1-cycle dispatch shift (they pass at both `+1`-early and on-time), which is why only the halt-timed `intr_2_sprites` exposes it. **A clean 105/105 cannot be reached by any bounded readback/IRQ-source tweak** — it needs the blank-frame `+1` offset eliminated at the source so BOTH readback (A′ becomes correct, un-shift deletes) AND dispatch are right. That elimination is the LCD-restart re-ground done in lockstep (countdown + `LCD_REENABLE_LINE0_*` mode timeline, §24.25.5 2nd-attempt — `enable+1` alone shifts the mode timeline and is refuted) OR the §24.18 `self.ly`-lead Cut 1. The landed 104/105 one-liner is the clean readback interim that survives into either end-state. RECOMMENDATION: land 104/105 as the interim (pending the cross-suite no-regression sweep), then close the last ROM via the lockstep restart re-ground (the offset elimination) — a bounded restart-timing cut, distinct from the 114-site `self.ly` rewrite.

**CROSS-SUITE SWEEP RESULT (6-agent workflow, clean vs WIP binaries): the un-shift is net ~+23 but NOT regression-free — the residual is uniformly the dispatch class.** gbmicrotest 391→395 (**+4**: `ppu-lcdon-to-stat0/3-{a,c}`), docboy-dmg 1508→1518 (**+14 newly-passing** turn-on-STAT/pixel-transfer/oam-scan, **−4 regressions**), mealybug-cgb / samesuite / little-things / docboy-cgb-dmg unchanged. The +4/+14 across THREE independent suites (gbmicrotest + docboy + the recovered wilbertpol/mooneye) is strong confirmation the blank-frame readback over-shift was a real, broad bug and the un-shift is the correct readback model. **The 5 regressions are all one class** — `intr_2_mode0_timing_sprites` (wb+mn) + 4× `docboy-ppu-pixel-transfer-timing-window-sprite-…-round2` — sprite/window-extended mode-boundary reads in a re-enable/halt context where the dispatch lands one cycle early (the §24.25.9 IRQ-dispatch effect), so the (correct) readback un-shift then reads the boundary one dot off. ⇒ The un-shift alone is a net-+23 interim with 5 dispatch-class regressions; the CLEAN end-state needs the blank `+1` offset eliminated at source (lockstep LCD-restart re-ground or Cut 1), which deletes the un-shift AND fixes all 5 dispatch regressions together. DECISION PENDING (user): land the +23/−5 interim, OR do the lockstep restart re-ground for a clean close, OR revert + document.

#### 24.25.10 LOCKSTEP RESTART RE-GROUND REFUTED — `enable+1` eliminates the offset (→0, verified) but tops at 85-94 (breaks write+sprites); the readback-only un-shift (104) is strictly better; clean 105 needs Cut 1 (2026-06-18)

Tested the user-chosen lockstep path empirically. `enable+1` (delay 5→6, the reorder-invariant countdown) **does eliminate the blank-frame `+1` internal offset → 0** (verified: `GB_TRACE_RASTER` ly143_144, 60000/60000 blank-frame cycles offset 0 vs `main`). BUT `enable+1 + A′` gates **85/105**: recovers the restart-MODE family (lcdon + ly143_144×4) but regresses 14 (ly_lyc 9 + ly00 5), and even `enable+1` + the full readback fixes only reaches 94 (§24.25.5's stuck point) with write(5)+sprites(6) still red. ROOT: the write ROMs + sprites **PASS at the branch's offset-`+1` IRQ phase** (their STAT-IRQ dispatch phase happens to match what they measure); `enable+1` shifts the restart → changes that IRQ phase → breaks them. The `LCD_REENABLE_LINE0_*` lockstep shift only adjusts the line-0 MODE timeline, not the subsequent-line IRQ phase, so it cannot recover write/sprites. ⇒ **offset elimination is strictly INFERIOR (≤94) to the readback-only un-shift (104)** — the un-shift fixes the readback WITHOUT touching the restart/IRQ timing, so write+sprites stay green. The lockstep re-ground is the doc's already-exhausted ≤94 path; REFUTED as a route past 104.
- **The fundamental obstruction to a clean 105:** at the branch's offset-`+1` phase, the write ROMs need the `+1`-early STAT-IRQ dispatch (they pass with it) while `intr_2_sprites`+4 docboy need the on-time dispatch — **incompatible at offset `+1`**, and eliminating the offset (to make dispatch on-time) breaks the write ROMs' phase. The only consistent resolution is one uniform phase everywhere = **Cut 1 (`self.ly` mid-line lead)**, which the un-shift + the deleted-offset both fold into.
- **STATE: the readback-only blank-frame un-shift (104/105 wilbertpol, +29/−5 cross-suite, the −5 all dispatch-class) is the best achievable without Cut 1.** It is the clean readback half that survives into Cut 1. A clean 105/105 (no regression) requires Cut 1. RECOMMENDATION: land the un-shift as the documented interim (net +29, the 5 dispatch regressions recorded as debt — 4 are in the non-passing docboy-dmg suite, 1 is `intr_2_sprites` wb+mn), and schedule Cut 1 as the clean-105 end-state; do NOT pursue the lockstep/`enable+1` further (refuted ≤94).

#### 24.25.11 Cut 1 GROUNDING DONE + the means is the `lead_ly()` helper (NOT literal `self.ly`-mutation) + the unify-blank refutation pins the real complexity (2026-06-18)

Executed the Cut 1 grounding (the user's mandated first step). Decisive new results; tree reverted to the committed clean 104/105 baseline (no code change committed this session).

**Grounding (probe `cut1_grounding_internal_ly_phase`, bare `PpuTestRig` steady-state, branch vs `../gb-cycle-main`, DMG+CGB):** the ONLY branch↔main divergence across the whole end-of-line / vblank-entry / line-153 scan is the **LY readback, and it is uniformly `1` `line_dot` EARLIER on the branch** (normal line: branch `obs_ly` flips N→N+1 at dot **450** vs main **451**; vblank-entry 143→144 same; line 152→153 branch leads at dot **455** vs main not-until-153/dot0; line 153→0 branch at dot **3**(DMG)/**7**(CGB) vs main **4**/**8**). `stat_mode`, `internal_mode`, `lyc_coin` are **byte-identical** branch↔main. ⇒ **`self.ly` ALREADY equals the physical non-leading line (= `raster_ly`), and `read_ly` ALREADY produces the correct leading observable** (via `readback_reference()=line_dot+1` (A′) + `current_ly_read_advance_start_dot()`); the `1`-dot-earlier lead IS the reorder compensation (the CPU reads pre-tick, so the branch leads one `line_dot` earlier to land the SAME value at the same wall-clock cycle as main, which is why it passes steady-state).
- **MEANS DECISION (user-confirmed): the `lead_ly()` helper, NOT literal `self.ly`-mutation.** Since `self.ly` already == `raster_ly` and `read_ly` already leads, the §24.18/§24.25.7 "self.ly leads mid-line + raster_ly() + a `led-this-line` save-state flag + re-derive 114 sites" is pure churn for an IDENTICAL observable. The functional goal (one leading value feeding `read_ly` + Cut 3's `last_ly`) = a `lead_ly()` helper (the existing `read_ly` lead body), `self.ly` untouched. This is SameBoy's single `ly_for_comparison` model (§24.25), equally canonical, deletes the irq-vs-readback split, no new state. So Cut 1's "self.ly-lead" content is essentially ALREADY PRESENT structurally; the real 104→105 work is B' (restart re-ground) + C (`last_ly` LYC edge via `lead_ly`) + delete-un-shift.

**Residual mechanism CONFIRMED firsthand (real-ROM differential trace `intr_2_mode0_timing_sprites`, branch vs main, `GB_TRACE_NOHALT`):** both set the mode2 STAT IF pending at the SAME `@604975` (the blank-frame pretrigger lead 3-vs-4 compensates the `+1`), but the branch's blank-frame raster runs **1 cycle ahead in wall-clock** (internal dot 453 vs main 452 at that cycle), so it reaches ly68/dot0 `@604978` (main `@604979`) → HALT-wakes 1 cycle early → the post-IRQ measuring FF41 read lands `@605242` reading Drawing `0xA3` (main `@605243` HBlank `0xA0`). ⇒ the residual is 100% the blank-frame `+1` raster offset propagating to the WALL-CLOCK timing of the dispatch+read; the un-shift fixes the readback model but not the wall-clock phase. Confirms §24.25.9.

**B' measurement (`enable+1` (delay 5→6) + delete un-shift): 85/105, the exact reopened set captured.** `enable+1` CLOSES the dispatch class (`intr_2_mode0_timing_sprites` base disappears) but reopens **20**: 6 readback (`intr_2_sprites_nops`, `sprites_scx1..4_nops`, `hblank_ly_scx`), 9 `ly_lyc_*` (incl. the 5 write ROMs), 5 `ly00_*`. Confirms §24.25.10's 85. Root: the un-shift AND the OTHER blank-frame compensations (`ordinary_mode2_stat_pretrigger_lead_dots` 3-vs-4, `mode2_vblank_entry_stat_source` `−1`, `read_ly` `!blank_frame_active` lead-suppress, the `published_stat` mode0/mode2/OAM override `!blank` guards) were ALL balanced against the `+1`.

**NEW REFUTATION — "unify blank with steady" is WRONG (85→82).** With `enable+1`, uniformly REMOVING the blank-frame readback guards (the 3 `published_stat` override `!blank`, `read_ly` `!blank`, the 2 mode2 leads) RECOVERS only 4 (`sprites_scx1..4_nops`) and BREAKS 7 (`lcdon_mode_timing` + `ly143_144_{145,152_153,mode0_1,mode3_0}` restart-MODE + `ly_lyc_{c,144_c}`). ⇒ **the blank frame is NOT phase-identical to a steady frame even at offset 0** — the `LCD_REENABLE_LINE0_*` restart phase (line-0 length `DOTS_PER_SCANLINE−8`, distinct mode timeline) makes the `published_stat` overrides interact differently. The blank guards are NOT all pure `+1` compensations; they are ENTANGLED with the restart-phase mode timeline. Each must be re-grounded INDIVIDUALLY against the oracle in lockstep with the restart timeline, NOT removed uniformly. This is the precise trap the prior `enable+1` attempts hit and the reason the close is multi-step, not a flag flip.

**The atomic recipe + ordering for the clean 105 (mapped, not yet landed):** `{ enable+1 (B', kills the +1 offset, verified offset→0) + re-ground EACH blank-frame readback compensation individually vs the LCD_REENABLE_LINE0_* restart timeline (NOT uniform removal) — the 6 readback ROMs + the restart-MODE family — + C (last_ly/last_lyc via lead_ly() anchoring the LYC edge so the 9 ly_lyc survive offset 0; the readback half = §24.25.6 c1/c2, the IRQ-edge/write half = the last_ly model) + delete the un-shift }`. The ly00 (5) are the §24.25.6 c2 frame-wrap mode reads. Every intermediate is net-negative (atomic), so it lands as one cut, gated `cargo rom-suite wilbertpol --suite wilbertpol-acceptance == 105` + mooneye-acceptance no-regress (boot-hwio-dmg0 stays) + blargg 58 + cross-suite. A bounded mode2-blank-`halt_wake_deferred` extension WOULD close `intr_2_sprites` cheaply and would NOT touch the LYC write ROMs (different source, no halt-wake-defer path) — but it ADDS a seam, against the canon goal (net seams must FALL), so it is correctly excluded. Probes live: branch `oracle.rs` cut1_* + irq_trace_env; ported to `../gb-cycle-main/.../oracle.rs` (cut1 grounding + irq_dispatch_trace infra) for differential diffing.

#### 24.25.12 Cut 1 EXECUTED to the readback-half end-state — `enable+1` + the full readback half lands 85→**99/105** (PAST the doc's claimed ≤94 enable+1 ceiling); the residual is EXACTLY the 5 LYC-write IRQ-edge ROMs + hblank, both Cut-1/dispatch-deep (2026-06-18)

Executed the atomic recipe through step 1 + 2 + the readback half of step 3, building the cut in oracle-gated layers (`cargo rom-suite wilbertpol --suite wilbertpol-acceptance`, branch-vs-`../gb-cycle-main` differential traces via `irq_trace_env`, all 20 main oracle traces regenerated to `/tmp/oracle/`). **Tree reverted to the committed clean 104/105 at close (the WIP 99 is net-negative vs the committed 104 — atomic cut incomplete — so reverted per the established pattern; the reproducible cut is recorded verbatim below).**

**The reproducible cut (85→99, each gated, ZERO regression among the closed set):**
1. `enable+1`: `CPU_LCDC_ENABLE_EFFECT_DELAY_T_CYCLES` 5→6 (`ppu.rs:67`) + delete the blank-frame un-shift (`published_stat.rs current_published_stat_line_dot` `blank→line_dot-1` branch). ⇒ **85/105**, reopening the exact documented 20 (6 readback + 9 ly_lyc + 5 ly00); `intr_2_mode0_timing_sprites` BASE now PASSES (enable+1 fixes its dispatch).
2. **Individual blank re-ground — `ordinary_mode2_stat_pretrigger_lead_dots` → `4` always** (delete the blank→3 special-case; at offset 0 the blank mode2 dispatch matches steady). ⇒ **90/105**, recovers all 5 sprites (`intr_2_sprites_nops` + `scx1..4_nops`), NO restart-MODE regression. (The "unify blank" refutation in §24.25.11 was removing ALL guards at once; removing this ONE is safe — confirming the individual-re-ground thesis.)
3. **c1 — LYC readback via `readback_reference()`**: parametrise `ly_for_lyc_compare_at(ly, line_dot)`, route `lyc_coincidence_for_readback` through `readback_reference()` (irq.rs). ⇒ **92/105**, closes `ly_lyc-GS` + `ly_lyc_144-GS`.
4. **c2 + vblank-wrap`+1`**: `current_published_stat_line_dot` returns `None` at the frame wrap (`self.ly==153 && readback_reference().0==0`); `published_stat_mode_at_line_start` keyed on `ref_ly`; vblank-wrap branch `(line_dot+1)-4`. ⇒ **97/105**, closes the 4 `ly00_*-GS` + `ly_lyc_0-GS`.
5. **CGB frame-wrap VBlank**: `published_stat_mode_at_line_start` returns VBlank for `is_cgb && self.ly==153 && ref_ly==0`. ⇒ **99/105**, closes `ly00_mode1_2-C` + `ly_lyc_0-C`.
6. (dot0 restore — `lyc_coincidence_for_irq_line` None case `regular_line_dot0_compare_window → ly==lyc`, deleting E1's defer: **zero gate effect** at 99, re-confirming §24.25.6 — the write ROMs are NOT the dot0 edge.)

**KEY RESULT — the doc's "enable+1 tops at ≤94" (§24.25.10) is REFUTED: the FULL readback half reaches 99.** §24.25.6/.10 only had a partial readback half; adding the `ordinary_mode2` lead re-ground + the CGB frame-wrap VBlank + the vblank-wrap`+1` together closes 13 of the 20 reopened ROMs cleanly. The residual at enable+1 is now **6, not 11**: the **5 LYC-write IRQ-edge ROMs** (`ly_lyc_write-{GS,C}`, `ly_lyc_0_write-{GS,C}`, `ly_lyc_153_write-C`) + `hblank_ly_scx_timing_variant_nops`. mooneye-acceptance on the WIP = 72/75 (`boot-hwio-dmg0` pre-existing + `ppu-lcdon-timing-gs` + `ppu-lcdon-write-timing-gs` — the last is the SAME write/IRQ-edge class).

**Fresh decisive diagnostics (differential traces, branch-WIP vs oracle, CPU-observable + register fields):**
- **The 6 readback ROMs were DISPATCH, not readback-formula.** Raster traces (`GB_TRACE_RASTER`) are byte-identical branch-vs-oracle for `ly143_144` (offset 0) AND `sprites_nops`'s first blank frame; the sprites failure is the per-testcase re-enable CPU dispatch shifting under enable+1's restart change. The fix was the `ordinary_mode2` halt-wake/pretrigger lead re-ground (step 2), NOT a published_stat/read_ly formula — exactly the `dmg_mode2_oam_halt_wake_deferred`/pretrigger family §24.25.11 flagged to "evaluate".
- **The 5 write ROMs are a `B`-count off-by-one, NOT the dot0 LYC edge.** `ly_lyc_write-GS`: at the measuring read (pc=0x0472) the **FF41 VALUE is identical (0xC2) but `B` differs (branch 01 vs main 00)** — the branch's poll loop runs ONE extra iteration because the LYC-write-triggered STAT IRQ lands ~40 dots (1 iteration) late (ly=3 dot=52 vs main dot=12). This is INVISIBLE to a read-value-only probe (why §24.25.7's "byte-identical trace yet fails" was a probe blind-spot; the BCDEHLA-augmented normalizer catches it). Restoring the dot0 edge = zero effect ⇒ it is the §24.25.7 enable+1 frame-cycle shift propagating to the write-IRQ-relative-to-CPU-count, i.e. the §24.25.10 "write ROMs need the +1-early dispatch / sprites need on-time — incompatible at offset +1" obstruction. The §24.17 last_ly variants (1/2/3) are all PRE-enable+1 and do not address the enable+1 frame shift.
- **`hblank_ly_scx` is tooling-blocked, not yet diagnosed:** byte-identical branch-vs-oracle through 2.4M cycles; the ROM fails at 6.3M, but the `irq_dispatch_trace` probe stops early on a spurious `0x40`/`0xED` opcode (~2.4M) so the divergence past 2.4M cannot be traced with the current probe (needs a magic-terminal guard that only fires at the real fibonacci terminal).

**Conclusion + next step.** The readback half of Cut 3 is **VALIDATED and net-positive (85→99, 13 ROMs, zero regression among them)** and is the clean interim that survives into the end-state. The clean 105 residual is precisely the **IRQ-edge/write half** + hblank.

#### 24.25.13 IRQ-edge half ATTEMPTED — the recipe's `enable+1`-based path is DECISIVELY REFUTED for the write ROMs; the canon-clean 105 needs either an excluded seam or the heavy proper-Cut-1 (the `lead_ly`-only reframe is insufficient) (2026-06-18)

Re-applied the 85→99 readback-half cut and attempted the IRQ-edge half (recipe step C). Two decisive experiments, both REVERTED (tree back at committed clean 104/105, only this doc modified):
- **C as a leading coincidence LEVEL — ZERO effect.** Routed `lyc_coincidence_for_irq_line` through `readback_reference()` (the same post-tick lead the readback uses, LIVE so writes re-eval immediately — avoiding the §24.17 last_lyc-DELAY refutation). Gate UNCHANGED at 99 (write ROMs still red, nothing regressed). Combined with the dot0-restore (also zero effect, §24.25.6), this **rules out the LYC-coincidence-level phase** as the write-ROM cause.
- **Write-ROM mechanism PINNED (`ly_lyc_write-GS`, branch-99 vs oracle).** Both read `FF44=0x01` at ly=1 (B=01); by the measuring read (pc=0x0472, ly=3) the **oracle has decremented B to 00 while the branch is still B=01, and the branch arrives 40 dots/cycles LATER** (ly=3 dot=52 @264363 vs oracle dot=12 @264323). The post-LYC-write IRQ/loop timing is shifted by ~1 loop iteration — the **§24.25.7 `enable+1` frame-cycle shift** propagating to the write-IRQ-relative-to-CPU count, invisible to a read-value diff.

**DECISIVE — `enable+1` cannot reach a clean 105 (the §24.25.10 obstruction, now triply confirmed and mechanised):** the 5 write ROMs **PASS at clean-104 (no `enable+1`) and FAIL only under `enable+1`**. The restart shift moves BOTH the blank frame and the steady frame by the same cycle: at clean-104 the blank frame is offset `+1` (so `intr_2_sprites`'s blank-frame HALT-wake dispatches 1-early → it is the lone 104 fail) while the steady frame is offset `0` (write ROMs pass); `enable+1` shifts both by `−1` → blank `0` (sprites pass) but steady `−1` (write ROMs fail). **No global restart knob makes both right — blank and steady differ by exactly 1.** The readback un-shift (104) is the correct readback model precisely because it aligns blank and steady INDEPENDENTLY; the residual `intr_2_sprites` is purely the blank-frame DISPATCH (HALT-wake), not readback, and no readback/coincidence change touches it.

**⇒ the recipe (§24.25.11) is over-optimistic on two counts:** (1) `enable+1` is net-negative (99 < committed 104) and irrecoverable for the write ROMs; (2) the `lead_ly()`-only reframe is INSUFFICIENT — it fixes the readback observable but NOT the dispatch/HALT-wake phase, which is what both `intr_2_sprites` (blank) and the write ROMs (steady) actually turn on. The grounding "self.ly already == raster_ly so self.ly-lead is churn" holds for the READBACK but NOT for the DISPATCH (the mode2 pretrigger / HALT-wake reads live `self.ly`/`line_dot`; a genuine mid-line `self.ly` lead WOULD shift the dispatch boundary, which `lead_ly` does not).

**The three remaining paths to a canon-clean 105 (a genuine fork — the recipe's bounded path is dead):**
1. **Bounded blank-frame HALT-wake defer for `intr_2_sprites`** at clean-104 (extend the existing `dmg_mode2_oam_halt_wake_deferred` family, like batches 1-4). Cheapest (104→105, write ROMs stay green, no `enable+1`). COST: it is the dispatch-defer the recipe/§24.25.11 flagged as "ADDS a seam" — so the net manual-seam count may RISE by 1 unless it can be framed as extending the existing halt-wake-deferred family (batches 1-4 precedent: "net seam count does not rise"). Needs the canon self-audit.
2. **Proper Cut 1 (heavy):** make `self.ly` genuinely lead mid-line (DocBoy dot-453 increment) + re-anchor every mode/IRQ boundary to `line_dot` (§24.18 groups 1-6, ~114 sites + a save-state flag). This makes the internal PPU phase uniformly main-equivalent so the DISPATCH (not just readback) is correct WITHOUT `enable+1`'s frame shift — closing `intr_2_sprites` AND keeping the write ROMs green. This is the canon-clean end-state, but it is the multi-session core-raster restructure §24.25.11 explicitly tried to avoid via `lead_ly` (now shown necessary for the dispatch half).
3. **Hold the committed 104/105 interim** (the readback-only un-shift, net +29 cross-suite); the lone `intr_2_sprites` fail recorded as dispatch debt.

**STATE: tree reverted to committed clean 104/105.** The readback half (85→99 reproducible cut) is recorded in §24.25.12 and survives into the Cut-1 end-state regardless. The `enable+1` line of attack is now fully exhausted (refuted at ≤94 by §24.25.10 and at 99 here). RECOMMENDATION: path 2 (proper Cut 1) is the only one that reaches a canon-clean 105; path 1 is the pragmatic 105 if a +1 seam is acceptable; the `lead_ly`-only reframe (§24.25.11) should be downgraded from "the means" to "the readback half of the means".

#### 24.25.14 PATH 1 LANDED — clean 105/105 via the halt-wake-deferral family extension; canon gate PASSES (it is the readback un-shift's parallel, not a new mechanism) (2026-06-18)

User chose Path 1. Executed and verified to a clean 105/105.

**Firsthand diagnosis re-confirmed (differential trace `intr_2_mode0_timing_sprites` DMG, branch vs `../gb-cycle-main`, `GB_TRACE_NOHALT=1`).** The mode2 STAT IF is set at the SAME wall-clock cycle on both (`@604975`, blank-frame pretrigger lead 3-vs-4 already aligns the IF-SET). But the branch's blank-frame raster leads `main` by +1 (dot 453 vs 452 @604975), so it reaches the wrap to ly68/dot0 one cycle early (`@604978` vs `@604979`) → `dmg_mode2_oam_halt_wake_deferred` (which holds during the pretrigger window {len-3,len-2,len-1} and RELEASES at the wrap dot0) released one wall-clock cycle early → the woken handler's measuring FF41 read landed at `@605242` reading mode3 `0xA3` (`main` `@605243` mode0 `0xA0`). The deferral's window START was already aligned (lead 4→3) but its RELEASE (at the wrap) was not — the +1 raster lead makes the wrap one cycle early.

**Fix (1 predicate body, no new mechanism): extend `dmg_mode2_oam_halt_wake_deferred` to hold through the wrap dot in the blank frame** (`line_dot == 0 && (1..VISIBLE_SCANLINES).contains(&ly) && mode2-enabled && lcd-enabled`), so the wake dispatches at `main`'s wall-clock cycle (releases at dot1, not dot0). Post-fix trace: branch dispatches `@604979` (ly68 dot1) and reads `0xA0` (mode0) `@605243` — byte-identical to `main`; the branch trace went 600→1961 lines (it now follows `main`'s post-pass execution path instead of diverging on the failure).

**CANON GATE — PASSES (self-audit: do we follow the DocBoy/SameBoy canon to avoid manual tables/seams?).** This is the `line_dot - 1` blank-frame correction the readback un-shift (`current_published_stat_line_dot`, §24.25.8) ALREADY applies, now extended to the parallel HALT-wake-DISPATCH path — the deferral release and the readback reference were ASYMMETRIC (readback corrected for the +1, the deferral release did not); the change removes that asymmetry. It is the batch-4 pattern exactly ("removes an asymmetry by extending an EXISTING blank-frame compensation to a parallel source; not a new manual table/constant; net manual-seam count does not rise"). Concretely: **NO new function/predicate, NO new constant (the one-dot shift is the same +1 blank lead the un-shift already corrects), NO new table; only the body of the existing predicate widened.** §245 (`current_mode0_start_dot` + sprite penalties) untouched; no `enable+1`; no readback change. §24.25.11's "a bounded extension ADDS a seam → excluded" was a conservative pre-judgment of a NAIVE per-ROM defer; the asymmetry-removal framing §24.25.13 path 1 flagged as the exception is what was implemented. **Evidence it is NOT a curve-fit:** the same fix RECOVERED two independent gbmicrotest ROMs (`ppu-int-oam-halt`, `ppu-oam-int-halt-b`) — a one-ROM curve-fit would not generalise; it corrects the general "blank-frame mode2-OAM HALT-wake releases one dot early" dispatch bug. **Net debt FALLS** (one ROM closed in two suites + two gbmicrotest recovered + one lib unit test re-grounded green). This remains INTERIM debt: Cut 1 (§24.18) deletes this wrap-hold with the rest of the blank compensations once the +1 offset is removed at source.

**Verification (all green, differential vs committed clean-104 where counts could move):**
- `wilbertpol-acceptance` **105/105** (closes `intr_2_mode0_timing_sprites`; 5 write ROMs + all else stay green).
- `mooneye-acceptance` 74/75 (closes `ppu-intr-2-mode0-timing-sprites`; `boot-hwio-dmg0` stays red = pre-existing §24.23, NOT a regression).
- `blargg` 58/58.
- Cross-suite fail-list differential (committed vs fixed binaries), ZERO regressions: `docboy-dmg` 1518 (identical set), `gbmicrotest` 395→**397** (+2 recovered, 0 regressed), `mealybug-tearoom-tests` DMG 8/24 (identical), `mealybug-tearoom-tests-cgb` 11/24 (identical), `samesuite-dmg` 3/3, `little-things-gb-dmg` 2/2, `docboy-cgb-dmg` 72/467 (identical). CGB suites provably unaffected (`is_dmg_family()` gate).
- `cargo fmt-check` + `cargo lint` clean; `cargo test --workspace` 1530 passed / 7 failed (the 7 are the pre-existing reorder-debt lib reds; committed had 8 — the `mode2_stat_pretrigger_defers_halt_wake_only_during_reenable_blank_frame` test was re-grounded to the current blank lead-3 window + the new wrap-hold and now PASSES; the other 7 are byte-identical to committed, none touched by this change).

**STATE: working tree carries the Path-1 fix (irq.rs predicate body + the one re-grounded unit test); not committed (awaiting user).** The §24.18 proper Cut 1 remains the canon-clean end-state that would delete this and the rest of the blank-frame compensations at source; Path 1 is the landed pragmatic 105/105 with net debt reduced and no new seam mechanism.
