...
- The v1 topology deliberately covers only native CGB-to-CGB: peer emitter OR own emitter feeds the sensor, emitter routing is sampled during `ExternalEventIngress`, DMG and CGB compatibility mode continue to read `$FF`, and no desktop UI, netplay, host light source, or external accessory protocol is introduced.
