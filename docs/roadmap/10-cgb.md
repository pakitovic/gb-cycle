...
- Status note: Slice `7B` adds the repo-managed `cgb-audio-samesuite` manifest, source hashes, RGB555 fixtures, and standalone `make run-cgb-audio-samesuite` target for all `61` upstream SameSuite APU rows in roadmap order; the target is intentionally not part of `make test-roms-cgb` yet because the current known promoted status is `11/61` green after fixing the shared CH1/CH2 align, align_cpu, and duty rows, and the next coarse blocker is shared pulse/envelope volume timing visible in `channel_1_volume` and `channel_2_volume`, while the new internal `DIV`/APU tests now prove natural edges, `DIV` writes, shared CH1/CH2/CH3/CH4 length fanout, and double-speed frame-sequencer routing through the Slice 2 event path.
