...
- The Makefile must maintain an aggregate `test-roms-cgb` target alongside the existing DMG `test-roms` target; green blocking `run-cgb-*` suite targets should be added to `test-roms-cgb` in the slice that promotes them, while explicitly red exploratory suites may remain standalone until their expected PASS/INFO/FAIL meaning is documented and the aggregate can stay useful as closure evidence.
...
- Status note: Slice `7B` adds the repo-managed `cgb-audio-samesuite` manifest, source hashes, RGB555 fixtures, and standalone `make run-cgb-audio-samesuite` target for all `61` upstream SameSuite APU rows in roadmap order; the target is intentionally not part of `make test-roms-cgb` yet because the current promoted contiguous status is `59/61` in roadmap order after fixing the shared CH1/CH2 align, align_cpu, duty, volume, normal-speed delay, duty-delay, frequency-change, active-restart, DAC-stop-restart, CH1 sweep/restart rows through `channel_1_sweep_restart_2`, and the shared DIV-trigger rows through `div_trigger_volume_10`, the CH3 inactive-fetch/startup buffer plus `channel_3_restart_delay` and `channel_3_stop_delay` behavior, and the CH4 PCM-visible LFSR/output/timing path for `channel_4_align`, `channel_4_lfsr`, `channel_4_lfsr15`, `channel_4_equivalent_frequencies`, `channel_4_delay`, `channel_4_freq_change`, `channel_4_frequency_alignment`, and `channel_4_lfsr_restart`. The latest full aggregate after the CGB NRx2-glitch promotion is `59/61` green overall with `2` known framebuffer mismatches, and contiguous roadmap closure is now `59/61` because `channel_1_nrx2_speed_change` is the first failing row. The CH1/CH2 volume promotion locks the native-CGB NRx2 live-write matrix in the shared envelope primitive, the CH1/CH2 delay promotion keeps normal-speed inactive pulse startup at a fixed `8` scheduler T-cycle delay while preserving the double-speed hidden-phase path, the CH1/CH2 duty-delay promotion latches live `NR11`/`NR21` duty writes and applies them only when the current pulse sample finishes at the next duty-step boundary, the CH1/CH2 restart promotion keeps active pulse retriggers in the shared pulse primitive with the current duty-step phase preserved, period timer reset, no renewed inactive-start suppression, and a short native-CGB restart delay before the next duty-step advance, the CH1/CH2 DAC-stop promotion freezes the shared pulse generator after `NR12`/`NR22` disables the DAC until the next real trigger while preserving sample phase, the CH1 delayed sweep promotion keeps native-CGB trigger-time and post-writeback overflow checks in a save-state-visible sweep pipeline instead of disabling CH1 in the same frame-sequencer instant, the CH1 sweep-restart promotion adds the native-CGB decreasing-writeback restart hold that extends only the next active CH1 retrigger by `4` scheduler T-cycles while preserving the base increasing-sweep path, the CH1 sweep-restart-2 promotion adds a save-state-visible native-CGB shift-zero restart hold that suppresses the immediate `0x7FF` overflow boundary for `9` scheduler T-cycles and then resolves the unshifted delayed overflow check after `4` scheduler T-cycles, the DIV-write-trigger promotion feeds runtime `NR52` power-on from the timer-owned current `DIV-APU` signal so power-on during the high half starts on frame-sequencer phase `7`, matching both length and envelope timing without creating an APU timer fork, the CGB NRx2-glitch promotion keeps the CH1/CH2 live-write zombie matrix and subsequent envelope clock-state update in the shared envelope primitive with save-state coverage for the pending short reload, the CH3 promotion keeps inactive CH3 from preloading the APU power-on-cleared sample buffer before `NR34` trigger, and the CH4 frequency-change promotion splits the hidden-counter `NR43` live-write profile so DMG keeps the SameBoy-guided pre-`CGB-D` staged `FF` path while CGB uses the direct CGB-family path and CGB reload-alignment offsets required by SameSuite. The next first failing roadmap row is `channel_1_nrx2_speed_change`, while the internal `DIV`/APU tests continue to prove natural edges, `DIV` writes, shared CH1/CH2/CH3/CH4 length fanout, and double-speed frame-sequencer routing through the Slice 2 event path.
...
- Every CGB PR or slice must run `make ci`, `make test-roms`, `make test-roms-cgb`, and the relevant `run-cgb-*` Makefile target for that slice after the target has fetched or verified its declared external ROM families; slices that touch boot handoff or startup contracts must also run `make test-roms-cgb-real-boot` locally with verified private CGB boot ROM assets.
