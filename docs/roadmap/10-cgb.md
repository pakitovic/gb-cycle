...
| 4 | `cgb-ppu-basic` | 4 | hacktix | `bully.gb (GBC)` | PPU / visible VRAM seed | HIGH |
...
- Define the CGB compatibility palette seed contract before promoting compatibility-mode visual gates: `SkipBoot` must use the same standard CGB boot palette-selection algorithm as `RealBoot` for Nintendo-licensee detection, title checksum lookup, fourth-letter correction, palette ID `0` fallback, and the runner-controlled boot-input override seam; the manifest should retain the resolved palette ID and the resulting BG/OBJ compatibility palette bytes for representative cases.
...
- Status note: Slice `4A` palette-MMIO baseline is implemented for native CGB, Slice `4B` latches BG/window tile-map attributes from VRAM bank `1`, Slice `4C` consumes CGB OBJ attributes from live Mode `3` OAM metadata, Slice `4D` exposes a parallel raw logical RGB555 framebuffer that looks up native CGB BG and OBJ palette RAM through the latched BG/OBJ palette-index sidebands while preserving OBJ color index `0` transparency and the existing grayscale framebuffer, and Slice `4E` implements the native CGB priority composer plus boot-latched OBJ priority mode and `OPRI` latch/readback baseline. Remaining Slice `4` work still owns compatibility palette seeding and routing, focused visual probes, and eventual `cgb-ppu-basic` promotion.
