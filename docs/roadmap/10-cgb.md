...
- Status note: Slice `7B` adds the repo-managed `cgb-audio-samesuite` manifest, source hashes, RGB555 fixtures, and standalone `make run-cgb-audio-samesuite` target for all `61` upstream SameSuite APU rows in roadmap order; the target is intentionally not part of `make test-roms-cgb` yet because the current promoted contiguous status is `28/61` in roadmap order after fixing the shared CH1/CH2 align, align_cpu, duty, volume, normal-speed delay, duty-delay, frequency-change, and active-restart rows through `channel_1_restart` / `channel_2_restart`, the CH3 inactive-fetch/startup buffer plus `channel_3_restart_delay` behavior, and the CH4 PCM-visible LFSR/output/timing path for `channel_4_align`, `channel_4_lfsr`, `channel_4_lfsr15`, `channel_4_equivalent_frequencies`, `channel_4_delay`, `channel_4_freq_change`, `channel_4_frequency_alignment`, and `channel_4_lfsr_restart`. The latest full aggregate after the active pulse-restart promotion is `49/61` green overall with `12` known framebuffer mismatches, but contiguous roadmap closure remains `28/61` because `channel_1_stop_restart` is the first failing row. The CH1/CH2 volume promotion locks the native-CGB NRx2 live-write matrix in the shared envelope primitive, the CH1/CH2 delay promotion keeps normal-speed inactive pulse startup at a fixed `8` scheduler T-cycle delay while preserving the double-speed hidden-phase path, the CH1/CH2 duty-delay promotion latches live `NR11`/`NR21` duty writes and applies them only when the current pulse sample finishes at the next duty-step boundary, the CH1/CH2 restart promotion keeps active pulse retriggers in the shared pulse primitive with the current duty-step phase preserved, period timer reset, no renewed inactive-start suppression, and a short native-CGB restart delay before the next duty-step advance, the CH3 promotion keeps inactive CH3 from preloading the APU power-on-cleared sample buffer before `NR34` trigger, and the CH4 frequency-change promotion splits the hidden-counter `NR43` live-write profile so DMG keeps the SameBoy-guided pre-`CGB-D` staged `FF` path while CGB uses the direct CGB-family path and CGB reload-alignment offsets required by SameSuite. The next first failing roadmap row is `channel_1_stop_restart`, while the internal `DIV`/APU tests continue to prove natural edges, `DIV` writes, shared CH1/CH2/CH3/CH4 length fanout, and double-speed frame-sequencer routing through the Slice 2 event path.
