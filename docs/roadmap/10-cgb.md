...
- The extra suite `docboy-cgb-dmg-extra` validates DocBoy `tests/config/cgb_dmg_mode.json` in CGB-family GB-compatible mode: at the pinned DocBoy commit it contains 505 JSON entries, one exact duplicate `docboy/boot/boot_vram.gb`, and one upstream-disabled Mealybug row, so gb-cycle materializes 504 manifest rows and runs 503 cases under `console = "cgb"`; rows default to strict unless they require the experimental KEY0 bit-2 compatibility-mode policy, currently `docboy/mode/mode_cgb_flag_84.gb`, `docboy/mode/mode_cgb_flag_85.gb`, `docboy/mode/mode_cgb_flag_86.gb`, and `docboy/mode/mode_cgb_flag_87.gb`. It is available through `make run-docboy-cgb-dmg` and included only in `make test-roms-docboy`, not the promoted `make test-roms-cgb` or non-DocBoy `make test-roms-cgb-extra` gates. Initial local evidence on 2026-05-20 is `71/503` passing, so this suite is a red bring-up signal rather than a closure gate until CGB GB-compatible startup, audio, boot-register, and PPU differences are resolved intentionally.
