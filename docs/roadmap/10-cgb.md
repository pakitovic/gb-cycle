...
- Preserve the current DMG suite at its `167/167` baseline as a mandatory gate after every CGB slice; the persisted `.roms/test/test-report.md` may count promoted CGB rows after `test-roms-cgb` runs and `.roms/test/test-report-extra.md` may count internal rows after `test-roms-extra` or `test-roms-cgb-extra` runs, but neither report may redefine the DMG gate.
...
- The Makefile must maintain aggregate `test-roms-cgb` and `test-roms-cgb-extra` targets alongside the existing DMG `test-roms` target; green blocking `run-cgb-*` suite targets should be added to `test-roms-cgb` in the slice that promotes them, while extra/internal suites such as `cgb-boot-hwio` should live under `test-roms-cgb-extra` until their expected PASS/INFO/FAIL meaning is documented and the promoted aggregate can stay useful as closure evidence.
...
- The extra suite `docboy-cgb-dmg-extra` validates DocBoy `tests/config/cgb_dmg_mode.json` in CGB-family GB-compatible mode: at the pinned DocBoy commit it contains 505 JSON entries, one exact duplicate `docboy/boot/boot_vram.gb`, and one upstream-disabled Mealybug row, so gb-cycle materializes 504 manifest rows and runs 503 cases under `console = "cgb"` with `execution_mode = "experimental"`; it is available through `make run-docboy-cgb-dmg` and included only in `make test-roms-cgb-extra`, not the promoted `make test-roms-cgb` gate. Initial local evidence on 2026-05-20 is `71/503` passing, so this suite is a red bring-up signal rather than a closure gate until CGB GB-compatible startup, audio, boot-register, and PPU differences are resolved intentionally.
...
- Treat `make test-roms` as the DMG gate even when the persisted reports also contain rows from earlier CGB runs; promoted CGB suite rows are produced by `make test-roms-cgb` or specific promoted `run-cgb-*` targets, while extra/internal CGB rows are produced by `make test-roms-cgb-extra` or their specific `run-cgb-*` targets and render to `test-report-extra.md`.
