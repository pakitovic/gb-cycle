...
- Preserve the current DMG suite at its `167/167` baseline as a mandatory gate after every CGB slice; the persisted `.roms/test/test-report.md` may count promoted CGB rows after `test-roms-cgb` runs, `.roms/test/test-report-extra.md` may count non-DocBoy internal rows after `test-roms-extra` or `test-roms-cgb-extra` runs, and `.roms/test/test-report-docboy.md` may count large DocBoy exploratory rows after `test-roms-docboy` runs, but none of those reports may redefine the DMG gate.
...
- CGB suite materialization must reuse the existing curated-store pipeline: source selection filters by family, required files are verified before copying, missing or hash-mismatched files fail early, `.roms/test/<family>/...` remains the runner-facing layout, and `.roms/test/.status/<suite>.toml` plus `.roms/test/test-report.md` for promoted suites, `.roms/test/test-report-extra.md` for non-DocBoy extra/internal suites, or `.roms/test/test-report-docboy.md` for large DocBoy exploratory suites remain the persisted status/report outputs.
...
- Each suite needs a Makefile target with the `run-cgb-<suite-suffix>` naming pattern, for example `run-cgb-smoke`; the target must fetch or verify the required upstream families first through `make fetch-test-roms FAMILIES="..."`, then invoke `cargo run --release -q -p gb-test-runner --bin run_rom_suite -- --suite <suite> --failure-artifact-root .artifacts/<suite>` so the command used for slice closure is stable and reviewable.
- The Makefile must maintain aggregate `test-roms-cgb`, `test-roms-cgb-extra`, and `test-roms-docboy` targets alongside the existing DMG `test-roms` target; green blocking `run-cgb-*` suite targets should be added to `test-roms-cgb` in the slice that promotes them, non-DocBoy extra/internal suites such as `cgb-boot-hwio`, `mooneye-cgb-extra`, `samesuite-cgb-extra`, `magen-cgb-extra`, and `little-things-gb-cgb-extra` should live under `test-roms-cgb-extra` with collect-and-continue semantics for red bring-up rows, and large DocBoy exploratory suites should live under `test-roms-docboy` until their expected PASS/INFO/FAIL meaning is documented and the promoted aggregate can stay useful as closure evidence.
...
| `cgb-boot-hwio` | Mooneye pass/fail signature for `misc/boot_hwio-C.gb` plus retained HWIO snapshot on failure | Serial pass/fail signature, HWIO snapshot artifact, default SkipBoot handoff trace, optional local RealBoot comparison artifact, and manifest-recorded expected CGB compatibility-mode readbacks |
...
- Keep `cgb0_boot.bin` and `cgbE_boot.bin` out of the standard CGB default: they may be hash-checked and listed in manifests, and `cgbE_boot.bin` may be paired with the explicit CGB-E hardware-revision axis for revision-specific validation, but ordinary Phase 10 behavior remains the CGB-C / `cgb_boot.bin` profile.
...
| `cgbE_boot.bin` | CGB-E boot ROM | `2304` bytes | `c56299bedd56debdbf36442238636bf5887a65c5173b33995682052353804da9` | Explicitly selectable CGB-E revision asset; not a Phase 10 default gate |
...
- Add `cgb-boot-hwio` as the extra/internal Mooneye HWIO check for this slice; use `misc/boot_hwio-C.gb` when available, keep it out of the promoted CGB aggregate, render it in `.roms/test/test-report-extra.md`, use the Mooneye pass/fail signature now that the CGB compatibility-mode HWIO readbacks are documented, and use it to detect divergence between the centralized `SkipBoot` handoff and optional local `RealBoot` comparison runs.
- CGB gate: `cgb-smoke` through `make run-cgb-smoke` and `cgb-boot-div` through `make run-cgb-boot-div` against the centralized CGB `SkipBoot` handoff so the default CGB aggregate and CI do not require boot ROM assets; extra CGB evidence runs `cgb-boot-hwio` and `mooneye-cgb-extra` through `make test-roms-cgb-extra`, while optional local `make test-roms-cgb-real-boot` and `make test-roms-cgb-extra-real-boot` rerun the promoted and extra CGB aggregates through verified CGB RealBoot for comparison evidence only.
- Regression gate: existing real boot DMG0/DMG/MGB coverage plus DMG `167/167`.
- Status: Slice `6` implementation locks the standard `cgb_boot.bin` path as the Phase 10 CGB `RealBoot` default with strict `2304`-byte SHA verification, keeps `cgb0_boot.bin` as a recognized future revision asset and `cgbE_boot.bin` as a revision-derived CGB-E asset, hardens split CGB boot windows plus cartridge-visible `0100-01FF`, seeds CGB RealBoot timer phase so canonical firmware preserves the header-dependent handoff counters used by direct-start validation buckets (`0x2674` for the `cgb-boot-div` DMG-compatible baseline, `0x1E84` for the Hacktix Bully native CGB non-Nintendo bucket, and the `0x1E98` native old-licensee `$33` plus binary-zero new-licensee bucket exposed by Nitro2k01 `whichboot.gb` through a narrow `24` T-cycle timer/raster handoff correction), routes explicit `FF50` newly-unmapped handoff signals, locks boot-written `KEY0`, applies boot-selected native/compatible mode to machine/bus/speed/PPU at the handoff boundary, aligns centralized CGB `SkipBoot` with the RealBoot handoff snapshot including `P1=$FF`, compatibility-mode CGB HWIO readbacks, and compatibility palette index readbacks, adds the extra/internal `cgb-boot-hwio` suite as a Mooneye pass/fail gate inside the extra report/aggregate, and adds local RealBoot aggregates that force the promoted and extra CGB suite groups through verified CGB RealBoot without synthetic startup-memory or startup timer state.
...
- Status note: Slice `8` adds the manifest-backed `cgb-rtc` suite with AX6 `rtc3test-1.gb`, `rtc3test-2.gb`, and `rtc3test-3.gb`, pinned source/fixture hashes in `sources.toml`, committed RGB555 fixtures, and a standalone `make run-cgb-rtc` target that fetches `ax6`. The core keeps MBC3 RTC subsecond phase explicit, advances it through a deterministic runner-injected `32.768 kHz` clock, preserves Pan Docs `DH.bit6 = 1` halt polarity, keeps standard MBC3 selectors `0x04..=0x07` reserved, advisory `rtc_access_ready_at`, and the compatibility relatch rule, and adds CGB-mode mapper regressions for MBC1/MBC2/MBC3/MBC5. Post-Slice `9`, MBC30 is implemented as the explicit MBC3-family `4 MiB` ROM / `64 KiB` SRAM variant, MBC6 now has dedicated split-window/flash runtime coverage plus the repo-owned synthetic `phase-6-mbc6-oracle`, and MBC7 is implemented as a dedicated sensor / serial-EEPROM cartridge without invented runtime rumble.
...
- The extra suite `docboy-cgb-extra` validates DocBoy `tests/config/cgb.json` in native CGB mode: at the pinned DocBoy commit gb-cycle materializes 6815 manifest rows from `tests/roms/cgb`, skips the one upstream-disabled `daid/ppu_scanline_bgp.gbc` row that is absent from the checkout, keeps the remaining 643 upstream-disabled rows visible but non-runnable, intentionally excludes duplicate Blargg `cgb_sound` rows already owned by `cgb-audio-blargg` plus Daid speed-switch / STOP rows already owned by `cgb-speed`, Acid `cgb-acid2.gbc` already owned by `cgb-ppu-basic`, Little Things `whichboot.gb` CGB custom-boot coverage split into `little-things-gb-cgb-extra`, and SameSuite rows either promoted into `cgb-audio-samesuite` / `cgb-ppu-basic` or split into `samesuite-cgb-extra` / `magen-cgb-extra`, and runs 6172 cases under `console = "cgb"` with the default strict execution mode because this suite exercises ordinary native CGB rather than the experimental CGB DMG-ext policy; it is available through `make run-docboy-cgb` and included only in `make test-roms-docboy`, not the promoted `make test-roms-cgb` or non-DocBoy `make test-roms-cgb-extra` gates.
- The extra suite `samesuite-cgb-extra` carries the 10 DocBoy-sourced SameSuite CGB-only APU variant rows removed from `docboy-cgb-extra`: it reports as logical family `samesuite`, materializes under `/.roms/test/samesuite/`, uses `console = "cgb"`, `report_model_suffix = true`, `timeout_frames = 180`, and RGB555 fixtures under `crates/gb-test-runner/data/fixtures/samesuite-cgb/`, selects `revision = "cpu-cgb-d"` for the `*-cgbDE` timing row and `revision = "cpu-cgb-e"` for `*-cgbE` rows, keeps revision behavior reproducible through `gb-cli --revision` / `gb-desktop --revision` instead of runner-only overlays, and is included in `make test-roms-cgb-extra` / `make test-roms-cgb-extra-real-boot` rather than in the promoted `make test-roms-cgb` or the large DocBoy report.
- The extra suite `magen-cgb-extra` carries the eight DocBoy-sourced Magen native-CGB rows removed from `docboy-cgb-extra`: it reports as logical family `magen`, materializes under `/.roms/test/magen/`, uses `console = "cgb"`, uses fixed-time RGB555 fixture oracles with `timeout_tcycles = 5000000`, and uses fixtures under `crates/gb-test-runner/data/fixtures/magen/`; it is included in `make test-roms-cgb-extra` / `make test-roms-cgb-extra-real-boot` rather than in the promoted `make test-roms-cgb` or the large DocBoy report.
- The extra suite `mooneye-cgb-extra` carries the upstream Mooneye acceptance PPU rows that match the DocBoy CGB-DMG Mooneye PPU subset plus `intr_2_mode0_timing.gb`: it reports as logical family `mooneye`, materializes 12 manifest rows under `/.roms/test/mooneye/`, runs 10 active rows after disabling the expected-CGB-red `lcdon_timing-GS.gb` and `vblank_stat_intr-GS.gb`, keeps `intr_2_mode0_timing_sprites.gb` active as CGB PPU/OBJ STAT-publication regression coverage, uses `console = "cgb"`, the `mooneye-result` oracle, and the default `SkipBoot` handoff, and is included in `make test-roms-cgb-extra` / `make test-roms-cgb-extra-real-boot` rather than in the promoted `make test-roms-cgb` or the large DocBoy report.
- The extra suite `little-things-gb-cgb-extra` carries the DocBoy-sourced CGB `whichboot.gb` row removed from `docboy-cgb-extra`: it reports as logical family `little-things-gb`, materializes under `/.roms/test/little-things-gb/`, uses `console = "cgb"`, `startup = "custom-boot"`, `timeout_frames = 180`, `report_model_suffix = true`, and the fixture under `crates/gb-test-runner/data/fixtures/little-things-gb-cgb/`; it is green in the default `CustomBoot` lane through the core CGB cartridge-entry timer/raster bucket, has local canonical `cgb_boot.bin` `RealBoot` evidence through `make test-roms-cgb-extra-real-boot`, and remains outside the promoted `make test-roms-cgb` and large DocBoy report.
- The extra suite `docboy-cgb-dmg-extra` validates DocBoy `tests/config/cgb_dmg_mode.json` in CGB-family GB-compatible mode: at the pinned DocBoy commit it contains 505 JSON entries, one exact duplicate `docboy/boot/boot_vram.gb`, two DocBoy-packaged Mooneye boot rows that are intentionally omitted because promoted `cgb-boot-div` / `cgb-smoke` own `boot_div-cgbABCDE.gb` and `boot_regs-cgb.gb`, 11 DocBoy-sourced Mooneye PPU duplicates now owned by `mooneye-cgb-extra`, and one upstream-disabled Mealybug row, so gb-cycle materializes 491 manifest rows and runs 490 cases under `console = "cgb"`; rows default to strict unless they require the experimental KEY0 bit-2 compatibility-mode policy, currently `docboy/mode/mode_cgb_flag_84.gb`, `docboy/mode/mode_cgb_flag_85.gb`, `docboy/mode/mode_cgb_flag_86.gb`, and `docboy/mode/mode_cgb_flag_87.gb`. It is available through `make run-docboy-cgb-dmg` and included only in `make test-roms-docboy`, not the promoted `make test-roms-cgb` or non-DocBoy `make test-roms-cgb-extra` gates. The adjusted local evidence after removing the two promoted-overlap green rows was `69/501` passing before those 11 Mooneye PPU rows moved out, so rerun this lane before quoting a current pass ratio; this suite remains a red bring-up signal rather than a closure gate until CGB GB-compatible startup, audio, boot-register, and PPU differences are resolved intentionally.
- The extra suite `docboy-cgb-dmg-ext-extra` validates the first implementation against DocBoy `tests/config/cgb_dmg_ext_mode.json` with 26 upstream `tests/roms/cgb_dmg_ext_mode/docboy/...` cases materialized without the redundant `docboy/` ROM prefix on `console = "cgb"` and the memory oracle `$FFF0 == $01` / fail-fast `$FFF0 == $02`; rows default to strict unless they require the CGB DMG-ext policy, currently `apu/pcm12_ch2_read.gb`, `hdma/gdma_basic_transfer.gb`, `hdma/hdma_basic_transfer.gb`, and `ppu/ocpd_write_read.gb`. It is available through `make run-docboy-cgb-dmg-ext` and included only in `make test-roms-docboy`, not the promoted `make test-roms-cgb` or non-DocBoy `make test-roms-cgb-extra` gates.
...
- Every CGB PR or slice must run `make ci`, `make test-roms`, `make test-roms-cgb`, and the relevant `run-cgb-*` or extra aggregate Makefile target for that slice after the target has fetched or verified its declared external ROM families; slices that touch boot handoff or startup contracts must also run `make test-roms-cgb-real-boot` and, when they affect an extra/internal boot suite, `make test-roms-cgb-extra-real-boot` locally with verified private CGB boot ROM assets.
...
- Treat `make test-roms` as the DMG gate even when the persisted reports also contain rows from earlier CGB runs; promoted CGB suite rows are produced by `make test-roms-cgb` or specific promoted `run-cgb-*` targets and render to `test-report.md`, non-DocBoy extra/internal CGB rows are produced by `make test-roms-cgb-extra` or their specific `run-cgb-*` targets and render to `test-report-extra.md`, and large DocBoy CGB rows are produced by `make test-roms-docboy` or their specific `run-docboy-*` targets and render to `test-report-docboy.md`.
...
- Use `docboy-test-suite` as a late exploratory GBC precision oracle through the dedicated `make test-roms-docboy` lane after the relevant promoted CGB suites are already green; it must not become a blocking Phase 10 gate until each case has an explicit manifest entry, acceptance channel, retained artifact model, and source-of-truth mapping back to Pan Docs/Gekkio/TCAGBD.
