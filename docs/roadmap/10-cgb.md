...
- Status note while Slice `2` is under implementation: the `cgb-speed` manifest and `make run-cgb-speed` target may mix promoted and exploratory cases during strict promotion. Daid `stop_instr.gb (GBC)` is promoted to a blocking absolute grayscale framebuffer fixture for final solid-black STOP output, `speed_switch_timing_div.gbc` is promoted to a blocking rank-normalized framebuffer fixture, and Blargg `interrupt_time.gb` is promoted to a blocking BG-map console-text `Passed` oracle; informational framebuffer outcomes in the remaining Daid rows are not sufficient to claim this slice is strictly closed.
