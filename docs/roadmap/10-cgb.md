...
- The extra suite `samesuite-cgb-extra` carries the 10 DocBoy-sourced SameSuite CGB-only APU variant rows removed from `docboy-cgb-extra`: it reports as logical family `samesuite`, materializes under `/.roms/test/samesuite/`, uses `console = "cgb"`, `report_model_suffix = true`, `timeout_frames = 180`, and RGB555 fixtures under `crates/gb-test-runner/data/fixtures/samesuite-cgb/`, and is included in `make test-roms-cgb-extra` / `make test-roms-cgb-extra-real-boot` rather than in the promoted `make test-roms-cgb` or the large DocBoy report.
- The extra suite `magen-cgb-extra` carries the eight DocBoy-sourced Magen native-CGB rows removed from `docboy-cgb-extra`: it reports as logical family `magen`, materializes under `/.roms/test/magen/`, uses `console = "cgb"`, preserves the original `timeout_tcycles = 5000000` until-match RGB555 oracles, and uses fixtures under `crates/gb-test-runner/data/fixtures/magen/`; it is included in `make test-roms-cgb-extra` / `make test-roms-cgb-extra-real-boot` rather than in the promoted `make test-roms-cgb` or the large DocBoy report.
