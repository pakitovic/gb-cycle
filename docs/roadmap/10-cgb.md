...
- Status note: Slice `4A` palette-MMIO baseline is implemented for native CGB and Slice `4B` now latches BG/window tile-map attributes from VRAM bank `1`, preserves ignored bit `4`, applies attribute VRAM-bank selection plus horizontal/vertical flips to the logical color-index path, and carries palette index plus BG priority as cached fetcher/FIFO sideband without full RGB555 rendering. Remaining Slice `4` work still owns CGB OBJ attribute consumption, RGB555 rendering, compatibility palette seeding, `LCDC.0` priority reinterpretation, `OPRI`, focused visual probes, and eventual `cgb-ppu-basic` promotion.
