...
- Status note: Slice `7B` adds the repo-managed `cgb-audio-samesuite` manifest, source hashes, RGB555 fixtures, and standalone `make run-cgb-audio-samesuite` target for all `61` upstream SameSuite APU rows in roadmap order; the target is intentionally not part of `make test-roms-cgb` yet because the current promoted contiguous status is `13/61` in roadmap order after fixing the shared CH1/CH2 align, align_cpu, duty, and volume rows, the CH3 inactive-fetch/startup buffer behavior, and the CH4 PCM-visible LFSR output polarity for `channel_4_align`, `channel_4_lfsr`, `channel_4_lfsr15`, and `channel_4_equivalent_frequencies`. The latest full aggregate before the CH4 polarity promotion was `29/61` green overall; refresh that aggregate with `make run-cgb-audio-samesuite` before using the total count as closure evidence. The CH1/CH2 volume promotion locks the native-CGB NRx2 live-write matrix in the shared envelope primitive and scales the inactive pulse trigger startup delay through the Slice 2 speed-domain contract; the CH3 promotion keeps inactive CH3 from preloading the APU power-on-cleared sample buffer before `NR34` trigger, so SameSuite `channel_3_first_sample`, `channel_3_delay`, and later CH3 timing/shift/wave-RAM rows now observe the first real post-trigger wave fetch; the CH4 promotion makes the repo's zeroed/XNOR LFSR representation resolve digital volume when bit `0` is `1`, matching the PCM34-visible SameSuite oracle. The next first failing roadmap row is `channel_1_delay`, while the internal `DIV`/APU tests continue to prove natural edges, `DIV` writes, shared CH1/CH2/CH3/CH4 length fanout, and double-speed frame-sequencer routing through the Slice 2 event path.
