...
- Status note: Slice `4A` palette-MMIO baseline is implemented for native CGB, Slice `4B` latches BG/window tile-map attributes from VRAM bank `1`, Slice `4C` consumes CGB OBJ attributes from live Mode `3` OAM metadata, and Slice `4D` exposes a parallel raw logical RGB555 framebuffer that looks up native CGB BG and OBJ palette RAM through the latched BG/OBJ palette-index sidebands while preserving OBJ color index `0` transparency and the existing grayscale framebuffer. Remaining Slice `4` work still owns compatibility palette seeding and routing, `LCDC.0` priority reinterpretation, `OPRI`, final CGB BG/OBJ composition, focused visual probes, and eventual `cgb-ppu-basic` promotion.
