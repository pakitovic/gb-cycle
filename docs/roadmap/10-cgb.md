...
| `cgb-boot-hwio` | Typed post-boot HWIO snapshot, optionally cross-checked against Mooneye `misc/boot_hwio-C.gb` | HWIO snapshot artifact, default SkipBoot handoff trace, optional local RealBoot comparison artifact, and manifest-recorded expected values |
...
- Add `cgb-boot-hwio` as the exploratory/internal Mooneye HWIO check for this slice; use `misc/boot_hwio-C.gb` when available, keep it out of strict blocking until the manifest records expected values and artifacts, render it in `.roms/test/test-report-extra.md`, and use it to detect divergence between the centralized `SkipBoot` handoff and optional local `RealBoot` comparison runs.
