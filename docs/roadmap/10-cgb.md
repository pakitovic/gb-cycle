...
- Status note: Slice `4A` palette-MMIO baseline is implemented for native CGB, Slice `4B` latches BG/window tile-map attributes from VRAM bank `1`, and Slice `4C` now consumes CGB OBJ attributes from live Mode `3` OAM metadata, applies OBJ VRAM-bank selection plus horizontal/vertical flips to the logical color-index path, and carries OBJ palette index plus BG-over-OBJ priority as OBJ FIFO sideband without full RGB555 rendering. Remaining Slice `4` work still owns RGB555 palette lookup/output, compatibility palette seeding and routing, `LCDC.0` priority reinterpretation, `OPRI`, final CGB BG/OBJ composition, focused visual probes, and eventual `cgb-ppu-basic` promotion.
