...
- The Makefile must maintain aggregate `test-roms-cgb`, `test-roms-cgb-extra`, and `test-roms-docboy` targets alongside the existing DMG `test-roms` target; green blocking `run-cgb-*` suite targets should be added to `test-roms-cgb` in the slice that promotes them, non-DocBoy extra/internal suites such as `cgb-boot-hwio`, `samesuite-cgb-extra`, `magen-cgb-extra`, and `little-things-gb-cgb-extra` should live under `test-roms-cgb-extra` with collect-and-continue semantics for red bring-up rows, and large DocBoy exploratory suites should live under `test-roms-docboy` until their expected PASS/INFO/FAIL meaning is documented and the promoted aggregate can stay useful as closure evidence.
...
- CGB gate: `cgb-smoke` through `make run-cgb-smoke` and `cgb-boot-div` through `make run-cgb-boot-div` against the centralized CGB `SkipBoot` handoff so the default CGB aggregate and CI do not require boot ROM assets; extra CGB evidence runs `cgb-boot-hwio` through `make test-roms-cgb-extra`, while optional local `make test-roms-cgb-real-boot` and `make test-roms-cgb-extra-real-boot` rerun the promoted and extra CGB aggregates through verified CGB RealBoot for comparison evidence only.
...
- The extra suite `docboy-cgb-dmg-extra` validates DocBoy `tests/config/cgb_dmg_mode.json` in CGB-family GB-compatible mode: at the pinned DocBoy commit it contains 505 JSON entries, one exact duplicate `docboy/boot/boot_vram.gb`, two DocBoy-packaged Mooneye boot rows that are intentionally omitted because promoted `cgb-boot-div` / `cgb-smoke` own `boot_div-cgbABCDE.gb` and `boot_regs-cgb.gb`, and one upstream-disabled Mealybug row, so gb-cycle materializes 502 manifest rows and runs 501 cases under `console = "cgb"`; rows default to strict unless they require the experimental KEY0 bit-2 compatibility-mode policy, currently `docboy/mode/mode_cgb_flag_84.gb`, `docboy/mode/mode_cgb_flag_85.gb`, `docboy/mode/mode_cgb_flag_86.gb`, and `docboy/mode/mode_cgb_flag_87.gb`. It is available through `make run-docboy-cgb-dmg` and included only in `make test-roms-docboy`, not the promoted `make test-roms-cgb` or non-DocBoy `make test-roms-cgb-extra` gates. Adjusted local evidence after removing the two promoted-overlap green rows is `69/501` passing, so this suite is a red bring-up signal rather than a closure gate until CGB GB-compatible startup, audio, boot-register, and PPU differences are resolved intentionally.
