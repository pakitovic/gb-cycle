...
- CGB-to-CGB v1 is now the first post-Slice 10 IR increment: `RP` / `FF56` lives in dedicated CGB infrared state, native-CGB reads expose bit `1 = 0` only when `$C0` read-enable is latched and the deterministic sensor sees effective light, the sensor advances per scheduler T-cycle with provisional warmup/fade constants cross-checked against SameBoy and GBE+, and linked sessions can attach an exact two-machine `cgb-ir` optical pair.
