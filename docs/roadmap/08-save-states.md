...
- `gb-desktop` exposes root-menu actions immediately below `OPEN RECENT`: `SAVE STATE`, `LOAD STATE`, and `STATE SLOT N`. The slot selector is runtime-only and cycles through slots `1` through `4` without moving the menu selection away from `STATE SLOT N`; `LOAD STATE` stays visible but disabled until the selected ROM-related slot file exists.
