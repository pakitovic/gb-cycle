...
- Phase 8 is considered closed for the strict single-machine save-state and rewind scope: durable `.gbstate` v3 DTOs, CLI/desktop `.gbstate` save/load, desktop slot/autoload UX, core rewind buffering, desktop hold-to-rewind, and restore-continuation validation are all in place.
