...
- The `.gbstate` host integration boundary is now format version `6`; this local-development v6 includes the core-visible `HardwareRevision` axis plus the Slice 0/1/2/3/4 `SgbHost` payload, including startup/acceptance, JOYP packet transport state, SGB palette/LCD composition state, pending and completed 4 KiB `_TRN` transfer state, border tile/tilemap/palette state, `MASK_EN` mask/freeze state, active SGB attribute map, packed ATF memory, system palette memory, and `PAL_PRI` state, and intentionally does not migrate or accept older incompatible slot files. Rewind continues to store in-memory `MachineSaveState` DTOs directly and does not depend on the host file version. Phase 8.8 reused the same host integration boundary after the ROM-less cartridge DTO update.
