...
- The `.gbstate` host integration boundary is now format version `5`; this local-development v5 includes the core-visible `HardwareRevision` axis plus the Slice 0/1/2/3 `SgbHost` payload, including startup/acceptance, JOYP packet transport state, SGB palette/LCD composition state, pending and completed 4 KiB `_TRN` transfer state, border tile/tilemap/palette state, and `MASK_EN` mask/freeze state, and intentionally does not migrate or accept older incompatible slot files. Rewind continues to store in-memory `MachineSaveState` DTOs directly and does not depend on the host file version. Phase 8.8 reused the same host integration boundary after the ROM-less cartridge DTO update.
