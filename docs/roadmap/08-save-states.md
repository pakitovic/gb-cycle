...
- `gb-persistence` owns the `.gbstate` envelope (`GBSTATE\0`, current version `3`, extension `.gbstate`) separately from `.gbsav`. Decode rejects unsupported versions, including the intentionally broken version `1` and superseded versions `2` and earlier, invalid magic, corrupt/truncated payloads, trailing bytes, unknown metadata tags, and envelope/payload metadata mismatches.
...
- Phase 8.5 originally did not change `.gbstate` v2, `MachineSaveState` DTOs, rewind buffering, compression, or deltas; Phase 8.8 supersedes the durable version with `.gbstate` v3 while keeping the same host integration boundary.
...
- `.gbstate` is bumped to version `3`, with the same `GBSTATE\0` magic and `.gbstate` extension. Versions `1` and `2` are rejected as unsupported rather than migrated, so old desktop slot files must be recreated.
- Rewind continues to store full `MachineSaveState` snapshots in memory, but `MachineSaveState::deep_size_bytes()` now scales with mutable cartridge payloads rather than cartridge ROM size. The v3 memory remeasure confirms that typical MBC1/MBC3 titles fit the default `10s` / `256 MiB` / `1` subframe-per-frame policy, while RAM-heavy `128 KiB` cartridges and Pocket Camera can use the existing `MEMORY 512MB` or `SUBFR OFF` desktop options when longer history is desired.
...
- The default Fast Forward hotkey is `Right Shift`. `SYSTEM -> F-FORWARD` owns the persistent policy with `F-FORWARD ON/OFF`, `SPEED 1x/2x/4x` (default `2x`), `DEFAULTS`, and `RETURN`; `ON/OFF` controls availability, while `Right Shift` and the mappable controller action are the momentary holds that activate Fast Forward when availability is `ON`.
...
- With no tracing, audio recording, printer output, rumble observer, or emulation profiling active, single-machine Fast Forward uses a lean `gb-desktop` stepping path backed by the core frame-origin helper: poll input/events, advance through `Machine::step_t_cycle()`, detect frame-origin crossings, flush cartridge saves at frame boundaries, and present only the final batch frame.
- Phase 8.9 also adds a core-only synthetic `frame_throughput` Criterion benchmark so Fast Forward speed targets can be compared without SDL, audio, rendering, or pacing noise; it is a core/frontend separation baseline, not a Super Mario Land 1 acceptance proxy.
