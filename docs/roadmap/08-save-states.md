...
- Phase 8 is considered closed for the strict single-machine save-state and rewind scope: durable `.gbstate` v2 DTOs, CLI/desktop `.gbstate` save/load, desktop slot/autoload UX, core rewind buffering, desktop hold-to-rewind, and restore-continuation validation are all in place.
...
- Compression, delta encoding, manual `.gbstate` file dialogs, and richer telemetry/settings remain optional follow-up ideas only if measured memory pressure or UX needs justify them.
