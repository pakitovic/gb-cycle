...
`--benchmark <path>` loads one portable benchmark TOML through `gb-benchmark` instead of taking a positional ROM path. The TOML owns the ROM path, console model, startup mode, compatibility mode, optional DMG grey palette, screenshot/stat toggles, and either one legacy duration/stimulus list or multiple fresh `[[run]]` entries. Each run starts from a new machine, applies expanded input pulses through `Machine::set_joypad_button_pressed`, uses its own duration as the final screenshot time, and writes `gb-cli/<artifact-id>.png` plus `gb-cli/<artifact-id>-stats.toml` when enabled. Concise run inputs use `[[run.input]]` with `button` or `buttons`, exactly one of `frame` / `second` / `tcycle`, optional `hold_frames`, and optional `repeat_every_frames`.
