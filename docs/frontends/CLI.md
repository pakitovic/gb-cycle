...
- `skip-boot` is the fast direct-start path for no-boot tooling: it uses the synthetic CPU/IO/hidden startup baseline, keeps plain PPU readbacks for Mooneye-style HWIO probes, and seeds the console-family boot-logo VRAM residue without loading a boot ROM asset. `custom-boot` remains the direct-start path for reset-facing ROMs: it starts from the same baseline and logo memory but also enables the DMG-family boot-facing PPU CPU-bus publication phase, and on CGB it applies the custom cartridge-entry raster phase.
...
- `--border-off` may be passed with any model so shared scripts do not need to branch by machine profile. It only has an effect with `--model SGB` or `--model SGB2`, where it hides the SGB host border for PNG framebuffer artifacts and writes the SGB-colored `160x144` LCD RGB555 output instead; for `DMG`, `MGB`, `LGB`, and `CGB` it is accepted and ignored. The default is border on, matching `gb-desktop` `CONFIG -> SYSTEM -> BORDER ON`.
