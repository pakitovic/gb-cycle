...
Both commands load the ROM first, then validate the save payload against the cartridge mapper and persistence profile instead of inferring compatibility from the filename. `saves export` remains the explicit legacy-migration path from an internal `.gbsav` envelope to an external `.sav`. `saves import` reads the selected external path exactly as provided, so extensionless files are valid when the caller supplies one, and then writes the same authoritative storage format that `run --save-dir` would use for the mapper: `.sav` when lossless externally, `.gbsav` only for fallback-only state. Use `--save-key <key>` when the save key differs from the ROM stem.
