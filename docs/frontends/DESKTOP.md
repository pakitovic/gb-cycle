...
Local `DMG-07` 4-Player Adapter sessions are selected from the overlay at runtime through `EXT. PORT -> 4P ADAPTER`; desktop intentionally does not add a separate CLI shortcut for 3P/4P startup.
...
- Local `DMG-04` uses the explicit `P2` keyboard profile for the secondary console. `DMG-07` reuses that `P2` profile and adds fixed keyboard profiles for `P3` and `P4`.
...
- `.gbstate` slots and rewind are disabled by design in local `DMG-04` 2-player Game Link and `DMG-07` 4-Player Adapter linked sessions; save states and rewind remain single-machine features.
