...
- `release-macos.yml` creates `gb-cycle-macos-aarch64.zip` on `macos-latest` and `gb-cycle-macos-x86_64.zip` on `macos-15-intel`, each containing `GB Cycle.app`, `README.md`, both license files, and `libSDL3*.dylib` inside `Contents/MacOS`; the dylib install names are rewritten to `@executable_path`, the app is ad-hoc signed after that rewrite, and the bundle reuses `crates/gb-desktop/macos/Info.plist` so the release artifact carries the same Pocket Camera usage string as the local macOS launcher.
