...
- `VIDEO -> BLEND` is a desktop-only LCD persistence simulator with `BLEND OFF` (default) and `BLEND ON`; `ON` gamma-blends the current RGB24 frame 50/50 with the previous raw frame. The experimental scanline-parity LCD mode was removed from the public menu because it introduced visible line artifacts in flicker-heavy games such as Wave Race and Zelda Link's Awakening; legacy settings or CLI values `simple` and `lcd` are accepted as aliases for `on`. The blend is applied after DMG palette or CGB RGB555 conversion and before HUD, rewind/Fast Forward indicators, and menu overlays; it never changes `gb-core` timing, framebuffers, save states, CLI oracles, screenshots, benchmark image exports, or host texture filtering.
