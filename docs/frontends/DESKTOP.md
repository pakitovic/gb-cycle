...
- `VIDEO -> BLEND` is a desktop-only LCD persistence simulator with `BLEND OFF` (default) and `BLEND ON`; `ON` gamma-blends the current RGB24 frame 50/50 with the previous raw frame. The experimental scanline-parity LCD mode was removed because it introduced visible line artifacts in flicker-heavy games such as Wave Race and Zelda Link's Awakening. The blend is applied after DMG palette or CGB RGB555 conversion and before HUD, rewind/Fast Forward indicators, and menu overlays; it never changes `gb-core` timing, framebuffers, save states, CLI oracles, screenshots, benchmark image exports, or host texture filtering.
...
- **`EXT. PORT`** — `NONE`, `PRINTER`, `GAME LINK`, and `4P ADAPTER`. The root label uses the full active selection name (`EXT NONE`, `EXT PRINTER`, `EXT GAME LINK`, or `EXT 4P ADAPTER`) for clarity. `GAME LINK` opens a `SAME GAME` / `SELECT GAME` submenu: `SAME GAME` rebuilds a fresh local `DMG-04` session with the already-loaded `P1` ROM in both slots, while `SELECT GAME` keeps the real two-cartridge `DMG-04` flow and asks for a second ROM. Each linked machine keeps a separate cartridge save slot, so same-ROM `P1` and `P2` sessions still use independent save files. `4P ADAPTER` opens a `2 PLAYERS` / `3 PLAYERS` / `4 PLAYERS` submenu; selecting a count rebuilds a fresh local `DMG-07` session and clones the already-loaded `P1` ROM into every adapter slot instead of opening more ROM dialogs.
