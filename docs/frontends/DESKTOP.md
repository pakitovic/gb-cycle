...
- **`CGB IR`** — root-level CGB-to-CGB infrared pairing, visible only when `SYSTEM -> MODEL GB COLOR` is selected. When off, it asks for a second ROM and rebuilds a fresh two-console native-CGB optical pair. When active, selecting `CGB IR ON` turns the pair off and returns to the single primary console without opening the ROM picker. It keeps serial / `EXT. PORT` attachments at `NONE`. This intentionally supports different cartridges in the two slots, such as Pokémon Gold / Silver / Crystal combinations, without treating IR as a cable-link mode.
