...
- The sampled `PPU` bucket is further split into `mode0_1`, `mode2`, `mode3_startup`, background fetch, window fetch/restart, BG push/fill, OBJ fetch, pixel transfer, PPU bus-sync boilerplate, PPU miscellaneous work, and a remaining `ppu_other` remainder so menu and HUD slowdowns can be narrowed to a specific raster phase without instrumenting the main thread.
