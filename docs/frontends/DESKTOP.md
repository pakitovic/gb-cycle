...
- `--benchmark <path>` to load one portable benchmark TOML through `gb-benchmark`; the file owns the ROM path, model, startup/mode, screenshot/stat artifact toggles, and one or more fresh `[[run]]` entries. Each run starts a new desktop session, expands concise `[[run.input]]` pulses/repeats into deterministic joypad button edges, uses its duration as the final screenshot time, and writes `gb-desktop/<artifact-id>.png` plus `gb-desktop/<artifact-id>-stats.toml` relative to the current benchmark directory when those toggles are enabled. The old top-level `duration_seconds` plus `[[stimulus]]` benchmark format is not accepted.
