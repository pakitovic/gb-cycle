...
- Keep the earliest-visible-sprite timing path explicit: when the first sprite fetch starts at the left edge of the visible region, the baseline currently exposes both the `8` OBJ fetch stall dots and the additional BG-startup starvation they cause instead of hiding that behavior behind a flat per-sprite penalty.
