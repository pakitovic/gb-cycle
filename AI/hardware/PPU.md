...
- The current provisional DMG window baseline should also keep the hidden-dot trigger path scoped to the real `WX = 0` case only. `WX = 7` and the deferred `WX = 166 -> next line` continuation should remain edge-visible trigger paths rather than being allowed to start during the hidden pre-visible transfer dots just because their effective trigger `X` is also `0`.
