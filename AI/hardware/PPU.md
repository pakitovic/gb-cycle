...
- In the current March 29, 2026 local refactor spike, the implementation is also exploring a stricter docboy-style startup seam where the discarded alignment fetch is treated as a real BG tile push while the placeholder count remains only an effective-consumption label. That direction is not closure-ready yet: current fallout is concentrated in `SCX discard`, early-left-edge raster timing, and `acid/dmg-acid2`, so keep that work isolated until the seam can be reconciled with the stable DMG baseline.
- The same March 29, 2026 spike has now established one more useful boundary: once the startup alignment seam is made explicit enough to move `fetch_x`, queue alignment-time occupancy, and optionally kick the next BG `TileIndex` immediately, the remaining big DMG failures (`m3_lcdc_bg_map_change`, `m3_lcdc_tile_sel_change`, `m3_scy_change`) do not improve just by retuning startup ownership. Treat that as evidence that the next closure work belongs to the fetch-phase live-write timing of the following visible BG tiles, not to another round of generic startup-placeholder heuristics.
- The same March 29, 2026 spike has now narrowed that live-write seam further too. Repo-local tracing against mealybug shows the relevant `FF40` / `FF42` writes landing while the second or later visible BG slice is already in `Push.0`, not during its original `TileIndex` read. Treat that as evidence that the remaining left-edge debt is not just "tilemap read too early" in isolation; it also depends on how long a fetched BG slice may wait cached in `Push` before it is finally enqueued into the BG FIFO.
- In the same current DMG spike, one useful experimental rule has emerged from that trace: when a background slice is still cached in `Push` during Mode `3`, a live `SCY` write or a live `LCDC.4` tile-data-selector write may need to revalidate the cached tile-data bytes before the slice finally reaches the FIFO. A localized pending-push tile-data refetch improves the still-open exploratory deltas (`m3_lcdc_tile_sel_change`, `m3_scy_change`) without reopening `acid/dmg-acid2`, whereas doing the same for cached tilemap/tile-index selection makes `m3_lcdc_bg_map_change` worse. Keep that seam explicit and isolated instead of broadening it into a generic "all MMIO writes re-fetch cached BG slices" rule.
- The same March 29, 2026 spike has now refined that cached-slice seam one level further. Repo-local tracing shows the still-open `m3_lcdc_bg_map_change` debt is not explained by a broad "any cached BG push may retarget on `LCDC.3`" rule; that broadening regresses the framebuffer oracle. The current best cut instead keeps the tilemap retarget path extremely narrow: it may reopen for a cached background slice during `Push` or `fill.pending`, but only on explicit same-T-cycle windows around the first cached post-startup slice. With that narrower window, repo-local PPU tests stay green, `acid/dmg-acid2.gb` stays green, and the exploratory `m3_lcdc_bg_map_change` delta drops further from `915` to `674` while `m3_lcdc_tile_sel_change`, `m3_lcdc_tile_sel_win_change`, and `m3_scy_change` stay at `1410`, `1232`, and `7819` respectively.
- The same March 29, 2026 local measurement pass has now added a stronger shape constraint for that remaining left-edge debt. Counting changed pixels by column in the current stable `m3_lcdc_bg_map_change = 674` cut shows the error is not uniform across `x=0..23`: columns `0..7` contribute about `30` pixels each, columns `8..15` about `38-58`, and columns `16..23` about `104-108` each. Treat that as evidence that the third visible BG tile after startup dominates the remaining `LCDC.3` debt more than the second tile does.
- In the same current DMG baseline, the post-alignment startup seam should therefore keep one explicit label for the second and third visible background tiles (`x=8..15` and `x=16..23`) instead of treating every post-startup background slice as the same generic "early left edge" case. The current repo baseline now carries that label as explicit internal state so later fetch/live-write work can target the third visible tile directly without reopening the older startup-placeholder heuristics.
- In the same current DMG baseline, a background slice that has already reached `Push` or `fill.pending` should own its own fetch metadata explicitly instead of continuing to borrow it from the still-live fetcher. The current repo baseline now keeps one cached-slice record across the `Push -> fill -> FIFO` seam, including source, left-edge continuation label, `fetch_x`, tilemap address, tile-data address, tile index, and the fetched bytes. That keeps later left-edge work focused on the cached slice itself rather than on ad hoc combinations of `fetcher.tile_*` plus side flags.
- In that same current baseline, the cached-slice record should also own the live-write machinery that still applies after the slice leaves the fetcher. The same-T-cycle tilemap-reread window, the pending `LCDC/SCY` reread flags, and the shared cached-slice reread recompute contract should remain attached to that cached slice instead of being split again between `write_register`, `Push`, and `fill.pending`.
- In that same current baseline, same-T-cycle refetch of a cached slice should remain a separate seam from the active dot's visible/pipeline register snapshots. The visible/pipeline snapshots still describe the dot currently being served by the LCD pipeline, but a slice already cached in `Push` or `fill.pending` is being reconsidered after a CPU MMIO write on the shared timeline. The repo baseline therefore keeps cached-slice refetch tied to the MMIO-owned storage values (`LCDC`, `SCY`, etc.) while still keeping the cached slice's own `fetch_x` and tile metadata explicit.
- In that same current baseline, the post-startup left-edge label should also flow through one explicit origin enum instead of a parallel `0/1/2` continuation index plus separate cached-slice flags. The startup seam, live fetcher, cached push slice, and cached fill slice should all speak in one `ordinary / alignment seed / alignment fill / visible tile 2 / visible tile 3` vocabulary so later left-edge timing work can target one origin seam instead of translating between duplicated labels.
