...
- In the same current baseline, the push phase should expose one explicit per-dot decision result instead of hiding all non-progress behind a boolean return. Entry-delay waiting, FIFO-space waiting, OBJ handoff, and queued-fill progression should remain distinguishable states so later stricter push policy can change the occupancy rule without rewriting the dot scheduler contract again.
...
- Keep the pending-hit queue tied to the full dot ownership that produced it, not only to a bare `current X`. In the current baseline that ownership should include at least the transfer-side `X` plus whether the dot belongs to abstract pre-visible transfer, hidden non-visible transfer, or visible transfer. If OBJ rendering is disabled or the pixel-transfer side advances to a different ownership class or `current X`, any still-pending hits owned by the old dot must be discarded as stale rather than being allowed to start a late OBJ fetch on a later, semantically different dot.
...
- In the same current baseline, do not collapse all served transfer dots into one undifferentiated "served" flag. Keep at least three explicit served-dot classes in the implementation: abstract pre-visible transfer service before FIFO-backed popping, hidden non-visible transfer service after FIFO-backed popping but before visible output begins, and real visible pixel service. That separation is needed for the current window-start rules, `WX = 166` deferral, and later stricter FIFO/push timing work.
