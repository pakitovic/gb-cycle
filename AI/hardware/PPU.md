...
- In the same current baseline, the service-side backing policy for startup transfer dots should also be explicit instead of being encoded indirectly through a pair such as `source = Abstract` plus `requires_fifo_backing = true`. The model should be able to say clearly whether a dot is abstract, abstract-but-FIFO-gated, or truly FIFO-backed, so later hidden-dot tightening can change that one backing policy without rediscovering what each call site meant by those combined flags.
...
- In the same current baseline, the execution effect of the current transfer dot should also be explicit instead of being inferred indirectly from a cross-product such as `action + backing`. The core should be able to say clearly whether this dot only consumes the last `SCX` discard, advances hidden startup without popping BG, advances a FIFO-backed hidden dot while consuming hidden BG/OBJ pixels, or emits a visible pixel. That leaves one direct seam for later hidden-dot tightening without having to rediscover execution semantics from multiple helper booleans.
...
- In the same current baseline, the late pre-visible dots immediately before the first visible output should no longer be unconditional heuristics. Once the pipeline reaches that seam, those dots may count as served only if the first BG FIFO fill already exists; otherwise they explicitly stretch `Mode 3` by one dot and wait for real FIFO readiness. Keep that gate separate from actual pixel popping for now, because the current repo-visible baseline still expects those late pre-visible dots to remain abstract rather than consuming BG pixels before the first visible output.
- In the same current baseline, that startup seam is now split explicitly into two ownership classes: the first four startup transfer dots remain abstract pre-visible service, while the last four startup dots immediately before the first visible pixel are treated as hidden transfer ownership that still requires real BG FIFO readiness. This is intentionally only a partial docboy-style closure: those late hidden dots already behave as hidden dots for OBJ/window ownership and `WX` trigger rules, but they still do not pop BG FIFO pixels yet because the stricter real-pop version regressed repo-managed DMG raster output such as `dmg-acid2`.
- In the same current baseline, window-trigger evaluation should no longer treat every dot with the right counters as equally eligible. A window trigger may be evaluated only on dots that could really serve transfer work on the current line: no active or still-pending OBJ fetch may still own the dot, and the BG side must already have real FIFO readiness for the current seam. Use that same servability rule for the provisional hidden-dot `WX = 0` path so the late-previsible FIFO gate is not bypassed by a separate window shortcut, but keep the DMG `WX = 0 && (SCX & 7) > 0` shortening case explicit too: the final remaining `SCX`-discard dot still carries its own one-dot shortening credit in the current baseline even before the first BG fill materializes, while the actual hidden-dot window restart remains gated by the later FIFO-backed seam.
