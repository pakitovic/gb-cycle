...
- [PPU][OBJ-PENDING-DOT-OWNERSHIP] The next structural follow-up slice is now landed too: pending OBJ hits are no longer keyed only by a bare `current X`. The PPU now keeps those pending hits tied to the full dot ownership that produced them, including both the transfer-side `X` and whether that ownership belongs to pre-visible transfer, hidden non-visible transfer, or visible transfer. That lets stale hits die when the pipeline crosses from one ownership class to another even if `current X` itself has not changed yet, which removes another implicit heuristic seam before the stricter shared fetcher rewrite. Repo-local tests now cover both stale-on-`X`-change and stale-on-phase-change behavior, and the current baseline remains closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
