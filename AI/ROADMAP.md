...
- [PPU][MODE3-LATE-HIDDEN-OWNERSHIP-SEAM] The next structural follow-up slice is now landed too: the last four startup dots immediately before the first visible pixel are no longer modeled as generic pre-visible ownership. The PPU now treats those dots as hidden transfer ownership with real BG FIFO readiness requirements, so OBJ-hit ownership, `WX = 0`/`WX = 7` trigger eligibility, and the shared served-transfer-dot model all cross a more explicit `pre-visible -> hidden -> visible` seam. This is intentionally still only partial docboy-style closure: an attempted version that also performed real BG FIFO pops on those late hidden dots regressed `acid/dmg-acid2.gb`, so the current baseline keeps those dots abstract on the pixel-consumption side while already treating them as hidden for arbitration and window timing. Validation for the landed slice is closed under the hard gate: local PPU tests, `make ci`, the materialized curated external suite, and the stable `.roms/test/test-report.md` baseline all remain green.
