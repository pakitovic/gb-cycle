...
- [PPU][MODE3-STARTUP-DUMMY-FIFO] The next startup-tightening slice is now landed too: the live DMG runtime no longer keeps the startup-effective FIFO purely as a logical placeholder counter. `Mode 3` line start now seeds the BG FIFO itself with `8` dummy startup pixels, closer to docboy's `bg_fifo.fill()` / `dummy_lx0` path, and the early startup `SCX`-discard plus `x < 8` transfer dots consume those real FIFO entries before the first real BG push arrives. The synthetic placeholder path is still retained for unit tests and later-seeded partial states that intentionally bypass the real line-start path, but the ordinary runtime now uses the real FIFO-backed startup path. This cut keeps `acid/dmg-acid2.gb` green, keeps repo-local PPU tests green after updating the line-start FIFO expectations, and materially improves the exploratory mealybug framebuffer deltas for `m3_lcdc_bg_map_change` and `m3_scy_change` without yet closing them; `m3_lcdc_tile_sel_change` remains open and slightly worse under this slice, so the next work in this lane should focus on first-line tile-data-selector timing rather than on reverting the startup FIFO change.
- [PPU][MODE3-STARTUP-DUMMY-FILL-SEAM] The next startup follow-up slice is now landed too: push-side FIFO emptiness for the first real BG/window fill no longer treats startup dummy occupancy as if it were already real tile payload. The runtime still seeds the live BG FIFO with `8` dummy startup pixels and still consumes them through the ordinary startup transfer path, but it now also tracks how many leading FIFO entries still belong to that dummy startup fill. The BG/window `Push` phase may therefore queue the first real fill while the queue still contains only those dummy startup pixels, instead of waiting for every dummy entry to drain first. This keeps `acid/dmg-acid2.gb` green, keeps the materialized curated external suite green, preserves the current exploratory `m3_lcdc_bg_map_change` delta (`1042`), improves `m3_scy_change` further (`10017 -> 8245`), and trims the left-edge tile-selector cases without closing them yet (`m3_lcdc_tile_sel_change: 1806 -> 1678`, `m3_lcdc_tile_sel_win_change: 1561 -> 1528`). Repo-local tests now lock down both the new push-side ownership seam for dummy-only FIFO content and the updated pre-visible startup snapshot around the first queued fill.
