...
4. Add persistence and header validation for MBC2.
   ...
   Status: header validation and battery capability distinction are done in the current branch baseline. `0x05` versus `0x06` now remains explicit in mapper metadata, and nonzero `0x0149` produces clear warnings or errors without being reinterpreted as external SRAM. The actual hardware-style persistence contract for exporting and restoring `0x06` nibble RAM remains intentionally sequenced under the later cartridge-persistence block of Phase `6` so it can land once for all supported families instead of as an MBC2-only side path.
5. Close with dedicated MBC2 tests and oracle comparisons.
   ...
   Status: implementation and local coverage are done in the current branch baseline. Unit tests, integration tests, and one retained synthetic Phase `6` ROM/trace now cover address-bit-`8` control decode, bank `0 -> 1`, ROM-size diagnostics, echo aliasing, low-nibble storage, the chosen high-nibble readback policy, and `0x0149` validation. External oracle comparison for the MBC2 control-decode and nibble-RAM edge cases is still deferred, and the true battery-persistence round-trip remains coupled to the later phase-wide cartridge persistence contract.
...
5. Add time-source separation and persistence.
   ...
   Status: partially done in the current branch baseline. The runtime now has explicit separation between visible RTC registers, live RTC state, and a deterministic injected advance path for tests, so RTC behavior is no longer tied to CPU T-cycles or host wall time. Battery-backed persistence and powered-off elapsed-time restore remain intentionally sequenced under the later cartridge-persistence block of Phase `6`.
...
5. Add MBC5 validation, diagnostics, and persistence expectations.
   ...
   Status: partially done in the current branch baseline. `MBC5` header types `0x19..=0x1E` are now distinguished through explicit variant metadata, ROM sizes above `8 MiB` produce clear errors, no-RAM types with nonzero `0x0149` emit clear validation diagnostics, and rumble-capable types now expose observable `rumble_on` state. Battery-backed persistence expectations remain intentionally sequenced under the later cartridge-persistence block of Phase `6` rather than landing as an MBC5-only side path.
6. Close with dedicated MBC5 tests and oracle comparisons.
   ...
   Status: local implementation coverage is done in the current branch baseline. Unit tests, integration tests, and one retained synthetic Phase `6` ROM/trace now cover header types `0x19..=0x1E`, bank `0` visibility in the switchable region, bank `0x1FF`, the `0xFF -> 0x100` boundary, RAM-enable behavior, SRAM behavior for `32 KiB` and `128 KiB`, RAM-bank masking, rumble on/off, and size-validation diagnostics. External oracle comparison is still deferred, and the later battery-backed persistence round-trip remains coupled to the shared cartridge-persistence block of Phase `6`.
...
- [TEST-RUNNER][OFFICIAL-INSTR-TIMING] The official `retrio/blargg instr_timing` ROM is now integrated as its own typed external suite contract under the same repo-managed asset store and also passes in `release`. That gives Phase `9` one official timing-focused CPU path on top of the functional `cpu_instrs` coverage, without needing a frontend or ad hoc local asset wiring.
