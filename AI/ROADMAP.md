...
- [PPU][MODE3-VISIBLE-REGISTERS] The PPU now keeps one explicit visible-register block for active-LCD fetch and pixel mixing, distinct from the MMIO-owned storage bytes, and the current implementation refreshes that block at the start of each PPU T-cycle. BG/window/OBJ fetch and DMG palette lookup now consume that visible block instead of reading `LCDC`, scroll, window, and palette registers live from ad hoc call sites, and the internal PPU tests now cover the "write between ticks becomes visible on the next dot" contract directly. This is intentional groundwork for the broader Mode `3` refactor, not closure by itself: the line still ends from the older aggregate `stall_dots` accounting, BG push timing still needs a stricter FIFO-empty contract, and window / sprite restart timing still needs a more explicit per-dot state machine before Phase `9` can treat the live-write lane as hardened.
