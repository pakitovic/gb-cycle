...
- [PPU][WINDOW-TRIGGER-SERVABILITY] The next window/fetcher follow-up slice is now landed too: the current provisional window trigger no longer evaluates on arbitrary dots just because `visible_pixels_output`, `SCX` discard, and `WX` counters happen to line up. The PPU now only evaluates a window trigger on dots that could really serve transfer work on the current line: active OBJ fetch and still-pending OBJ-hit ownership block the trigger, late-previsible `WX = 0` activation now requires real FIFO readiness, and that hidden pre-visible trigger path is explicitly limited to the real `WX = 0` case instead of also pulling in `WX = 7` or the deferred `WX = 166 -> next line` continuation. The same slice now keeps the DMG `WX = 0 && (SCX & 7) > 0` shortening case explicit as well: the final remaining `SCX`-discard dot contributes a one-dot shortening credit before the later FIFO-backed hidden restart seam, preserving the expected one-dot lead over `WX = 7` without reopening arbitrary hidden-dot triggers. Repo-local tests now cover the late-previsible `WX = 0` gate, the restored last-discard shortening path, the negative `WX = 7` pre-priming case, and the OBJ-pending block, while the existing `WX = 0`, `WX = 166`, window line-counter, and mid-scanline restart tests remain green. Validation for this slice is also closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
