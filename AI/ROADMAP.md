...
- [PPU][PUSH-DOT-DECISION] The next shared-fetcher follow-up slice is now landed too: the BG/window push phase no longer hides all of its per-dot outcomes behind a boolean. The PPU now exposes explicit push-dot results for entry-delay wait, FIFO-space wait, OBJ handoff, queued fill, and the degenerate not-ready case, while preserving the current low-watermark occupancy rule and external DMG baseline. Repo-local tests now lock down each push-side outcome directly, including the no-pending-slice path and the object-handoff path, so the next strict-FIFO-empty attempt can replace the occupancy policy itself without first rediscovering the scheduler contract. Validation for this slice is also closed under the hard gate: local PPU tests stay green, and the full repo validation remains the required gate before committing or keeping any later push-policy change.
