...
- The same local spike has also shown a useful negative result: once `startup alignment`, `fetch_x`, and same-dot post-alignment `TileIndex` are explicit, the stubborn DMG mealybug families (`m3_lcdc_bg_map_change`, `m3_lcdc_tile_sel_change`, `m3_scy_change`) still do not improve materially. The next implementation block should therefore pivot from generic startup-seam work to the fetch-phase live-write timing of the second and third visible BG tiles.
- The same local spike has now also identified the current best seam inside that follow-up work: repo-local tracing shows the relevant `FF40` / `FF42` writes landing while the next BG slice is already cached in `Push.0`, not while it is still in `TileIndex`. The current best exploratory cut therefore keeps the first real post-alignment BG push one dot later than before and allows a localized pending-push tile-data refetch for background on live `SCY` / `LCDC.4` writes. That cut keeps repo-local PPU tests green, keeps `acid/dmg-acid2.gb` green, and currently moves the targeted exploratory counters to `m3_lcdc_bg_map_change: 915`, `m3_lcdc_tile_sel_change: 1410`, `m3_lcdc_tile_sel_win_change: 1232`, and `m3_scy_change: 7819`. The next work in this lane should therefore stay on cached-`Push` live-write timing, not reopen generic startup ownership or broad tilemap rereads.
- The same local spike now has one more landed reduction inside that cached-slice lane: repo-local tracing showed the remaining `m3_lcdc_bg_map_change` debt was not on the generic `TileIndex` read anymore, but on very narrow same-T-cycle windows where the first cached post-startup background slice is already waiting in `Push` or `fill.pending`. Broadening `LCDC.3` retargeting to every cached slice regresses the oracle, so the current best cut keeps that retarget path tied to those explicit windows only. Validation for that local cut is closed again: repo-local PPU tests stay green, `acid/dmg-acid2.gb` stays green, and the current exploratory counters now sit at `m3_lcdc_bg_map_change: 674`, `m3_lcdc_tile_sel_change: 1410`, `m3_lcdc_tile_sel_win_change: 1232`, and `m3_scy_change: 7819`. The next useful work in this lane should therefore continue from cached-slice same-T-cycle behavior instead of reopening broad startup or broad tilemap rereads.
- The same March 29, 2026 local measurement pass now adds a more specific target inside that remaining `m3_lcdc_bg_map_change = 674` debt. Counting changed pixels per column on the current stable cut shows the left-edge error is heaviest on the third visible BG tile after startup (`x=16..23`, about `104-108` changed pixels per column), lighter on the second tile (`x=8..15`, about `38-58`), and lightest on the first visible tile (`x=0..7`, about `30`). The roadmap should therefore treat the third visible tile as the next primary closure target for `LCDC.3` timing rather than continuing to treat all of `x=0..23` as one undifferentiated edge case.
- The repo-local core now also carries one explicit post-alignment label for the second and third visible background tiles. That label is intended as infrastructure for the next block, not as closure by itself: it makes those two tiles directly addressable in tests and fetcher logic without reopening the broad startup/dummy-seed refactors that have already been ruled out as low-yield.
- The same local lane now has one more landed structural cut even though it does not move the exploratory mealybug counters by itself: cached background slices in `Push` and `fill.pending` are no longer modeled as shadow state split between `fetcher.tile_*` and a few side flags. The repo baseline now keeps one explicit cached-slice record, including `fetch_x`, tilemap/tile-data addresses, tile index, fetched bytes, and the second/third-visible-tile label, and that record stays the source of truth across the `Push -> fill -> FIFO` handoff. Validation for this structural cut is closed again (`ppu::tests`, `acid/dmg-acid2.gb`), and the current exploratory counters remain `m3_lcdc_bg_map_change: 674`, `m3_lcdc_tile_sel_change: 1410`, `m3_lcdc_tile_sel_win_change: 1248`, and `m3_scy_change: 7819`. The next useful work should therefore use this clearer cached-slice owner seam to debug the left edge, not reopen the older "fetcher fields shadow cached slice" pattern.
- The same local lane now has one more structural cut on top of that cached-slice owner seam: the repo baseline also keeps the cached-slice live-write window and reread recompute path concentrated on the cached slice itself instead of duplicating that logic between `write_register`, `Push`, and `fill.pending`. Validation is still closed on the short gates (`ppu::tests`, `acid/dmg-acid2.gb`). The next useful work should therefore attack left-edge behavior by changing cached-slice policy or cached-slice timing, not by re-splitting the same seam across three owners again.
- The same local lane now has one more consolidation on top of that owner seam: the post-startup left-edge label is no longer tracked as a parallel `0/1/2` continuation index plus separate cached-slice flags. The repo baseline now publishes that state through one explicit cached-slice origin enum from the startup seam into the live fetcher and then across `Push -> fill -> FIFO`. Validation is closed again (`ppu::tests`, `acid/dmg-acid2.gb`, `make ci`), and the next useful work should therefore target behavior on top of that single origin seam instead of keeping parallel continuation labels alive.
...
- [PPU][MEALYBUG-MODE3-LIVE-WRITES] The exploratory `mealybug-tearoom-dmg-curated` suite now has one concrete closure point: `m3_bgp_change` passes under `Strict` after refining two explicit DMG-family behaviors in the PPU model, namely the non-line-`0` Mode `2` LCD STAT pretrigger path and DMG palette-write conflict handling for `BGP/OBP*` writes during late Mode `3` / early HBlank. Follow-up work in this lane has also clarified a harness-side precondition shared by multiple sprite-driven cases: ROMs such as `m3_obp0_change` and `m3_bgp_change_sprites` assume the DMG boot trademark tile is already present in VRAM, so the runner now supports typed startup-memory writes and those curated cases seed the boot-derived tile `0x19` under `SkipBoot` instead of misclassifying missing-logo output as a PPU regression. With that boot-derived seed in place, `m3_obp0_change` no longer fails as a blank-output startup artifact and the remaining mismatch shrinks to a small real PPU delta. The next useful reductions in this lane are now landed too: the PPU no longer clamps early OBJ scheduling to visible `X = 0`, and instead keeps a distinct pre-visible raw-`X` match path for left-edge sprites; for those low-`X` startup requests, the OBJ fetch no longer freezes the whole line immediately on match and instead lets the BG side finish priming the FIFO and leave the initial `TileIndex` phase before the sprite path starts stealing dots; and the DMG palette-conflict approximation now keeps `OBP*` writes slightly wider than `BGP` writes in the retroactive window, which trims `m3_obp0_change` further without regressing `m3_bgp_change_sprites` or the existing internal PPU coverage. That keeps the general DMG OBJ-priority tests green, preserves the smaller `m3_obp0_change` delta, and reduces `m3_bgp_change_sprites` further without broadening the change into a whole-pipeline refactor. It still does not close either case yet. The curated inventory for this exploratory lane has now also been expanded to the full 24 desired `GBEmulatorShootout`-sourced DMG framebuffer ROMs in `mealybug-tearoom-tests`, and the mandatory `before/after` report check for that expansion shows only new mealybug rows, with no regressions outside the family. That inventory pass is intentionally non-closure work: newly added cases such as `m3_lcdc_bg_en_change`, `m3_lcdc_bg_map_change`, `m3_lcdc_obj_en_change*`, `m3_lcdc_tile_sel_change*`, `m3_lcdc_win_map_change`, `m3_scx_high_5_bits`, `m3_scy_change`, and `m3_wx_[4-6]_change` remain red by design after their first curated run. The still-red follow-up space therefore narrows down further instead of leaving the whole lane amorphous: low-`X` sprite/live-OBJ timing (`m3_bgp_change_sprites`, the remaining `m3_obp0_change` timing delta), window/live-`LCDC.5` timing (`m2_win_en_toggle`, `m3_window_timing*`, `m3_lcdc_win_en_change_multiple*`, `m3_wx_4_change*`, `m3_wx_5_change`, `m3_wx_6_change`), live `LCDC` map / enable / tile-select changes (`m3_lcdc_bg_en_change`, `m3_lcdc_bg_map_change`, `m3_lcdc_win_map_change`, `m3_lcdc_tile_sel_change*`, `m3_lcdc_obj_en_change*`), `SCX/SCY` live-scroll timing (`m3_scx_high_5_bits`, `m3_scx_low_3_bits`, `m3_scy_change`), and live sprite-size timing (`m3_lcdc_obj_size_change`, `m3_lcdc_obj_size_change_scx`) still need dedicated closure. Phase dependency: this does not block entering Phase `7`, but Phase `9` should not mark the mealybug PPU oracle lane as closed until those remaining Mode `3` live-write families are rechecked against the new baseline.
- [PPU][MODE3-VISIBLE-REGISTERS] The PPU now keeps one explicit visible-register block for active-LCD fetch and pixel mixing, distinct from the MMIO-owned storage bytes, and the current implementation refreshes that block at the start of each PPU T-cycle. BG/window/OBJ fetch and DMG palette lookup now consume that visible block instead of reading `LCDC`, scroll, window, and palette registers live from ad hoc call sites, and the internal PPU tests now cover the "write between ticks becomes visible on the next dot" contract directly. The next slice of the broader Mode `3` refactor is landed too: visible-line HBlank entry no longer comes from separate accumulated `stall_dots` / `window_extra_stall_dots` counters, and instead one line-local `Mode 0` start target is updated on the exact dots where active OBJ fetch or post-window FIFO starvation actually steal pixel-transfer time. That cut stays green under `make ci`, and the repo-managed external framebuffer baseline remains stable after rerunning the materialized curated suites: the temporary attempt to also tighten BG push timing to a strict FIFO-empty rule in the same slice regressed `acid/dmg-acid2.gb`, so that stricter push contract was intentionally backed out and is still deferred until the fetcher / BG-OBJ interruption model is rewritten more completely. This is still not closure by itself: the remaining work is the shared per-dot state machine for window / sprite restart timing plus the later stricter BG push semantics once they can land without external regressions.
- [PPU][MODE3-WINDOW-ACTIVATION-DOT] The next Mode `3` follow-up slice is also landed: window restart no longer behaves as a zero-cost fetcher switch on the trigger dot. The PPU now keeps one explicit one-dot `WindowActivating` fetcher state, clears the BG FIFO on trigger, suppresses same-dot OBJ-fetch start, and only begins the restarted window fetcher on the following dot. Repo-local unit and machine tests for `WX = 0`, `WX = 166`, mid-scanline restart, and OBJ-FIFO survival across window restart remain green under that model, and the materialized external curated suites stay stable too. This still stops short of the full shared BG/window/OBJ fetcher rewrite: the stricter FIFO-empty BG push contract and the sprite-start arbitration tied to the fetcher push phase are still open follow-up work.
- [PPU][MODE3-OBJ-HIT-LATCH] The next shared-fetcher slice is now landed too: matching OBJ hits are no longer consumed only by an immediate pre-fetcher equality check. The PPU now latches matching sprite slots into explicit pending per-line state, lets those hits survive until object fetch can really start, and marks BG/window push-side state as explicitly interrupted while an already fetched BG tile waits behind an intervening OBJ fetch. Repo-local unit and live-machine tests for BG priming, window restart, early left-edge OBJ startup ordering, live OBJ timing stretch, and LCDC.1 mid-fetch cancel behavior stay green under this cut. This is still not the final `docboy`-style push model: strict FIFO-empty BG push and a fully push-owned OBJ-start cadence remain deferred until the shared per-dot fetcher state can be tightened without external regressions.
- [PPU][MODE3-PUSH-PER-DOT-RETRY] The next follow-up slice is now landed as well: the BG/window `Push` state no longer remains under the generic 2-dot fetch-stage clock after it is entered. The current repo baseline keeps one explicit one-dot entry delay when `Push` is first reached, but after that point the cached BG/window push is rearbitrated every dot, which brings the fetcher closer to the `docboy`-style per-dot push model without moving the already-validated first visible pixel timing. Repo-local unit tests now cover that "one-dot arm, then per-dot retry" contract directly, and the existing BG priming, window restart, live OBJ stretch, and LCDC.1 mid-fetch-cancel tests remain green under it. This is still an intermediate cut: strict FIFO-empty BG push and a truly push-owned FIFO-fill cadence remain deferred until the remaining shared fetcher timing can land without external regressions.
- [PPU][MODE3-PUSH-FILL-SPLIT] The next shared-fetcher slice is now landed too: BG/window push arbitration no longer writes pixels into the BG FIFO directly. Instead, the push state now queues one explicit pending BG FIFO fill and the pipeline materializes that queued fill in a separate fill phase at the start of the following `Mode 3` dot before OBJ fetch or BG pixel pop. The same slice also leaves one explicit dot-local phase order in the core implementation: current-dot window handling, then pending fill materialization, then active OBJ-fetch work, then BG pop/discard/output, and only after that the shared BG/window fetcher tick. That leaves the repo-visible external baseline stable while giving the PPU the state boundary and phase ordering it was missing for later tighter push semantics and docboy-style shared fetcher sequencing. Repo-local unit tests now cover both "push queues fill first, fill materializes on the following dot" and the shifted first-fill timing around the initial BG priming path, and the usual BG priming, window restart, live OBJ stretch, and live LCDC.1 cancel cases remain green under `make ci` plus the repo-managed curated external suite.
- [PPU][MODE3-EXPLICIT-DOT-FETCHERS] The next structural fetcher slice is also landed: BG/window and OBJ fetch no longer advance through one generic "increment `stage_dot`, then maybe commit the stage" helper. Instead, the internal `stage + stage_dot` pair is now treated as the explicit one-dot automaton state for each fetcher, with direct per-edge transitions for `TileIndex`, `TileDataLow`, `TileDataHigh`, OBJ `Startup`, and the corresponding data phases. That keeps the repo-visible snapshot contract stable while making future timing changes local to one state edge at a time rather than hidden behind shared counter logic. Repo-local unit tests now cover both the BG/window fetch progression and the overlapped OBJ-start progression under that explicit state machine, and the external DMG baseline is expected to remain byte-for-byte stable under the normal validation gate.
- [PPU][MODE3-OUTPUT-GATED-BY-PENDING-OBJ] The next shared-fetcher slice is now landed too: after a sprite hit has already been latched for the current dot, the output side no longer keeps advancing BG pop or pre-visible `current X` while that hit is still pending and OBJ rendering is still enabled. The PPU now keeps one explicit output-side gate for that state, so a pending-but-not-yet-served OBJ hit stretches `Mode 3` by one dot and stalls the `current X` advance until the OBJ fetch really starts or OBJ rendering becomes disabled. Repo-local unit tests now cover both the visible-output stall and the pre-visible raw-`X` stall directly, and the live-machine OBJ timing stretch case remains green under the same model. This is still not the final strict `docboy` push contract, but it moves the pixel-output side closer to the same "do not advance LX while an OBJ hit still owns the dot" rule without forcing the isolated strict-FIFO-empty push change that previously regressed the external DMG baseline.
- [PPU][MODE3-OBJ-HIT-OWNERSHIP] The next follow-up slice is now landed too: latched OBJ-hit queues are no longer treated as unscoped pending work. The PPU now keeps those pending hits owned by the specific `current X` that produced them, uses that ownership when deciding whether the output side is still blocked, and discards stale pending hits if OBJ rendering becomes disabled or the pixel-transfer side has already moved on to a different `current X`. Repo-local unit tests now cover both the stale-hit discard path and the updated ownership-sensitive blocking path, and the existing live OBJ timing/stretch cases remain green under the same model. This still is not the final strict `docboy`-style fetcher closure, but it removes another source of accidental late OBJ work before the later tighter push/fill steps.
- [PPU][MODE3-SERVED-OUTPUT-DOTS] The next output-side cleanup slice is now landed too: the PPU no longer treats "entered the output phase" and "served one transfer dot" as the same event. The current baseline now distinguishes the eight hidden pre-visible startup dots, later BG-pop / `SCX`-discard dots, and true post-priming BG FIFO starvation. Only the served transfer-dot cases advance the pre-visible `current X`; a real starvation dot now leaves that `current X` in place and stretches `Mode 3` by one dot instead. Repo-local unit tests cover all three cases directly, and the external DMG baseline remains the hard gate for future tightening. This is still not the final strict `docboy` push closure, but it removes another output-side heuristic before the later shared fetcher rewrite.
- [PPU][MODE3-TRANSFER-PHASES] The next structural follow-up slice is now landed too: the transfer side no longer relies only on scattered checks against `pre_visible_obj_match_x`, `visible_pixels_output`, and `SCX` counters to decide what kind of dot it is serving. The PPU now keeps one explicit internal transfer phase across hidden pre-visible startup dots, the follow-on `SCX` discard path, and ordinary visible output, and `current X` derivation is now phase-owned rather than inferred ad hoc at every use site. Repo-local tests cover the startup-to-discard transition, the discard-to-visible transition, and the phase-owned `current X` readback directly. This still preserves the current repo baseline rather than claiming full docboy-style `LX` accuracy, but it creates the seam needed for the later stricter rewrite without re-spreading transfer semantics across `ppu.rs`.
- [PPU][MODE3-TRANSFER-OWNERSHIP-COUNTER] The next follow-up slice is now landed too: `Mode 3` dot ownership no longer comes from a mix of `pre_visible_obj_match_x` plus special-case visible-output math. The PPU now keeps one explicit `current_transfer_x`-style counter as the single source of truth for the BG/window/OBJ ownership coordinate, while preserving the current repo-visible phase behavior. This is intentionally still a preparatory slice rather than full docboy-style `LX` closure: the counter is now centralized, but its phase-to-phase behavior is still the repo baseline rather than the stricter `discard_lx0 -> hidden lx<8 -> visible lx>=8` model. That tradeoff is deliberate so later work can tighten the semantics without having to first uproot duplicated ownership math from multiple paths again.
- [PPU][MODE3-PARTIAL-LX-OWNERSHIP] The next behavioral slice is now landed too: `current_transfer_x` is no longer just a centralized mirror of the older heuristic phases. `Mode 3` now separates initial priming from active transfer, keeps transfer ownership at `lx0` while initial `SCX` discard is consumed, advances that same ownership counter through the hidden `lx < 8` region, and only then continues through visible output for `lx >= 8`. The post-priming BG-pop path now owns hidden/visible progress directly instead of relying on a later transfer-phase helper to reconstruct it. Repo-local unit tests now cover abstract pre-visible discard at `lx0`, post-priming discard and hidden-lx progress, the `WX = 0` shortening case under the new ownership model, and visible palette output once transfer has reached `lx >= 8`. Validation for this slice is closed under the current hard gate too: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline. This still is not full docboy-style closure: the earliest hidden startup dots are still modeled as abstract served transfer dots rather than strict FIFO-empty BG pops, and strict BG push timing remains explicitly deferred.
- [PPU][MODE3-LATE-PREVISIBLE-FIFO-GATE] The next follow-up slice is now landed too: the last pre-visible transfer dots before the first visible pixel are no longer unconditional phase-counter progress. The PPU now requires the first BG FIFO fill to exist before those late pre-visible dots can count as served; otherwise they explicitly stretch `Mode 3` by one dot and wait for real FIFO readiness. This keeps the current repo-visible baseline intact, because those late pre-visible dots still do not pop BG pixels yet, but it removes another pure heuristic from the startup side of `Mode 3` and gives the pipeline one explicit seam between "priming with no FIFO" and "pre-visible dots backed by a real first fill". Repo-local tests now cover both the late-previsible stall-without-fill case and the resumed progress once the fill exists, plus the matching `SCX`-discard case. Validation for this slice is also closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][WINDOW-TRIGGER-SERVABILITY] The next window/fetcher follow-up slice is now landed too: the current provisional window trigger no longer evaluates on arbitrary dots just because `visible_pixels_output`, `SCX` discard, and `WX` counters happen to line up. The PPU now only evaluates a window trigger on dots that could really serve transfer work on the current line: active OBJ fetch and still-pending OBJ-hit ownership block the trigger, and late-previsible `x = 0` activation now requires real FIFO readiness. The same slice also keeps the DMG `WX = 0 && (SCX & 7) > 0` shortening case explicit: the final remaining `SCX`-discard dot contributes a one-dot shortening credit before the later FIFO-backed hidden restart seam instead of leaving that quirk implicit. Repo-local tests now cover the late-previsible `x = 0` FIFO gate, the restored last-discard shortening path, the pre-priming negative case, and the OBJ-pending block, while the existing `WX = 0`, `WX = 166`, window line-counter, and mid-scanline restart tests remain green. Validation for this slice is also closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-SERVED-DOT-KINDS] The next structural follow-up slice is now landed too: `Mode 3` no longer reports only a coarse "served / not served" result from the output side. The PPU now distinguishes abstract pre-visible transfer service, hidden non-visible transfer service after FIFO-backed popping, and real visible-pixel service, and the current window-start / `WX = 166` / `x0` restart logic consumes those explicit dot kinds instead of inferring everything indirectly from counters alone. Repo-local tests now lock down each served-dot class directly, and the current baseline behavior stays closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][OBJ-PENDING-DOT-OWNERSHIP] The next structural follow-up slice is now landed too: pending OBJ hits are no longer treated as stale merely because the same `current_transfer_x` crossed from one ownership class to another. The PPU now keeps those pending hits keyed to the transfer-side `X` itself, so a hit latched during the abstract pre-visible startup can survive into the later hidden seam while that `X` is still current, which matches the docboy-style `oam_entries[lx]` model more closely. Hits still die when `current_transfer_x` advances or OBJ rendering is disabled, so the queue cannot leak into a semantically later dot. Repo-local tests now cover both stale-on-`X`-change and survives-on-phase-change behavior, and the current baseline remains closed under the hard gate: `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][WINDOW-TRIGGER-FROM-SERVED-TRANSFER-DOT] The next structural follow-up slice is now landed too: window restart and the DMG `WX = 0 && (SCX & 7) > 0` shortening path are no longer driven by separate pre-dot helpers. The PPU now derives both behaviors from the same explicit served-transfer-dot result object that comes out of the output side, carrying the served-dot kind plus whether that dot consumed the final remaining `SCX` discard. That gives the current baseline one common state boundary for `WX = 0`, `WX = 7`, and forced `x = 0` continuation lines, while still keeping the extra one-dot shortening credit scoped to the real `WX = 0` case. Repo-local tests now cover the late-previsible `x = 0` restart seam, served-dot-driven shortening, `WX = 7` start at the first served `x = 0` seam, OBJ-pending blocking, the window line counter, and the existing `WX = 166` continuation path. Validation for this slice is also closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-FIFO-EMPTY-PUSH] The next shared-fetcher slice is now landed too: BG/window push occupancy no longer uses the old low-watermark slack. A fetched BG/window slice may now queue its pending fill only when the BG FIFO is empty, which matches the stricter Pan Docs/docboy contract more closely while keeping the repo-managed DMG baseline green. Repo-local PPU tests now lock down the empty-FIFO wait and retry behavior directly, and validation for this slice is closed under the hard gate: `make ci` stays green, the materialized curated external suite stays green, `acid/dmg-acid2.gb` remains green, and `.roms/test/test-report.md` stays byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-PUSH-CAN-FILL-AND-START-OBJ] The next shared-fetcher follow-up slice is now landed too: strict FIFO-empty push no longer forces a false choice between queueing the BG fill and starting an OBJ fetch when both are valid on the same dot. If the BG FIFO is empty and the current transfer dot already owns a pending OBJ hit, the PPU now queues the BG fill first and still overlaps the OBJ fetch on that same push dot, which is closer to the corresponding docboy branch than the older "handoff to OBJ only" behavior. Repo-local PPU tests now cover both the old cached-tile handoff path and the new empty-FIFO dual-action path, and validation for this slice is closed under the hard gate too: `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-DOT-ARBITRATION] The next shared-fetcher cleanup slice is now landed too: `Mode 3` no longer decides "BG serves this dot" and "OBJ may start on this dot" through separate ad hoc helper paths. The PPU now derives one explicit current-dot arbitration result that is shared by output-side BG service, FIFO-backed OBJ start, and queued-fill OBJ overlap. That keeps the current repo-visible behavior intact while moving the fetcher closer to the docboy-style split between `is_bg_fifo_ready_to_be_popped()` and `is_obj_ready_to_be_fetched()`, and it gives later tightening work one common seam for hidden-dot service and push-side overlap instead of re-deriving those rules again. Repo-local tests now cover the distinction between FIFO-backed OBJ start and queued-fill OBJ start directly, and validation for this slice is closed under the same hard gate: `make ci` stays green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-FIFO-BACKED-OBJ-START-GATE] The next shared-fetcher follow-up slice is now landed too: the FIFO-backed OBJ-start branch no longer keys only off "BG FIFO happens to be non-empty". The current dot must now also really belong to the FIFO-backed hidden/visible transfer side of `Mode 3`; abstract startup dots that still carry a leftover BG FIFO byte no longer qualify for the FIFO-backed OBJ-start path and must instead rely on the queued-fill overlap path if an OBJ hit is pending there. Repo-local tests now cover both sides of that distinction explicitly, including the negative case where the BG FIFO is non-empty on an abstract priming dot, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-SERVED-STARTUP-SEAM] The next structural follow-up slice is now landed too: the `pre-visible -> hidden` startup seam is no longer inferred from raw `line_dot` once `Mode 3` has entered the startup transfer window. The PPU now tracks how many startup transfer dots were actually served and uses that explicit progress to decide when the abstract pre-visible lane gives way to the hidden lane for transfer planning and OBJ-hit ownership. That closes a real modeling gap in the previous baseline, where a stalled startup could drift into "hidden" ownership merely because time elapsed on the scanline even though the missing startup transfer dots had never been served. Repo-local tests now cover both transfer-plan classification and OBJ-hit ownership under that stalled-startup condition, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-TRANSFER-CONTEXT] The next structural follow-up slice is now landed too: the current transfer dot no longer derives "lane" and "source window" through separate partial helpers. The PPU now builds one explicit transfer context per current dot that says both which transfer lane is being served (`pre-visible`, `hidden`, or `visible`) and whether the dot belongs to the abstract startup window or the FIFO-backed post-priming window. That lets the core represent an important stalled-startup case explicitly: after priming has ended, the transfer source can already be FIFO-backed while the lane is still pre-visible because not enough startup transfer dots have actually been served yet. Repo-local tests keep the existing startup seam cases green under this model, and validation for this slice is gated the same way: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-STARTUP-ENTRY-DELAY-STATE] The next structural follow-up slice is now landed too: entering the startup transfer window is no longer gated only by checking `line_dot` against a constant threshold at every call site. The PPU now keeps one explicit startup-entry delay state that is initialized by `start_line()` and counts down to the first transferable startup dot before the transfer window opens. That makes the beginning of the startup transfer window inspectable and stateful in the live machine model, while still preserving a controlled fallback for synthetic tests that intentionally build later `Mode 3` states without running the real line-start path first. Repo-local tests now cover the explicit countdown itself, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-ABSTRACT-SOURCE-WINDOW-STATE] The next structural follow-up slice is now landed too: the switch from the abstract startup transfer window to the FIFO-backed post-priming window is no longer recovered from `line_dot` on demand. The PPU now keeps one explicit countdown for that source-window transition, so the current transfer context can say "startup transfer is already open, but this dot still belongs to the abstract source window" without reading raw scanline-dot thresholds again. That makes the transfer context more self-contained and auditable, and it aligns better with the longer-term goal of deriving `Mode 3` behavior from explicit dot-state rather than scattered time checks. Repo-local tests now cover that abstract-to-fifo-backed source-window transition directly, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-PREVISIBLE-LANE-STATE] The next structural follow-up slice is now landed too: the startup `pre-visible -> hidden` seam is no longer re-derived from a running "served startup dots" total every time the PPU asks what kind of non-visible startup dot it owns. The current baseline now keeps that seam in one explicit countdown state, initialized at line start and consumed only by served non-visible startup transfer dots, so the transfer context and OBJ-hit ownership can answer the seam directly without scattering that comparison logic across callers. Synthetic tests that intentionally bypass `start_line()` can still seed the countdown directly, which keeps the model debuggable without making runtime `Mode 3` depend on raw `line_dot` again. Repo-local tests now cover the countdown itself, the transfer-plan seam, and the OBJ-hit seam directly, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-TRANSFER-BACKING-POLICY] The next structural cleanup slice is now landed too: `Mode 3` transfer plans no longer describe the awkward late-startup hidden case through the indirect combination `source = Abstract` plus `requires_fifo_backing = true`. The current baseline now uses one explicit backing policy for each serviced transfer dot: purely abstract startup, abstract-but-FIFO-gated hidden startup, or truly FIFO-backed transfer. That gives the fetch/output side one clearer place to express "this dot must wait for FIFO readiness but still must not pop BG yet", which is exactly the seam that later hidden-dot tightening will need to revisit. Repo-local tests now assert the explicit backing policy in the transfer plan, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-TRANSFER-READINESS] The next structural cleanup slice is now landed too: output-side transfer service and FIFO-backed OBJ-start no longer re-derive readiness through separate helpers. The current baseline now builds one explicit current-dot transfer-readiness result from the transfer plan plus current FIFO state, and both the output path and the FIFO-backed OBJ-start gate consume that same readiness boundary. This keeps the current repo-visible behavior intact, but it removes another duplicated decision point before the later hidden-dot tightening, so the project can refine "is this current dot really FIFO-backed and ready?" in one place instead of patching the output path and the OBJ-start path independently. Validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-CURRENT-TRANSFER-SNAPSHOT] The next structural cleanup slice is now landed too: the PPU no longer lets adjacent helpers rebuild the current transfer context and the current transfer readiness separately. The current baseline now materializes one explicit current-dot transfer snapshot that carries both the transfer context (`pre-visible`/`hidden`/`visible` plus `AbstractStartup`/`FifoBacked`) and the current readiness for that same dot. Output-side transfer execution, OBJ-hit ownership, and FIFO-backed OBJ-start arbitration all consume that shared snapshot, which removes another seam where later hidden-dot tightening could otherwise drift apart into slightly different interpretations of the same dot. Repo-local tests now lock down that one hidden-dot snapshot can stay context-stable while only its readiness flips from waiting to ready as FIFO state changes, and validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, and `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline hash.
- [PPU][MODE3-TRANSFER-EXECUTION] The next structural cleanup slice is now landed too: the effect of servicing the current transfer dot is no longer inferred indirectly from the cross-product of "action" and "backing". The current baseline now keeps one explicit execution kind for the current transfer dot, distinguishing `SCX`-discard service, hidden startup advance without BG pop, FIFO-backed hidden advance with BG/OBJ consumption, and visible pixel emission. That keeps the repo-visible behavior stable while giving the hidden-dot lane one clearer seam for later docboy-style tightening, because future changes can now refine the execution kind directly instead of reverse-engineering it from helper booleans on the backing policy. Repo-local tests stay green under this cut, and the same hard validation gate remains required before the slice can be treated as stable: `acid/dmg-acid2.gb`, the materialized curated external suite, `.roms/test/test-report.md`, and `make ci` all have to remain green.
- [PPU][MODE3-PUSH-DOT-OWNERSHIP] The next shared-fetcher cleanup slice is now landed too: the BG/window `Push` state no longer discovers "wait for empty FIFO", "queue fill", and "start OBJ from this same push dot" by walking a branchy mutation sequence. The current baseline now derives one explicit push-dot ownership result first, distinguishing `EntryDelay`, `WaitingForEmptyFifo`, FIFO-backed handoff to `OBJ`, plain `QueuedFill`, and `QueuedFill` followed by same-dot `OBJ` handoff. The execution path then consumes that ownership directly. Repo-local tests now lock down the ownership transitions and the existing push/fill/handoff behavior remains green under the same hard gate: local PPU tests, `acid/dmg-acid2.gb`, the materialized curated external suite, `.roms/test/test-report.md`, and `make ci` must all remain green.
- [PPU][MODE3-STARTUP-EFFECTIVE-FIFO] The next startup-tightening slice is now landed too: startup transfer dots no longer behave as abstract timing-only service after the `pre-visible -> hidden` seam. The PPU now keeps one explicit startup effective-BG-FIFO occupancy counter, initialized with placeholder slots that mirror the docboy `bg_fifo.fill()` behavior at `dummy_lx0`, and startup `SCX`-discard plus all `x < 8` transfer dots now consume one effective BG FIFO slot per served dot. Visible output still requires a real BG FIFO pixel, so this cut keeps `dmg-acid2.gb` and the repo-managed external framebuffer baseline green while moving startup timing materially closer to docboy's `discard_lx0` / `lx0` model. Repo-local tests now cover placeholder-backed `SCX` discard, placeholder-backed late hidden service, shifted first-fill timing, and the updated startup seam contract directly.
- [PPU][MODE3-STARTUP-DUMMY-FIFO] Historical note: the intermediate March 28, 2026 baseline that seeded `8` real dummy startup pixels directly into the BG FIFO at `start_line()` is now superseded by [PPU][STARTUP-ALIGNMENT-SEAM]. Keep the old measurements in version history only; the current live DMG baseline no longer uses that direct line-start seeding path.
- [PPU][MODE3-STARTUP-DUMMY-FILL-SEAM] Historical note: the intermediate March 28, 2026 push-side rule that treated "FIFO contains only seeded startup dummy pixels" as empty for the first real fill is also superseded by [PPU][STARTUP-ALIGNMENT-SEAM]. The current baseline still preserves that hardware intent, but it now does so through the alignment-seam materialization path rather than through dummy pixels seeded into the FIFO at `start_line()`.
- [PPU][MODE3-FIFO-BACKED-HIDDEN-REAL-FIFO] The next startup follow-up slice is now landed too: the startup-effective-FIFO model no longer keeps using placeholder occupancy after the source window has already crossed into `FifoBacked`. The repo baseline now clears startup placeholders at the `AbstractStartup -> FifoBacked` seam, and once that seam is crossed the late hidden startup dots plus any remaining `SCX` discard must wait for and consume a real BG FIFO pixel instead of a synthetic placeholder. This closes one of the main remaining gaps between the earlier effective-FIFO model and the healthier docboy-style startup contract: post-priming hidden transfer is no longer "FIFO-backed in name only". Validation for this slice is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains byte-for-byte identical to the stable baseline, and `make ci` stays green.
- [PPU][MODE3-PUSH-CACHED-OBJ-HANDOFF] The next shared-fetcher follow-up slice is now landed too: push-side OBJ handoff no longer depends on pretending that the output-side transfer dot itself is already FIFO-backed and ready. When the push phase is holding one fetched BG/window tile but the effective BG FIFO is still logically non-empty, the PPU may now still cache that fetched tile and hand the same push dot to OBJ fetch if a pending OBJ hit owns the current transfer `X`, which matches the docboy `!can_push_to_bg_fifo && is_obj_ready_to_be_fetched()` branch more closely. This resolves the startup deadlock for very small OBJ `X` values without reopening the older external regressions from moving all OBJ-start decisions onto the output-side FIFO-backed branch.
- [PPU][MODE3-EXECUTION-KINDS-UPDATED] The current transfer-execution model has now also been tightened to match that startup-effective-FIFO slice. The explicit execution kinds are no longer "hidden without BG pop" versus "hidden with BG pop" only; they now distinguish `SCX`-discard consumption at `lx0`, pre-visible startup advance with effective-BG-FIFO consumption, hidden startup/output advance with effective BG and OBJ consumption, and visible pixel emission from a real BG pixel. That keeps the transfer-side seam debuggable while aligning the runtime model with the new placeholder-backed startup behavior.
- [PPU][MODE3-LATE-HIDDEN-OWNERSHIP-SEAM] The next structural follow-up slice is now landed too: the last four startup dots immediately before the first visible pixel are no longer modeled as generic pre-visible ownership. The PPU now treats those dots as hidden transfer ownership, so OBJ-hit ownership, `WX = 0`/`WX = 7` trigger eligibility, and the shared served-transfer-dot model all cross a more explicit `pre-visible -> hidden -> visible` seam. This slice is now closed together with the later startup-effective-FIFO follow-up: those late hidden dots no longer wait for a real BG FIFO pixel before they can be served, but they still consume one explicit effective-BG-FIFO slot per served dot and visible output itself still waits for a real BG pixel. That keeps the repo-managed DMG baseline green while moving startup timing materially closer to docboy's `lx0` startup model.
- [PPU][MODE3-PIPELINE-REGISTER-SNAPSHOT] The next live-write-sensitive slice is now landed too: the PPU no longer treats the current-dot-visible MMIO snapshot as the only register view inside `Mode 3`. It now keeps one second explicit pipeline-register snapshot for the previous-dot DMG view, and the current baseline uses that delayed snapshot for window activation (`WX`, `LCDC.5`) and for the ordinary visible-pixel `LCDC.BG_WIN_ENABLE` / `LCDC.OBJ_ENABLE` path, while still leaving the first visible pixel at `LX = 8` on its own explicit current-dot-visible `LCDC` exception hook. The same slice also tightens DMG BG palette visibility for visible pixels by resolving `BGP` through `current_visible_bgp | previous_pipeline_bgp`, which matches the docboy-style one-dot overlap model more closely and stays explicitly debuggable in repo-local tests. This cut is intentionally infrastructural as well as behavioral: it gives the remaining mealybug live-write families one explicit "current visible vs previous pipeline" seam instead of another spread of ad hoc register reads. Validation for this slice is closed under the hard gate too: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green. The repo-managed exploratory report did not move on this slice yet, which is acceptable because this cut is mainly a precondition for later live-write closure rather than a standalone oracle-closing change.
- [PPU][CPU-PPU-MMIO-PHASE7-COMMIT] The shared-scheduler mismatch between the documented phase contract and the runtime CPU->PPU MMIO path is now closed for the current DMG baseline. CPU-originated writes to PPU-owned MMIO registers on the real scheduler path no longer mutate PPU storage during `CpuMicroOperation`; they now stage there and commit during `MmioSideEffectCommit`, which keeps `LCDC` / `STAT` / scroll / window / DMG palette storage aligned with the documented `phase 6 -> phase 7 -> phase 8` ordering while preserving the already-landed one-dot lag from MMIO storage into the active `Mode 3` visible-register snapshots. Repo-local tests now cover both the staging helper boundary and the real CPU-driven scheduler path, and the remaining work in this lane narrows further: what is still open for mealybug is no longer the gross cross-phase CPU->PPU contract, but the finer `Mode 3` fetch / left-edge / previous-dot visibility families such as `m3_lcdc_bg_map_change`, `m3_lcdc_tile_sel_change*`, `m3_lcdc_win_map_change`, and `m3_scy_change`.
- [PPU][FETCH-PHASE-ADDRESS-STATE] The next fetcher-structure slice is now landed too: BG/window fetch no longer hides tilemap and tile-data address selection entirely inside read helpers. The fetcher now materializes the tilemap address at the tile-index phase and the tile-data address at each low/high data phase, and those addresses are exposed through the live PPU snapshot for debugging. The same slice keeps DMG `SCY` / `LCDC.4` behavior phase-local by recomputing the tile-data address independently for the low and high bitplanes, and it also keeps the docboy-style live `WIN_ENABLE` reprise explicit: if window fetch is already active and `LCDC.5` turns off, the fetcher can abort back to BG on the current fetch-stage boundary while restoring BG-side fetch progress from the saved resume pixel. This is still primarily a health/debuggability slice rather than a direct report-closing one, but it gives the remaining mealybug fetch-phase cases one much clearer state seam. Validation is closed under the hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green.
- [PPU][FETCH-PHASE-DOT-ORDER] The next follow-up slice is now landed too: BG/window fetch-phase side effects no longer occur one dot later than the explicit state machine suggests. The repo baseline now performs tilemap selection plus tile-number read on `TileIndex,0`, low-plane address selection plus data read on `TileDataLow,0`, and high-plane address selection plus data read on `TileDataHigh,0`; the matching `stage_dot = 1` positions are now pure transition dots into the next phase or into `Push`. This is another docboy-alignment slice rather than an immediately report-closing one, but it removes a known source of one-dot-late fetch behavior before the remaining mealybug live-write closure work. Validation is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green.
- [PPU][FIRST-WINDOW-PUSH-SEAM] The next window-specific follow-up slice is now landed too: the first window tile after activation no longer inherits the ordinary one-dot `Push` entry delay unconditionally. The repo baseline now keeps one explicit "first window tile after activation" seam from fetch into push, aligned with the docboy-style `w.just_activated` split, so the first activated window tile can skip the ordinary push-entry delay while later window tiles still use the normal cadence. This is still a structural/timing-health slice rather than a closure claim for the remaining mealybug window family, but it moves the first-window-tile cost model closer to the intended hardware contract without regressing the repo-managed DMG baseline. Validation is closed under the hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green.
- [PPU][FIRST-WINDOW-PUSH-OBJ-GATE] The next small follow-up slice on top of that seam is now landed too: the first activated window-tile push no longer hands its dot straight to OBJ fetch when a pending sprite hit already exists at the current transfer `X`. The repo baseline now keeps that "first activated window tile cannot immediately start OBJ fetch" rule explicit through push arbitration instead of letting it fall out of the generic push/OBJ overlap path. This again does not claim closure on the remaining mealybug window family, but it keeps the `w.just_activated`-style first-window-tile behavior aligned with docboy and more debuggable in the current DMG core. Validation is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green.
- [PPU][WINDOW-TILEMAP-X-SEAM] The next window-fetch follow-up slice is now landed too: the window fetcher no longer derives tilemap progress only from the generic fetch pixel counter. The repo baseline now keeps one explicit window tilemap-`X` counter and advances it on the `TileIndex,0` dot, matching the docboy-style `wf.tilemap_x++` seam more closely. This is still a structural/live-write-health slice rather than a claim that the mealybug `window map` family is fully closed, but it moves the `m3_lcdc_win_map_change` preconditions closer to hardware and leaves the first tilemap-selection dot far more debuggable than before. Validation is closed under the same hard gate: local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` remains free of diff, and `make ci` stays green.
- [PPU][TILE-SELECTOR-GLITCH-SEAM] The next live-write closure slice is now landed too: DMG `Mode 3` no longer treats `LCDC.4` / BG-window tile-data selector changes as a generic "next phase just uses the new value" rule everywhere. The repo baseline now keeps one explicit previous-dot/current-dot `LCDC` comparison during `TileDataLow,1` and `TileDataHigh,1`, plus one cache of the last unsigned tile-data byte, so BG/window fetch can model the localized docboy-style selector glitch instead of routing that behavior through broad fetcher delays. On top of that, active window fetch now uses the delayed pipeline `LCDC` snapshot for tilemap-base and tile-data-base selection during real `Mode 3` fetch, rather than only for `WX` / `WIN_ENABLE` activation. This keeps `acid/dmg-acid2.gb` green, keeps the materialized curated external suite green, keeps `.roms/test/test-report.md` free of diff, and keeps `make ci` green, while improving the exploratory mealybug tile-selector cases from `m3_lcdc_tile_sel_change: 1678 -> 1614` and `m3_lcdc_tile_sel_win_change: 1528 -> 1475`. The neighboring exploratory deltas are unchanged in this slice (`m3_lcdc_bg_map_change: 1042`, `m3_scy_change: 8245`), so the next work in this lane should move from tile-data-selector timing to the remaining `GetTile0` / tilemap-base seams behind `m3_lcdc_bg_map_change` and `m3_lcdc_win_map_change` rather than broadening the same glitch path again.
- [PPU][LEFT-EDGE-OBJ-START-GATE] The next left-edge timing slice is now landed too: the FIFO-backed OBJ-start path for low-`X` startup requests no longer becomes eligible as soon as the BG FIFO is merely non-empty and the BG fetcher has left `TileIndex`. In the current March 28, 2026 baseline, that path now also waits until BG has left `TileDataLow`, which keeps the left edge closer to docboy's "let the current BG slice really exist before OBJ steals the dot" seam. Validation is closed for this slice: repo-local PPU tests stay green, `acid/dmg-acid2.gb` stays green, the materialized curated external suite stays green, `.roms/test/test-report.md` stays free of diff, and `make ci` remains required before commit. Exploratory mealybug impact is narrow but real: `m3_scy_change` improves from `7763 -> 7266`, while the still-open left-edge `m3_lcdc_bg_map_change` (`978`) and `m3_lcdc_tile_sel_change` (`1614`) remain unchanged under this cut.
- [PPU][STARTUP-DUMMY-SEED-DEFERRED] A larger March 28, 2026 experiment tried to move the live DMG dummy-startup fill closer to docboy by discarding the first BG fetch and seeding the startup dummy FIFO only when that first fetch reached `Push`, instead of seeding `8` dummy pixels immediately at `start_line()`. That experiment produced one useful signal but was intentionally rejected rather than merged: it improved the exploratory `m3_lcdc_bg_map_change` delta materially (`978 -> 722`) and trimmed `m3_lcdc_tile_sel_change` slightly (`1614 -> 1581`), which strongly suggests the remaining left-edge live-write debt really does sit in the startup dummy/first-fetch seam; however, it also regressed repo-local raster tests (`mode3_bg_fetcher_fills_the_fifo_before_visible_pixels_begin`, `mode3_scx_discard_shifts_visible_pixels_and_delays_hblank_entry`, `tick_advances_the_raster_through_the_baseline_visible_line_modes`, and `turning_off_lcdc1_during_object_fetch_cancels_sprite_pixels_but_keeps_timing_cost`), broke `acid/dmg-acid2.gb`, and worsened `m3_scy_change` badly (`7266 -> 10099`). Follow-up conclusion: Phase `4` should keep this seam flagged as the most promising remaining left-edge target, but the eventual fix must preserve the stable startup timing and `acid` baseline instead of simply moving the dummy fill later wholesale.
- [PPU][STARTUP-ALIGNMENT-SEAM] The March 28, 2026 startup-alignment rewrite is now landed as the stable DMG baseline rather than remaining only an experiment. The PPU now gives the first startup BG fetch its own explicit alignment/discard path, materializes the remaining startup dummy occupancy from that alignment seam instead of seeding real dummy BG FIFO bytes at `start_line()`, allows only the truly pre-visible left-edge OBJ case to overlap that seam, and lets the first subsequent real BG/window push skip the ordinary one-dot push-entry delay and execute immediately from `TileDataHigh,1`. This closes the one-dot drift in `mode3_scx_discard_shifts_visible_pixels_and_delays_hblank_entry`, restores `acid/dmg-acid2.gb`, and keeps the repo-local PPU/unit/integration coverage, the materialized curated external suite, `.roms/test/test-report.md`, and `make ci` all green at the same time. Follow-up conclusion: the remaining left-edge debt is now narrower and better isolated; Phase `4` should keep pushing on the `bg_map / tile_sel / scy` live-write families from this new startup seam instead of reopening the older generic push/FIFO architecture.
- [PPU][PUSH-DOT-DECISION] The next shared-fetcher follow-up slice is now landed too: the BG/window push phase no longer hides all of its per-dot outcomes behind a boolean. The PPU now exposes explicit push-dot results for entry-delay wait, FIFO wait, OBJ handoff, queued fill, and the degenerate not-ready case. Repo-local tests now lock down each push-side outcome directly, including the no-pending-slice path and the object-handoff path, so later occupancy-policy changes do not have to rediscover the per-dot scheduler contract first. Validation for this slice is also closed under the hard gate: local PPU tests stay green, and the full repo validation remains the required gate before committing or keeping any later push-policy change.
- [PPU][MODE3-PUSH-ARBITRATION-DEFERRED] A follow-up attempt on March 26, 2026 to land those remaining `Mode 3` pieces together, specifically strict FIFO-empty BG push plus OBJ-start arbitration anchored to the BG fetcher's `Push` state, was intentionally rejected rather than merged. The branch-level experiment regressed the repo-managed external DMG baseline in multiple families at once: `mooneye acceptance/ppu/hblank_ly_scx_timing-GS`, `intr_2_mode0_timing`, `intr_2_oam_ok_timing`, and `manual-only/sprite_priority`, `hacktix/strikethrough`, and `mealybug-tearoom-tests ppu/m3_bgp_change` all flipped from green to red even though repo-local unit and machine tests were only partially upset. The branch was returned to the previously green baseline, `make ci` was revalidated, and the materialized `test-report.md` was restored byte-for-byte to the pre-experiment state. Follow-up conclusion: Phase `4` should not attempt another isolated "strict push" or "push-state-only OBJ start" change; the next productive slice has to rewrite more of the shared BG/window/OBJ fetcher contract at once, with external report comparison treated as a hard gate from the first commit.
