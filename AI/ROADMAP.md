...
2. **Phase 1B — Requester-aware arbitration and blocked-access policy**
  ...
  - the current repo baseline already has a pure router plus explicit `state`, `dispatch`, `IoHram`, `Wram`, `Oam`, and `Vram` helpers, bus-originated PPU-facing video views, scheduler-visible video ownership synchronization, and a dedicated bus child module for DMG OAM-corruption trigger routing
