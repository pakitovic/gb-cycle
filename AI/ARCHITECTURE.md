...
- For the current DMG-first repo baseline, a concrete split of `bus.rs` plus `bus/map.rs`, `bus/router.rs`, `bus/policy.rs`, `bus/access.rs`, `bus/corruption.rs`, `bus/iohram.rs`, `bus/wram.rs`, `bus/video.rs`, and `bus/view.rs` is the preferred shape.
